 ...
 * Identifies what type of IP input the user is entering (IPv4, IPv6, CIDR, Range, hostnames, etc.)
 */
...
export function isHostname(input) {
  ...
  if (isValidIPv4(input) || isValidIPv6(input)) return false

  // Valid hostname pattern: alphanumeric, hyphens, dots
  // Must have at least one dot (domain format) or be localhost
  const hostnameRegex = /^([a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$|^localhost$/
...
export function detectIPInputType(input) {
  ...
  // Check for hostname/domain (before IP checks since some could be ambiguous)
  if (isHostname(trimmed)) {
    return {
      type: 'Hostname',
      description: `Domain name (${trimmed})`,
      hostname: trimmed,
      confidence: 0.95,
      isHostname: true,
    }
  ...
  // Check for IP range (e.g., "192.168.1.1 - 192.168.1.10", "192.168.1.1 to 192.168.1.10", etc.)
  const rangeMatch = trimmed.match(/^([0-9a-fA-F:.]+)\s+(?:to|[-–—])\s+([0-9a-fA-F:.]+)$/)
  if (rangeMatch) {
  ...
  // Range candidate
  const rangeCandidateMatch = trimmed.match(/^([0-9a-fA-F:.]+)\s+(?:to|[-–—])\s*/)
