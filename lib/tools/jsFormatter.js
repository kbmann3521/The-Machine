...
async function buildJsDiagnostics(code, { showErrors, showLinting }) {
  const structuralErrors = collectStructuralDiagnosticsJS(code);
  const syntax = showErrors ? detectSyntaxErrors(code) : { status: 'unknown', errors: [] };

  let linting = { total: 0, warnings: [] };
  if (showLinting) {
    linting = await lintJavaScript(code);
  }

  const diagnostics = [];

  // Structural diagnostics (already in diag shape)
  diagnostics.push(...structuralErrors);

  // Strict syntax diagnostics
  if (syntax.status === 'invalid') {
    syntax.errors.forEach((e) => {
      diagnostics.push({
        type: 'error',
        category: 'syntax',
        message: e.message,
  ...
  // Lint diagnostics
  if (showLinting && linting && Array.isArray(linting.warnings)) {
    linting.warnings.forEach((w) => {
      diagnostics.push({
  ...
  const isWellFormed = structuralErrors.length === 0 && syntax.status !== 'invalid';
...
async function jsFormatter(text, config = {}) {
  ...
  try {
    ...
    // Build structural and syntax diagnostics ON ORIGINAL INPUT
    const structuralErrors = collectStructuralDiagnosticsJS(trimmedText);
    const syntax = showErrors ? detectSyntaxErrors(trimmedText) : { status: 'unknown', errors: [] };
    const isWellFormed = structuralErrors.length === 0 && syntax.status !== 'invalid';
    ...
    switch (mode) {
      case 'format': {
        ...
        // Only pretty-print if code appears well-formed
        if (isWellFormed) {
      ...
      case 'minify': {
        let minified = trimmedText;

        // Only aggressively minify if code is well-formed
        if (isWellFormed) {
          minified = await minifyJavaScript(trimmedText, { compressCode });
        }
      ...
      case 'obfuscate': {
        let obfuscated = trimmedText;

        if (isWellFormed) {
          obfuscated = obfuscateJavaScript(trimmedText);
        }
    ...
    // Build linting on OUTPUT CODE (so formatting options are reflected)
    let linting = { total: 0, warnings: [] };
    if (showLinting) {
      linting = await lintJavaScript(outputCode);
    }

    // Build consolidated diagnostics array
    const diagnostics = [];
    ...
    // Add lint warnings (from output code)
    if (showLinting && linting && Array.isArray(linting.warnings)) {
      linting.warnings.forEach((w) => {
        diagnostics.push({
          type: w.level === 'error' ? 'error' : 'warning',
    ...
    if (showErrors) {
      result.errors = syntax; // { status, errors }
    }
    if (showLinting) {
      result.linting = linting; // { total, warnings }
    }
