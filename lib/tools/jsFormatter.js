...
async function jsFormatter(text, config = {}) {
  ...
  try {
    ...
    // Build diagnostics ON ORIGINAL INPUT (not formatted/minified/obfuscated)
    const diag = await buildJsDiagnostics(trimmedText, {
      showErrors,
      showLinting,
    });
    ...
    if (showErrors) {
      result.errors = diag.syntax; // { status, errors }
    }
    if (showLinting) {
      result.linting = diag.linting; // { total, warnings }
    }

    result.diagnostics = diag.diagnostics; // unified array
    result.isWellFormed = diag.isWellFormed;
