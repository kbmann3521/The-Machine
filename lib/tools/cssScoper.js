 ...
 */
function scopeCss(css, scopeClass = '.pwt-markdown-preview') {
  ...
  while (i < css.length) {
    ...
    if (css[i] === '@') {
      ...
      // For media queries and other at-rules, recursively scope the inner rules
      if (atRuleHeader.includes('@media')) {
        const scopedContent = scopeCss(atRuleContent, scopeClass)
        result += atRuleHeader + scopedContent + '}\n'
      } else {
        // For other @rules like @keyframes, don't scope the inner content
    ...
    if (selector.includes(cleanScope)) {
      ...
      const selectors = selector.split(',').map(sel => {
        ...
        // For all selectors, prepend the scope class with descendant combinator
        return `${cleanScope} ${sel}`
      }).filter(s => s)
 ...
 */
function isCssScoped(css, scopeClass = '.pwt-markdown-preview') {
