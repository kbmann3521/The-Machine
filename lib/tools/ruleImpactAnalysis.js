 ...
 */
export function isRuleRedundant(
  ruleImpact,
  ...
  // Only count non-disabled properties
  ...
  affectedNodes.forEach(node => {
    ...
    nodeProps.forEach(prop => {
      // Skip disabled properties (they're not contributing to the rule anyway)
      const disabledKey = `${currentRuleIndex}-${prop.property}`
      const isDisabled = disabledProperties.has(disabledKey)
      if (isDisabled) {
        console.log('[isRuleRedundant] Skipping disabled property:', disabledKey)
        return // Skip this property
      }

      totalProperties++

      // Check if property is truly overridden by considering disabled properties in later rules
      // Use the isPropertyOverriddenByLaterRule function if available, otherwise fall back to ruleImpact.effective
      let isTrulyOverridden = prop.effective === false
      if (isPropertyOverriddenByLaterRule && selector) {
        isTrulyOverridden = isPropertyOverriddenByLaterRule(currentRuleIndex, selector, prop.property)
      }

      if (isTrulyOverridden) {
        overriddenProperties++
      }
      console.log('[isRuleRedundant] Property:', prop.property, 'effective:', prop.effective, 'trulyOverridden:', isTrulyOverridden, 'total:', totalProperties, 'overridden:', overriddenProperties)
    })
  })

  console.log('[isRuleRedundant] Final count - total:', totalProperties, 'overridden:', overriddenProperties)
