...
function createRuleNegationCSS(selector, declarations = []) {
  ...
  const negations = declarations
    .map(decl => `${decl.property}: initial !important`)
    .join('; ')
...
function measureRuleImpact(selector, declarations, iframeDoc) {
  ...
  try {
    ...
    declarations.forEach(decl => {
      ...
      // If negation changed the computed style, the rule was having an effect
 ...
 *
 * Walks the rules tree from ruleIndex+1 onwards to see if any later rule
 * also declares the same property for the same or higher specificity.
 */
function findOverridingRule(selector, property, ruleIndex, rulesTree = []) {
  if (!rulesTree || rulesTree.length === 0) return null

  for (let i = ruleIndex + 1; i < rulesTree.length; i++) {
    const laterRule = rulesTree[i]
    ...
    if (laterRule.selector === selector) {
      ...
      if (hasProperty && (laterRule.specificity || 0) >= (rulesTree[ruleIndex].specificity || 0)) {
        return {
          ruleIndex: i,
    ...
    if (laterRule.selector.startsWith(selector + ':')) {
      const hasProperty = (laterRule.declarations || []).some(d => d.property === property)
      if (hasProperty && (laterRule.specificity || 0) > (rulesTree[ruleIndex].specificity || 0)) {
        return {
          ruleIndex: i,
    ...
    if (laterRule.children && Array.isArray(laterRule.children)) {
      for (let j = 0; j < laterRule.children.length; j++) {
        const childRule = laterRule.children[j]
        if (childRule.selector === selector) {
          const hasProperty = (childRule.declarations || []).some(d => d.property === property)
          if (hasProperty && (childRule.specificity || 0) >= (rulesTree[ruleIndex].specificity || 0)) {
            return {
              ruleIndex: i,
              childIndex: j,
...
export function computeRuleImpact(ruleIndex, selector, declarations, iframeDoc, rulesTree = []) {
  ...
  try {
    // Get all elements that match this selector in the preview
    const elements = iframeDoc.querySelectorAll(selector)
    if (!elements || elements.length === 0) {
      return {
        ruleIndex,
