...
function parseSimpleCss(css, options = {}) {
  ...
  // Find all @media rules with their positions
  let mediaRegex = /@media\s+([^{]+)\s*\{/g
  let mediaMatch
  while ((mediaMatch = mediaRegex.exec(css)) !== null) {
    const mediaStart = mediaMatch.index
    const mediaQuery = mediaMatch[1].trim()
    const braceStart = mediaStart + mediaMatch[0].length - 1
  ...
  while ((ruleMatch = ruleRegex.exec(css)) !== null) {
    // Check if this rule is inside a @keyframes or @media block
    const isInsideSpecialBlock = rulePositions.some(
      block => ruleMatch.index >= block.start && ruleMatch.index < block.end && (block.type === 'keyframes' || block.type === 'media')
    )
  ...
  for (const rulePos of rulePositions) {
    ...
    } else if (rulePos.type === 'media') {
      // Parse @media rules
      const mediaChildren = []
      const mediaRuleRegex = /([^{}]+?)\s*\{\s*([^}]*)\}/g
      let mediaRuleMatch

      while ((mediaRuleMatch = mediaRuleRegex.exec(rulePos.content)) !== null) {
        let selectorText = mediaRuleMatch[1].trim()
        const declarationsText = mediaRuleMatch[2].trim()

        // Skip empty selectors or nested at-rules
        if (!selectorText || selectorText.startsWith('@')) {
          continue
        }
