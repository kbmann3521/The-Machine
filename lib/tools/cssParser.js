...
function parseSimpleCss(css, options = {}) {
  ...
  // First, extract @keyframes before parsing regular selectors
  // AND build a map of keyframes ranges to skip in regular rule parsing
  let cssPos = 0
  const keyframesRanges = [] // Track which character ranges are @keyframes
  while (cssPos < css.length) {
    const keyframesStart = css.indexOf('@keyframes', cssPos)
    if (keyframesStart === -1) break

    // Find the name (word after @keyframes)
    const nameMatch = css.substring(keyframesStart).match(/@keyframes\s+([a-zA-Z0-9_-]+)\s*\{/)
    if (!nameMatch) {
      cssPos = keyframesStart + 1
      continue
    }

    const keyframesName = nameMatch[1]
    const braceStart = keyframesStart + nameMatch[0].length - 1
    ...
    if (braceCount === 0) {
      // Track this range to skip later
      keyframesRanges.push({ start: keyframesStart, end: braceEnd + 1 })

      const keyframesContent = css.substring(braceStart + 1, braceEnd)

      // Parse keyframe selectors (0%, 50%, 100%, from, to)
      const keyframeSelectorsRegex = /([0-9.%a-z]+)\s*\{([^}]*)\}/gi
      const keyframeChildren = []
      let kMatch

      while ((kMatch = keyframeSelectorsRegex.exec(keyframesContent)) !== null) {
      ...
      if (keyframeChildren.length > 0) {
        rules.push({
          ...
          loc: {
            startLine: getLineNumberAtPosition(css, keyframesStart) + lineOffset,
            endLine: getLineNumberAtPosition(css, braceEnd) + lineOffset,
          },
          origin: {
            source: options.origin || 'css',
            containerId: options.containerId || 'default',
  ...
  // Helper to check if a match position is inside a @keyframes block
  const isInsideKeyframes = (position) => {
    return keyframesRanges.some(range => position >= range.start && position < range.end)
  }

  // Match regular CSS rules: selector { declarations }
  // This regex finds content before opening brace, then captures declarations
  const ruleRegex = /([^{}@]+?)\s*\{\s*([^}]*)\}/g
  let match

  while ((match = ruleRegex.exec(css)) !== null) {
    // IMPORTANT: Skip any matches that are inside @keyframes blocks
    if (isInsideKeyframes(match.index)) {
      continue
    }
