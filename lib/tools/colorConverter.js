...
function labToRgb(l, a, b) {
  ...
  let b_out = x * 0.0557 + y * -0.2040 + z * 1.0570
  ...
  return {
    ...
    b: Math.max(0, Math.min(255, Math.round(b_out * 255))),
  }
}
...
function interpolateOKLab(lab1, lab2, progress) {
  // Linear interpolation in OKLab space (perceptually uniform)
  const l = lab1.l + (lab2.l - lab1.l) * progress
  const a = lab1.a + (lab2.a - lab1.a) * progress
  const b = lab1.b + (lab2.b - lab1.b) * progress
  ...
  return {
    ...
    b: Math.max(0, Math.min(1, toSRGB(b_))),
  }
}
...
function rgbToOKLab(r, g, b) {
  ...
  const l = 0.4121656120 * r_lin + 0.5362752080 * g_lin + 0.0514575653 * b_lin
  const m = 0.2119034982 * r_lin + 0.6807612419 * g_lin + 0.1073790571 * b_lin
  const s = 0.1929763977 * r_lin + 0.0829488751 * g_lin + 0.9478674713 * b_lin
...
function generateLinearGradient(startColor, endColor, steps = 5, angle = 90, mode = 'rgb', reversed = false) {
  try {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      if (mode === 'oklab') {
        ...
        const oklab1 = rgbToOKLab(result1.rgb.r, result1.rgb.g, result1.rgb.b)
        const oklab2 = rgbToOKLab(result2.rgb.r, result2.rgb.g, result2.rgb.b)
        const interpolated = interpolateOKLab(oklab1, oklab2, progress)
        r = Math.round(interpolated.r * 255)
        g = Math.round(interpolated.g * 255)
        b = Math.round(interpolated.b * 255)
      } else if (mode === 'hsl') {
        // Interpolate in HSL space with proper hue handling (shortest path)
        let h1 = result1.hsl.h
        let h2 = result2.hsl.h
        let dh = h2 - h1
        ...
        const h = (h1 + dh * progress + 360) % 360
        const s = result1.hsl.s + (result2.hsl.s - result1.hsl.s) * progress
        const l = result1.hsl.l + (result2.hsl.l - result1.hsl.l) * progress
        const rgb = hslToRgb(h, s, l)
        r = rgb.r
        g = rgb.g
        b = rgb.b
      } else if (mode === 'lab') {
        // Interpolate in LAB space (perceptually even)
        const lab1 = rgbToLab(result1.rgb.r, result1.rgb.g, result1.rgb.b)
        const lab2 = rgbToLab(result2.rgb.r, result2.rgb.g, result2.rgb.b)
        const l = lab1.l + (lab2.l - lab1.l) * progress
        const a = lab1.a + (lab2.a - lab1.a) * progress
        const labB = lab1.b + (lab2.b - lab1.b) * progress
        const rgb = labToRgb(l, a, labB)
        r = rgb.r
        g = rgb.g
        b = rgb.b
      } else if (mode === 'lch') {
        // Interpolate in LCH space (best for modern UI design)
        const lch1 = rgbToLch(result1.rgb.r, result1.rgb.g, result1.rgb.b)
        const lch2 = rgbToLch(result2.rgb.r, result2.rgb.g, result2.rgb.b)

        // Handle hue interpolation (shortest path around the wheel)
        let h1 = lch1.h
        let h2 = lch2.h
        let dh = h2 - h1
        ...
        const l = lch1.l + (lch2.l - lch1.l) * progress
        const c = lch1.c + (lch2.c - lch1.c) * progress
        const h = (h1 + dh * progress + 360) % 360
        ...
        r = Math.round(result1.rgb.r + (result2.rgb.r - result1.rgb.r) * progress)
        g = Math.round(result1.rgb.g + (result2.rgb.g - result1.rgb.g) * progress)
        b = Math.round(result1.rgb.b + (result2.rgb.b - result1.rgb.b) * progress)
      }
    ...
    if (reversed) {
      colors = colors.reverse().map((c, idx) => ({
        ...c,
        percentage: Math.round((idx / (colors.length - 1)) * 100),
      }))
    }
    ...
    // Generate Tailwind gradient class (simplified)
    const tailwindStartColor = colors[0].hex.toLowerCase().replace('#', '')
    const tailwindEndColor = colors[colors.length - 1].hex.toLowerCase().replace('#', '')
    const tailwindClass = `bg-gradient-to-r from-[#${tailwindStartColor}] to-[#${tailwindEndColor}]`

    return {
      gradient: {
        startColor: reversed ? endColor : startColor,
        endColor: reversed ? startColor : endColor,
        angle,
        steps,
        mode,
        reversed,
...
function exportPaletteASE(colors) {
  ...
  // Note: Full binary ASE export requires Node.js or specialized tools
  const colorEntries = colors.map(color => {
    ...
    return {
      hex: result.formats.hex,
      name: color,
      r: Math.round(result.rgb.r / 255 * 100),
      g: Math.round(result.rgb.g / 255 * 100),
      b: Math.round(result.rgb.b / 255 * 100),
    }
  }).filter(Boolean)

  const aseData = {
    format: 'Adobe Swatch Exchange',
    colors: colorEntries,
    note: 'For binary ASE import into Adobe apps, convert this JSON using online ASE converters or Node.js libraries',
  }
  ...
  return {
    ...
    filename: 'palette.json',
    note: 'ASE binary format requires external conversion tools.',
  }
}
...
function colorConverter(text, config) {
  try {
    ...
    return {
      ...
      ...(config?.gradientEndColor ? generateLinearGradient(text, config.gradientEndColor, 5, 90, config?.gradientMode || 'rgb', config?.gradientReversed || false) : {}),
    }
  } catch (error) {
...
function getSuggestionsForColor(input) {
  if (!input || input.length === 0) return []

  const lowercaseInput = input.toLowerCase()
  const matches = Object.keys(CSS_NAMED_COLORS)
    .filter(colorName => colorName.includes(lowercaseInput))
    .slice(0, 5)
    .map(colorName => ({
      name: colorName,
      hex: CSS_NAMED_COLORS[colorName],
    }))
