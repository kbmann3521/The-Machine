...
export function mergeRuleGroup(group, preferredDirection = 'css') {
  ...
  // CRITICAL: When ONLY CSS or ONLY HTML has duplicates (no cross-tab), only remove duplicates from THAT source
  let removedRuleIndices = []

  if (htmlHasDuplicates && !cssHasDuplicates && !isCrossTabDuplicate) {
    // Only HTML has duplicates → remove duplicate HTML rules ONLY
    // Keep all CSS rules as-is
    removedRuleIndices = htmlRules.filter(r => r.ruleIndex !== baseRule.ruleIndex).map(r => r.ruleIndex)
    console.log(`  → Removing HTML duplicates:`, removedRuleIndices)
  } else if (cssHasDuplicates && !htmlHasDuplicates && !isCrossTabDuplicate) {
    // Only CSS has duplicates → remove duplicate CSS rules ONLY
    // Keep all HTML rules as-is
    removedRuleIndices = cssRules.filter(r => r.ruleIndex !== baseRule.ruleIndex).map(r => r.ruleIndex)
    console.log(`  → Removing CSS duplicates:`, removedRuleIndices)
  } else {
    // Both sources have duplicates OR cross-tab duplicates → remove ALL except base rule
    removedRuleIndices = group.rules.filter(r => r.ruleIndex !== baseRule.ruleIndex).map(r => r.ruleIndex)
    console.log(`  → Removing all duplicates (both sources):`, removedRuleIndices)
  }
 ...
 */
function findRuleByIndex(rules, targetRuleIndex, currentPath = [], depth = 0) {
  for (let i = 0; i < rules.length; i++) {
    const rule = rules[i]
    if (rule.ruleIndex === targetRuleIndex) {
      const path = [...currentPath, i]
      console.log(`    [findRuleByIndex] FOUND ruleIndex ${targetRuleIndex} at path ${JSON.stringify(path)}`)
      return { path, rule }
    }
    if (rule.children && Array.isArray(rule.children)) {
      const found = findRuleByIndex(rule.children, targetRuleIndex, [...currentPath, i, 'children'], depth + 1)
      if (found) return found
    }
  }
  if (depth === 0) {
    console.log(`    [findRuleByIndex] NOT FOUND ruleIndex ${targetRuleIndex} in rules tree`)
  }
  return null
}
...
export function mergeSelectedGroups(rulesTree = [], selectedSelectors = new Set(), preferredDirection = 'css') {
  ...
  selectedMergeableGroups.forEach(group => {
    ...
    // Find the merged rule by its ruleIndex (which is the base rule's index)
    // The mergedRule.ruleIndex tells us which source the merged rule belongs to
    const mergedRuleInfo = findRuleByIndex(mutatedRulesTree, mergedRule.ruleIndex)
    if (mergedRuleInfo) {
      ...
      console.log(`  WARNING: Could not find merged rule with ruleIndex=${mergedRule.ruleIndex} in tree!`)
    }

    // Mark others for removal
    removedRuleIndices.forEach(idx => {
      allRemovedIndices.add(idx)
      console.log(`  Adding ruleIndex ${idx} to removal list`)
    })
  ...
  allRemovedIndices.forEach(ruleIndex => {
    const found = findRuleByIndex(mutatedRulesTree, ruleIndex)
    if (found) {
      console.log(`  Found ruleIndex ${ruleIndex} at path: ${JSON.stringify(found.path)}`)
      pathsToRemove.push({ path: found.path, ruleIndex })
    } else {
      console.log(`  WARNING: Could not find ruleIndex ${ruleIndex} in mutatedRulesTree!`)
  ...
  pathsToRemove.forEach(item => {
    console.log(`  Path: ${JSON.stringify(item.path)}, ruleIndex: ${item.ruleIndex}`)
  })

  // Remove rules starting from highest index to lowest
  pathsToRemove.forEach(({ path, ruleIndex }) => {
    console.log(`[mergeSelectedGroups] REMOVING rule at path ${JSON.stringify(path)} (ruleIndex=${ruleIndex})`)
    removeAtPath(mutatedRulesTree, path)
  })
