 ...
 *
 * Algorithm:
 * 1. Collect all declarations in order
 * 2. Keep each declaration with source tracking (which rule it came from)
 * 3. Later declarations naturally override earlier ones in the merged rule
 *
...
export function mergeRuleGroup(group) {
  ...
  // Preserve order: iterate through rules in sequence
  group.rules.forEach((rule, ruleSeq) => {
    ...
    rule.declarations.forEach(decl => {
      mergedDeclarations.push({
        ...
        value: decl.value,
        sourceRule: ruleSeq, // Track which rule this came from
...
/**
 * Apply merge by injecting properties and removing duplicate rules from source text
 *
 * Algorithm:
 * 1. Map which lines to delete (all lines from duplicate rules)
 * 2. Map which lines to insert properties before (the closing brace of first rule)
 * 3. Rebuild the output by iterating through source lines:
 *    - Skip lines marked for deletion
 *    - Insert new properties before the closing brace of surviving rules
 *    - Keep all other lines
 * 4. Clean up excess whitespace
 *
 * mergeInfo format: { mergeGroups }
 */
export function applyMergeToSourceText(sourceText, rulesTree, mergeInfo) {
  ...
  // If called with old API (rulesToRemove as Set), handle it
  ...
  const linesToDelete = new Set() // Lines to completely skip
  const propertiesToInsert = new Map() // lineIdx -> { properties, indent }

  for (const group of mergeGroups) {
    if (group.rules.length < 2) continue

    const firstRule = group.rules[0]
    const restRules = group.rules.slice(1)

    // Collect all properties from rules being merged into the first rule
    const newProperties = []
    for (const rule of restRules) {
      if (rule.declarations && rule.declarations.length > 0) {
        rule.declarations.forEach(decl => {
          // Only add if not already in first rule (later declarations override)
          const existsInFirst = firstRule.declarations?.some(d => d.property === decl.property)
          if (!existsInFirst) {
            newProperties.push(decl)
          }
        })
      }
    }

    // Mark properties to insert at the end of the first rule
    if (firstRule.loc && firstRule.loc.endLine && newProperties.length > 0) {
      const endLineIdx = firstRule.loc.endLine - 1 // Convert to 0-based

      // Detect indentation from first rule's properties
      let indent = '  '
      for (let i = firstRule.loc.startLine; i < firstRule.loc.endLine; i++) {
      ...
      propertiesToInsert.set(endLineIdx, { properties: newProperties, indent })
    }

    // Mark all lines of duplicate rules for deletion
    for (const ruleToRemove of restRules) {
      if (ruleToRemove.loc && ruleToRemove.loc.startLine && ruleToRemove.loc.endLine) {
        for (let lineNum = ruleToRemove.loc.startLine; lineNum <= ruleToRemove.loc.endLine; lineNum++) {
  ...
  // Rebuild output by processing source lines
  const result = []
  ...
  for (let lineIdx = 0; lineIdx < sourceLines.length; lineIdx++) {
    ...
    // Check if we need to insert properties before this line (the closing brace)
    if (propertiesToInsert.has(lineIdx)) {
      const { properties, indent } = propertiesToInsert.get(lineIdx)
  ...
  // Clean up excessive whitespace (collapse multiple blank lines to single blank line)
  const finalResult = []
