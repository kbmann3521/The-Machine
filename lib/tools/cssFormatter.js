...
function analyzeCss(text) {
  ...
  try {
    ...
    normalizedText = normalizedText.replace(/([a-z0-9\]\)"\'\*])(\{)/gi, '$1 $2')

    const root = postcssModule.parse(normalizedText)
...
function validateCss(text) {
  ...
  try {
    ...
    csstreeModule.parse(normalizedText, {
      positions: true,
...
async function lintCss(text) {
  ...
  try {
    // Normalize CSS: add space before opening braces if missing (e.g., *{ → * {)
    // This ensures compatibility with stylelint and other CSS parsers
    let normalizedText = text
    normalizedText = normalizedText.replace(/([a-z0-9\]\)"\'\*])(\{)/gi, '$1 $2')
...
async function cssFormatter(text, config = {}) {
  ...
  try {
    ...
    processingText = processingText.replace(/([a-z0-9\]\)"\'\*])(\{)/gi, '$1 $2')
