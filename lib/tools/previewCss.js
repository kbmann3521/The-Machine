    ...
    if (rule.type === 'rule' && rule.selector) {
      ...
      if (rule.declarations && Array.isArray(rule.declarations)) {
        for (const decl of rule.declarations) {
          const resolvedValue = resolveValueVariables(
            decl.value,
            declaredVariables,
            variableOverrides
          )

          // Phase 7D: Comment out disabled properties instead of removing them
          const disabledKey = `${rule.ruleIndex}-${decl.property}`
          const isDisabled = disabledProperties.has(disabledKey)

          if (isDisabled) {
            // Comment out the property (disabled via what-if simulation)
...
function buildPreviewDocument(
  ...
  const css = buildPreviewCss(rulesTree, declaredVariables, variableOverrides, disabledProperties)
