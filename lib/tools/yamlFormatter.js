...
function yamlFormatter(text, config) {
  ...
  const convertTabsToSpaces = config.convertTabsToSpaces === true
  ...
  try {
    ...
    // Apply tab-to-space conversion FIRST (before validation)
    ...
    // Apply tab-to-space conversion if enabled
    if (convertTabsToSpaces && typeof formatted === 'string') {
