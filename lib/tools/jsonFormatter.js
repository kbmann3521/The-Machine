...
function sortJSONKeys(text, config) {
  ...
  try {
    ...
    // Apply removal options if enabled
    let result = sorted
    if (config.removeEmptyArrays || config.removeEmptyStrings) {
      result = removeNullRecursive(sorted, {
        removeNull: false,
...
function flattenJSON(text, config) {
  ...
  try {
    ...
    let flattened = flattenRecursive(parsed)

    // Apply removal options if enabled
    if (config.removeEmptyArrays || config.removeEmptyStrings) {
      flattened = removeNullRecursive(flattened, {
        removeNull: false,
        removeEmptyArrays: config.removeEmptyArrays,
        removeEmptyStrings: config.removeEmptyStrings,
      })
...
function unflattenJSON(text, config) {
  ...
  try {
    const parsed = JSON.parse(trimmed)
    let unflattened = unflattenRecursive(parsed)

    // Apply removal options if enabled
    if (config.removeEmptyArrays || config.removeEmptyStrings) {
      unflattened = removeNullRecursive(unflattened, {
        removeNull: false,
        removeEmptyArrays: config.removeEmptyArrays,
        removeEmptyStrings: config.removeEmptyStrings,
      })
...
function jsonMinifier(text, config = {}) {
  const trimmed = text.trim()
  if (!trimmed) {
    return ''
  }
  ...
  try {
    ...
    // Apply removal options if enabled (even in minify mode)
    if (config.removeEmptyArrays || config.removeEmptyStrings) {
      parsed = removeNullRecursive(parsed, {
        removeNull: false,
        removeEmptyArrays: config.removeEmptyArrays,
...
function jsonFormatter(text, config = {}) {
  ...
  if (isWellFormed) {
    try {
      switch (config.mode) {
        case 'minify':
          formatted = jsonMinifier(trimmed, config)
          break
        case 'beautify':
