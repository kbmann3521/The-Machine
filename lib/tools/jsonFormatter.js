...
function removeNullRecursive(obj, options) {
  if (Array.isArray(obj)) {
    let filtered = obj
      ...
      .filter(item => {
        ...
        if (typeof item === 'object' && item !== null && !Array.isArray(item) && Object.keys(item).length === 0 && options.removeEmptyObjects) return false
        return true
      })
    return filtered
  }
  ...
  if (obj !== null && typeof obj === 'object') {
    ...
    for (const key in obj) {
      ...
      // Remove empty objects
      if (typeof value === 'object' && value !== null && !Array.isArray(value) && Object.keys(value).length === 0 && options.removeEmptyObjects) continue
      result[key] = value
    }
    return result
  }
