...
function checksumCalculator(text, config = {}) {
  ...
  try {
    ...
    if (compareMode && compareText) {
      ...
      if (parsed2.error) {
        result.compareResult = { error: parsed2.error }
      } else {
        ...
        const match = (hashHex || checksum.toString(16)) === (hashHex2 || checksum2.toString(16))
        result.compareResult = {
          input2: compareText,
          checksum2: hashHex2 || checksum2.toString(10),
          hex2: hashHex2 || '0x' + checksum2.toString(16).toUpperCase().padStart(8, '0'),
          match,
