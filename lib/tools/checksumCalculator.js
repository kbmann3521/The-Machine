...
/**
 * Format checksum value based on output format
 */
function formatChecksumValue(value, format) {
  const isHash = typeof value === 'string' && value.length > 8
  ...
  if (isHash) {
    switch (format) {
      ...
      case 'hex-plain':
        return value
      case 'binary':
        return parseInt(value.substr(0, 8), 16).toString(2).padStart(32, '0')
      case 'bytes-be': {
        const bytes = []
        for (let i = 0; i < value.length; i += 2) {
          bytes.push(value.substr(i, 2))
        }
        return bytes.join(' ')
      }
      case 'bytes-le': {
    ...
    switch (format) {
      ...
      case 'binary':
        return numValue.toString(2).padStart(32, '0')
      case 'bytes-be': {
        const hex = numValue.toString(16).padStart(8, '0').toUpperCase()
        const bytes = []
        for (let i = 0; i < 8; i += 2) {
          bytes.push(hex.substr(i, 2))
        }
...
/**
 * Generate all format conversions for a checksum value
 */
function generateAllFormats(value) {
  const isHash = typeof value === 'string' && value.length > 8
  ...
  if (isHash) {
    ...
    conversions.bytesBE = bytes.join(' ')

    const bytesLE = []
    for (let i = value.length - 2; i >= 0; i -= 2) {
      bytesLE.push(value.substr(i, 2))
    }
    conversions.bytesLE = bytesLE.join(' ')

    return conversions
  } else {
    const numValue = parseInt(value, 10)
    const hexStr = numValue.toString(16).padStart(8, '0').toUpperCase()
    const bytes = []
    for (let i = 0; i < 8; i += 2) {
    ...
    return {
      decimal: numValue.toString(10),
      hex: '0x' + hexStr,
      hexPlain: hexStr,
      binary: numValue.toString(2).padStart(32, '0'),
      bytesBE: bytes.join(' '),
      bytesLE: [...bytes].reverse().join(' '),
    }
  }
}
...
function checksumCalculator(text, config = {}) {
  ...
  if (autoDetect) {
    ...
    isAutoDetected = detected.detected
  }
  ...
  try {
    ...
    const allConversions = generateAllFormats(checksum)

    // Select the primary output format
    const primaryOutput = formatChecksumValue(checksum, outputFormat)
    ...
    // Handle compare mode
    if (compareMode && compareText && compareText.trim()) {
      const parsed2 = parseInputBytes(compareText, inputMode)
      if (parsed2.error) {
        ...
        const bytes2 = parsed2.bytes
        let checksum2
        let hashHex2

        // Calculate checksum for second input
        if (['md5', 'sha1', 'sha256', 'sha512'].includes(algorithm)) {
          const hashAlgo = algorithm === 'md5' ? 'md5' : algorithm === 'sha1' ? 'sha1' : algorithm === 'sha256' ? 'sha256' : 'sha512'
          hashHex2 = crypto.createHash(hashAlgo).update(bytes2).digest('hex').toUpperCase()
          checksum2 = hashHex2
        } else {
          switch (algorithm) {
            case 'crc32': checksum2 = crc32(bytes2); break
            case 'crc32c': checksum2 = crc32c(bytes2); break
            case 'crc16': checksum2 = crc16(bytes2); break
            case 'crc16ccitt': checksum2 = crc16ccitt(bytes2); break
            case 'crc8': checksum2 = crc8(bytes2); break
            case 'adler32': checksum2 = adler32(bytes2); break
            case 'fletcher16': checksum2 = fletcher16(bytes2); break
            case 'fletcher32': checksum2 = fletcher32(bytes2); break
            case 'simple': checksum2 = simpleChecksum(bytes2); break
            case 'bsd': checksum2 = bsdChecksum(bytes2); break
            default: checksum2 = null
          }
          hashHex2 = null
        }

        const conversions2 = generateAllFormats(checksum2)
        const primaryOutput2 = formatChecksumValue(checksum2, outputFormat)

        // Compare the checksums
        const checksumStr1 = hashHex || checksum.toString(16).toUpperCase()
        const checksumStr2 = hashHex2 || checksum2.toString(16).toUpperCase()
        const match = checksumStr1 === checksumStr2

        result.compareResult = {
          input2: compareText,
          byteLength2: bytes2.length,
          primaryOutput2,
          conversions2,
          match,
          checksumStr1,
