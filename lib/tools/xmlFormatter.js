...
/**
 * Auto-repair malformed XML using a 5-pass recovery pipeline.
 ...
 * STAGE 1: Fix broken tag closures (missing >)
 * STAGE 2: Sanitize illegal chars/entities
 * STAGE 3: Token-based soft repair for orphaned tags
 * STAGE 4: Parse → Rebuild → Validate
 */
function autoRepairXML(xmlString) {
  // STAGE 0 — Detect and fix accidental opening tags that should be closing tags
  // This handles: <name>John<name> → <name>John</name>
  // Critical for recovering when </name> was mistakenly typed as <name>
  function fixAccidentalOpeningTags(xml) {
    return xml.replace(
      /<([a-zA-Z0-9:_-]+)>([\s\S]*?)<\1>/g,
      (match, tag, between) => {
        // If no proper closing tag exists between, the second opening is actually a closing tag
  ...
  // STAGE 1 — Fix broken tag closures BEFORE parsing
  ...
  // STAGE 3 — Soft token repair for orphaned content and unclosed tags
 ...
 *   removeDeclaration, removeComments, removeCDATA, trimWhitespaceBetweenTags,
 *   formatMode
 * }
 *
