...
function fixAccidentalOpeningTags(xml) {
  return xml.replace(
    ...
    (match, tag, attrs, between) => {
      // If there's already a proper closing tag inside, don't touch it
      if (between.includes(`</${tag}>`)) {
        return match;
      }
      // Treat the second <tag> as a mistaken closing tag
 ...
 *
 * removeCDATA uses "smart" behavior (Option C):
 * - If CDATA content parses as valid markup snippet → unwrap as markup
...
function applyCleanupOptions(xmlString, options = {}) {
  ...
  if (removeCDATA) {
    ...
    out = out.replace(/<!\[CDATA\[(.*?)\]\]>/gs, (match, content) => {
      ...
      // If no risky characters, just trim & return as text
      if (!/[<>&]/.test(inner)) {
        return inner.trim();
      }
      ...
      if (result === true) {
        // Safe to interpret as markup; unwrap "as-is"
        // (We don't auto-close orphan tags here; that's done in autoRepairXML stage)
        return inner;
      }
