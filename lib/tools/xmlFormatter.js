...
/**
 * Lint XML with mode-aware behavior.
 *
 * In beautify mode: Run all lints (validation, readability, best practices)
 * In minify mode: Skip readability-based lints (no-root, multiple-roots, missing-declaration)
 *                 since minification intentionally destroys formatting and visual structure
 *
 * @param {string} xmlString - The XML to lint
 * @param {string} formatMode - The formatting mode ('beautify', 'minify', etc.)
 * @returns {Array} Array of warning objects
 */
function lintXML(xmlString, formatMode = 'beautify') {
  const warnings = [];
  const trimmed = xmlString.trim();

  // Skip readability-based lints in minify mode
  // Minification intentionally destroys whitespace and visual structure,
  // so heuristics that rely on these characteristics will produce false positives
...
function processXmlTool(inputXml, options = {}) {
  ...
  const inputDiagnostics = diagnoseXML(inputXml, 'beautify');
  ...
  if (inputIsWellFormed) {
    ...
    outputDiagnostics = diagnoseXML(finalXml, formatMode);
  }
  ...
  if (inputIsWellFormed && showLinting) {
    outputLintWarnings = lintXML(finalXml, formatMode);
  }
