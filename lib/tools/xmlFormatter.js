...
function sanitizeLoose(xml) {
  ...
  return parts
    .map((part, idx) => {
      ...
      if (idx % 2 === 1 && part.startsWith('<![CDATA[')) {
        return part;
      }
      return part
...
/**
 * STAGE 3 — Placeholder for soft token fixes.
 * For now, we leave it as identity to avoid over-aggressive changes.
 */
...
function formatFinalXML(xmlString, mode = 'beautify') {
  if (mode === 'minify') {
    return pd.xmlmin(xmlString, false); // don't auto-strip comments
  }
  if (mode === 'beautify') {
...
/**
 * High-level pipeline used by the tool hub.
 *
