...
function diagnoseXML(xmlString) {
  ...
  const lintDiagnostics = lintWarnings.map((msg) => ({
    type: 'warning',
    category: 'lint',
    message: msg,
    line: null,
...
/**
 * Whitespace handling:
 * - trimWhitespace = false (default): keep whitespace as-is
 * - trimWhitespace = true: trim trailing spaces and collapse leading/trailing blank lines
 */
function applyWhitespaceOptions(xmlString, options = {}) {
  const { trimWhitespace = false } = options;
  if (!trimWhitespace) return xmlString;
...
function processXmlTool(inputXml, options = {}) {
  ...
  // 1. Run diagnostics on the *original* XML
  const diagnostics = diagnoseXML(inputXml);

  // 2. Cleanup / whitespace options
  let cleanedXml = applyCleanupOptions(inputXml, options);
  cleanedXml = applyWhitespaceOptions(cleanedXml, options);

  // 3. Optionally collapse empty elements (only makes sense if well-formed)
  let collapsedXml = cleanedXml;
  if (diagnostics.isWellFormed) {
    collapsedXml = collapseEmptyElements(cleanedXml);
  }

  // 4. Only format (beautify/minify) if XML is well-formed
  let finalXml = inputXml;
  if (diagnostics.isWellFormed) {
    finalXml = formatFinalXML(collapsedXml, formatMode, indentSize);
  } else {
    // Pass through (but still honor cleanup/whitespace if you prefer)
    finalXml = cleanedXml;
  }

  // Backwards-ish compatible validation object
  const originalValidation = diagnostics.strict;
  const repairedValidation = diagnostics.strict; // no "repaired" version anymore

  const validation = {
    original: originalValidation,
    repaired: repairedValidation,
    structuralErrors: diagnostics.structuralErrors,
  };
  ...
  return {
    ...
    collapsedXml,
    finalXml,
    lintWarnings: diagnostics.lintWarnings,
    validation,
