...
function preprocessXML(xmlString) {
  ...
  // Fix the most critical issue: closing tags that are missing the '>'
  // Pattern: </ followed by tag name, then CONTENT (not >), then either < or end of string
  // e.g., <name>John</name becomes <name>John</name>
  // or   </name<age> becomes </name><age>

  // Step 1: Fix closing tags missing their closing >
  // Look for </tagname but no closing >
  fixed = fixed.replace(/<\/([a-zA-Z_][\w\-:]*)\s*([^>]*)(<|$)/g, (match, tagName, afterTag, nextChar) => {
    // If there's content after the tag name before < or end, we need to fix it
    const trimmed = afterTag.trim()
    if (trimmed && trimmed !== '' && nextChar === '<') {
      // There's unexpected content, close the tag and continue
      return `</${tagName}>${nextChar}`
    } else if (!match.includes('>')) {
      // Missing the closing > on the tag itself
      return `</${tagName}>${nextChar}`
    }
    return match
  })

  // Step 2: Fix opening tags missing their closing >
  // Pattern: <tagname but no closing >
  // This is trickier - look for < followed by word chars/spaces but no >
  // before the next < or end of line
  let inTag = false
  let tagStart = -1
  let result = ''
  ...
  for (let i = 0; i < fixed.length; i++) {
    ...
    if (char === '<' && (nextChar === '/' || /[a-zA-Z_]/.test(nextChar))) {
      ...
      inTag = false
      result += char
    } else if (inTag && char === '<' && nextChar !== '!') {
      // Found a new tag start while still in a tag - means previous was incomplete
      if (!fixed.substring(tagStart, i).includes('>')) {
        // Insert missing >
        result += '>'
      }
      inTag = true
  ...
  if (inTag && !result.endsWith('>')) {
    result += '>'
  }

  fixed = result

  // Step 3: Fix orphaned closing tags (e.g., </age> appearing without matching opening)
  // and tag name mismatches
  // Pattern: <tagname>content</wrongtagname> -> <tagname>content</tagname>
  const tagStack = []
  const tagRegex = /<(\/?)([\w\-:]+)([^>]*)>/g
  let match
  ...
  while ((match = tagRegex.exec(fixed)) !== null) {
    ...
    if (!isClosing) {
      ...
      if (lastOpen !== tagName) {
        // Mismatch - replace closing tag with correct one
        fixed = fixed.replace(`</${tagName}>`, `</${lastOpen}>`)
