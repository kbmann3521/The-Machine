...
/* ============================
 *  PRE-BEAUTIFY NORMALIZATION
 * ============================ */

/**
 * Prevent CDATA from causing xml-formatter to expand following tags.
 */
function normalizePostCdataWhitespace(xml) {
  return xml.replace(/\]\]>\s*\n\s*</g, ']]>\n<');
}

/**
 * Collapse simple one-line text nodes before formatting.
 */
function collapseSimpleTextNodes(xml) {
  return xml.replace(
    /<([a-zA-Z0-9:_-]+)>\s*([^<>\n]+)\s*<\/\1>/g,
    '<$1>$2</$1>'
  );
...
function formatFinalXML(xmlString, mode = 'beautify', indentSize = '2') {
  ...
  if (mode === 'beautify') {
    ...
    // 1) Normalize CDATA whitespace
    let pre = normalizePostCdataWhitespace(xmlString);

    // 2) Collapse simple text-only tags
    pre = collapseSimpleTextNodes(pre);

    // 3) Beautify
    return xmlFormatter(pre, {
      indentation,
