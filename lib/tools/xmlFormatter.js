 ...
 * ============================ */
 ...
 // NEW — preserves all whitespace exactly as input
function beautifyXmlPreserveWhitespace(xmlString, indentUnit = '  ') {
  // Do NOT fix spacing, do NOT collapse anything.
  // Only ensure newlines are normalized as-is.
  return xmlString
    .split('\n')
    .map(line => line)   // pass-through
    .join('\n');
}
...
function formatFinalXML(xmlString, mode = 'beautify', indentSize = '2') {
  ...
  if (mode === 'beautify') {
    if (!options.trimWhitespaceBetweenTags) {
      // ✨ NEW: preserve whitespace (no collapsing)
...
function processXmlTool(inputXml, options = {}) {
  ...
  const shouldTrimBetweenTags = !!options.trimWhitespaceBetweenTags;
