...
function autoRepairXML(xmlString) {
  // First attempt: Use preserveOrder to maintain structure where possible
  const parserWithOrder = new XMLParser({
  ...
  // Second attempt: Use normalized parsing (without preserveOrder) for better repair
  const parserNormalized = new XMLParser({
    ignoreAttributes: false,
    preserveOrder: false,
    allowBooleanAttributes: true,
    commentPropName: '#comment',
  ...
  try {
    // Try normalized parsing first for better structure recovery
    parsed = parserNormalized.parse(xmlString)
    useNormalized = true
  } catch (err) {
    // Fall back to preserveOrder parsing
    try {
      parsed = parserWithOrder.parse(xmlString)
      useNormalized = false
    } catch (finalErr) {
      return {
        ok: false,
  ...
  const builder = new XMLBuilder({
    ignoreAttributes: false,
    preserveOrder: useNormalized ? false : true,
