...
function detectHeaderNormalizationIssues(text, headers, strictMode = false) {
  ...
  lines.forEach((line, idx) => {
    ...
    // 7. Trailing whitespace in value
    if (fieldValue !== fieldValue.trim() && fieldValue.trim()) {
      issues.push({ level: 'info', message: `Header "${fieldName}" has leading/trailing whitespace (unusual)` })
    }
...
function detectValidatorConflicts(headers, cacheControl) {
  ...
  // 2. Weak ETag with public caching (semantic issue)
  if (hasETag && headers.ETag.startsWith('W/') && isPublic) {
