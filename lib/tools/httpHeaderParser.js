...
function detectHeaderNormalizationIssues(text, headers, strictMode = false) {
  ...
  lines.forEach((line, idx) => {
    ...
    const rawHasLeadingSpace = line.match(/^\s+[^\s]/) // Line starts with spaces before first char
    const rawHasTrailingSpace = line.match(/[^\s]\s+$/) && !line.match(/[^\s]\s*$/) // Line ends with spaces after last char
