...
function calculateOverallStatus(headerAnalysis, analysis) {
  ...
  // Count security-related issues only (from header analysis and security checks)
  let securityErrors = 0
  let securityWarnings = 0

  Object.values(headerAnalysis).forEach(headerAnal => {
    if (headerAnal && headerAnal.issues) {
      securityErrors += headerAnal.issues.filter(i => i.level === 'error').length
      securityWarnings += headerAnal.issues.filter(i => i.level === 'warning').length
    }
  })

  if (analysis.security.conflicts.length > 0) {
    securityErrors += analysis.security.conflicts.length
  }

  if (analysis.security.warnings.length > 0) {
    securityWarnings += analysis.security.warnings.length
  }

  // Count protocol/RFC compliance issues separately
  ...
  if (analysis.rfcCompliance) {
    ...
    protocolErrors += rfcIssues.filter(i => i.level === 'error').length
    protocolWarnings += rfcIssues.filter(i => i.level === 'warning').length
    protocolInfos += rfcIssues.filter(i => i.level === 'info').length
  }

  if (analysis.http2Incompatibilities) {
    protocolErrors += analysis.http2Incompatibilities.filter(i => i.level === 'error').length
    protocolWarnings += analysis.http2Incompatibilities.filter(i => i.level === 'warning').length
  }

  if (analysis.cacheHeuristics?.issues) {
    protocolWarnings += analysis.cacheHeuristics.issues.filter(i => i.level === 'warning').length
    protocolInfos += analysis.cacheHeuristics.issues.filter(i => i.level === 'info').length
  }

  // Total counts
  totalErrors = securityErrors + protocolErrors
  totalWarnings = securityWarnings + protocolWarnings
  totalInfos = protocolInfos

  // Determine security risk level (independent of protocol issues)
  let securityRiskLevel = 'low'
  if (analysis.security.missingRecommendedHeaders.length > 3 || securityErrors > 0) {
    securityRiskLevel = securityErrors > 0 ? 'high' : 'medium'
  }

  // Determine protocol risk level (independent of security)
  let protocolRiskLevel = 'low'
  if (protocolErrors > 0 || protocolWarnings > 0) {
    protocolRiskLevel = protocolErrors > 0 ? 'high' : 'medium'
  }
