...
function detectHeaderNormalizationIssues(text, headers, strictMode = false) {
  ...
  lines.forEach((line, idx) => {
    ...
    // 2. Illegal characters in field-name
    ...
    // 3. Multiple colons in field name (invalid)
    if (fieldName.includes(':')) {
      issues.push({ level: 'error', message: `Header name "${fieldName}" contains colon (invalid)` })
    }

    // 4. Control characters or unescaped CR/LF in value
...
function calculateCacheHeuristics(headers, statusLine) {
  ...
  if (cc) {
    cc.split(',').forEach(dir => {
      ...
      directives[key.trim()] = val ? parseInt(val.trim()) : true
    })
  }

  const now = Math.floor(Date.now() / 1000)

  // 1. Browser vs CDN TTL logic
  if (directives['max-age']) {
  ...
  if (directives['s-maxage']) {
    ...
    // If s-maxage = 0, CDN must revalidate
  ...
  // 2. Heuristic freshness (when max-age is missing)
  if (!directives['max-age'] && headers['Last-Modified']) {
    const lastModified = new Date(headers['Last-Modified']).getTime() / 1000
    if (!isNaN(lastModified)) {
      const age = now - lastModified
      // Browsers compute: (now - lastModified) * 0.1 as implied freshness
      const heuristicTTL = Math.floor(age * 0.1)
      heuristics.heuristicFreshness = heuristicTTL
      heuristics.issues.push({ level: 'info', message: `Heuristic freshness calculated: ${heuristicTTL}s based on Last-Modified` })
    }
  }

  // 3. stale-while-revalidate timeline
  if (directives['stale-while-revalidate']) {
    const swrSeconds = directives['stale-while-revalidate']
    const ttl = heuristics.browserFreshnessTTL || 0
    heuristics.staleWhileRevalidateUntil = now + ttl + swrSeconds
  }

  // 4. stale-if-error timeline
...
function detectCompressionValidity(headers) {
  ...
  const alreadyCompressed = ['image/jpeg', 'image/png', 'image/webp', 'image/gif', 'audio/mp3', 'audio/mpeg', 'video/mp4', 'application/pdf', 'application/zip']
  if (contentEncoding === 'gzip' && alreadyCompressed.some(fmt => contentType.includes(fmt))) {
    issues.push({ level: 'warning', message: 'gzip applied to already-compressed format (ineffective and may increase size)' })
  }

  // 2. Brotli limitations
...
function detectValidatorConflicts(headers, cacheControl) {
  ...
  // 2. ETag strong/weak mismatch with immutable
...
export function httpHeaderParser(text, config = {}) {
  ...
  // 4. Detect HTTP/2 incompatibilities (more comprehensive)
  const http2IncompatibilityIssues = detectHTTP2Issues(headers, statusLine, strictMode)
  ...
  const analysis = {
    ...
    http2Incompatibilities: http2IncompatibilityIssues,
    compressionRecommendations,
    hopByHopHeaders,
    responseAnomalies: missingContentLengthIssues,
    // NEW RFC-LEVEL DETECTIONS
    rfcCompliance: {
      headerConflicts: headerConflictIssues,
      headerNormalization: headerNormalizationIssues,
      validatorConflicts: validatorConflictIssues,
      compressionValidity: compressionValidityIssues,
    },
    cacheHeuristics,
