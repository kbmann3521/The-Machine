...
function calculateCacheHeuristics(headers, statusLine) {
  ...
  if (!directives['max-age'] && !directives.expires && headers['Last-Modified']) {
...
export function httpHeaderParser(text, config = {}) {
  ...
  // Collect all issues and rewrite severity levels based on strictMode
  const allIssuesArray = [
    ...headerConflictIssues,
    ...headerNormalizationIssues,
    ...validatorConflictIssues,
    ...compressionValidityIssues,
    ...http2IncompatibilityIssues,
    ...(cacheHeuristics.issues || []),
    ...foldingIssues,
    ...missingContentLengthIssues,
  ]

  const rewrittenIssues = applySeverityRewriting(allIssuesArray, strictMode)

  // Separate rewritten issues back into their categories
  const headerConflictsRewritten = rewrittenIssues.filter(i => headerConflictIssues.includes(i))
  const headerNormRewritten = rewrittenIssues.filter(i => headerNormalizationIssues.includes(i))
  const validatorConflictsRewritten = rewrittenIssues.filter(i => validatorConflictIssues.includes(i))
  const compressionValidityRewritten = rewrittenIssues.filter(i => compressionValidityIssues.includes(i))
  const http2RewrittenIssues = rewrittenIssues.filter(i => http2IncompatibilityIssues.includes(i))
  const cacheHeuristicsRewritten = { ...cacheHeuristics, issues: rewrittenIssues.filter(i => cacheHeuristics.issues?.includes(i) || []) }
