...
const HTTP_STATUS_DATA = [
  // ==== 2xx ====
  {
    ...
    examples: {
      ...
      flask: `return {"ok": True, "data": data}, 200`,
      go: `w.WriteHeader(http.StatusOK)\njson.NewEncoder(w).Encode(map[string]interface{}{"ok": true})`,
      php: `http_response_code(200);\necho json_encode(["ok" => true]);`,
      ruby: `render json: { ok: true }, status: :ok`,
      cloudflare: `return new Response(JSON.stringify({ ok: true }), { status: 200 });`,
    },
  },
  {
    code: 201,
    ...
    examples: {
      ...
      flask: `return {"id": new_id}, 201, {"Location": f"/items/{new_id}"}`,
      go: `w.Header().Set("Location", fmt.Sprintf("/items/%d", id))\nw.WriteHeader(http.StatusCreated)`,
      php: `http_response_code(201);\nheader("Location: /items/" . $id);\necho json_encode(["id" => $id]);`,
      ruby: `render json: { id: id }, status: :created, location: item_url(id)`,
      cloudflare: `return new Response(JSON.stringify({ id }), { status: 201, headers: { Location: \`/items/\${id}\` } });`,
    },
  },
  {
    code: 204,
    ...
    examples: {
      ...
      ruby: `head :no_content`,
      cloudflare: `return new Response(null, { status: 204 });`,
    },
  },
  ...
  // ==== 3xx ====
  {
    ...
    examples: {
      ...
      flask: `return redirect("/new-path", code=301)`,
      go: `http.Redirect(w, r, "/new-path", http.StatusMovedPermanently)`,
      php: `header("HTTP/1.1 301 Moved Permanently");
header("Location: /new-path");`,
      ruby: `redirect "/new-path", status: :moved_permanently`,
      nginx: `return 301 https://example.com$new_uri;`,
      cloudflare: `return Response.redirect("/new-path", 301);`,
    },
  },
  {
    code: 302,
    ...
    examples: {
      ...
      flask: `return redirect("/login", code=302)`,
      go: `http.Redirect(w, r, "/login", http.StatusFound)`,
      php: `header("Location: /login", true, 302);`,
      ruby: `redirect "/login", status: :found`,
      cloudflare: `return Response.redirect("/login", 302);`,
    },
  },
  {
    code: 307,
    ...
    examples: {
      ...
      flask: `return redirect("/maintenance", code=307)`,
      go: `http.Redirect(w, r, "/maintenance", http.StatusTemporaryRedirect)`,
      php: `header("Location: /maintenance", true, 307);`,
      ruby: `redirect "/maintenance", status: :temporary_redirect`,
      cloudflare: `return Response.redirect("/maintenance", 307);`,
    },
  },
  {
    code: 308,
    ...
    examples: {
      ...
      flask: `return redirect("/v2/api", code=308)`,
      go: `http.Redirect(w, r, "/v2/api", http.StatusPermanentRedirect)`,
      php: `header("Location: /v2/api", true, 308);`,
      ruby: `redirect "/v2/api", status: :permanent_redirect`,
      cloudflare: `return Response.redirect("/v2/api", 308);`,
    },
  },
  ...
  // ==== 4xx ====
  {
    ...
    examples: {
      ...
      flask: `return {"error": "Invalid payload"}, 400`,
      go: `w.WriteHeader(http.StatusBadRequest)
json.NewEncoder(w).Encode(map[string]string{"error": "Invalid payload"})`,
      php: `http_response_code(400);
echo json_encode(["error" => "Invalid payload"]);`,
      ruby: `render json: { error: "Invalid payload" }, status: :bad_request`,
      cloudflare: `return new Response(JSON.stringify({ error: "Invalid payload" }), { status: 400 });`,
    },
  },
  {
    code: 401,
    ...
    examples: {
      ...
      flask: `return {"error": "Unauthorized"}, 401`,
      go: `w.Header().Set("WWW-Authenticate", "Bearer")
w.WriteHeader(http.StatusUnauthorized)`,
      php: `http_response_code(401);
header('WWW-Authenticate: Bearer');
echo json_encode(["error" => "Unauthorized"]);`,
      ruby: `render json: { error: "Unauthorized" }, status: :unauthorized`,
      cloudflare: `return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401 });`,
    },
  },
  {
    code: 403,
    ...
    examples: {
      ...
      flask: `return {"error": "Forbidden"}, 403`,
      go: `w.WriteHeader(http.StatusForbidden)
json.NewEncoder(w).Encode(map[string]string{"error": "Forbidden"})`,
      php: `http_response_code(403);
echo json_encode(["error" => "Forbidden"]);`,
      ruby: `render json: { error: "Forbidden" }, status: :forbidden`,
      cloudflare: `return new Response(JSON.stringify({ error: "Forbidden" }), { status: 403 });`,
    },
  },
  {
    code: 404,
    ...
    examples: {
      ...
      flask: `return {"error": "Not found"}, 404`,
      go: `http.NotFound(w, r)`,
      php: `http_response_code(404);
echo json_encode(["error" => "Not found"]);`,
      ruby: `render json: { error: "Not found" }, status: :not_found`,
      cloudflare: `return new Response(JSON.stringify({ error: "Not found" }), { status: 404 });`,
    },
  },
  {
    code: 405,
    ...
    examples: {
      ...
      flask: `from flask import make_response
resp = make_response("", 405)
resp.headers['Allow'] = 'GET'
return resp`,
      go: `w.Header().Set("Allow", "GET")
w.WriteHeader(http.StatusMethodNotAllowed)`,
      php: `http_response_code(405);
header('Allow: GET');`,
      ruby: `head :method_not_allowed, headers: { Allow: "GET" }`,
      cloudflare: `return new Response(null, { status: 405, headers: { Allow: "GET" } });`,
    },
  },
  {
    code: 409,
  ...
  },
  {
    ...
    examples: {
      node: `app.use(express.json({ limit: "1mb" }));`,
      flask: `app.config['MAX_CONTENT_LENGTH'] = 1 * 1024 * 1024`,
      go: `http.MaxBytesReader(w, r.Body, 1024*1024)`,
      php: `// php.ini: upload_max_filesize = 1M`,
      ruby: `config.middleware.use Rack::ContentLength`,
      nginx: `client_max_body_size 10m;`,
      cloudflare: `// Cloudflare: ~128MB limit`,
    },
  },
  {
    code: 415,
...
if request.content_type != 'application/json':
    return {"error": "Unsupported"}, 415`,
      go: `if r.Header.Get("Content-Type") != "application/json" {
    w.WriteHeader(http.StatusUnsupportedMediaType)
}`,
      php: `if ($_SERVER['CONTENT_TYPE'] !== 'application/json') {
    http_response_code(415);
}`,
      ruby: `before_action do
  head :unsupported_media_type unless request.json?
end`,
      cloudflare: `if (request.headers.get('content-type') !== 'application/json') {
  ...
  },
  {
    ...
    examples: {
      ...
      flask: `return {"error": "I'm a teapot"}, 418`,
      go: `w.WriteHeader(http.StatusTeapot)
json.NewEncoder(w).Encode(map[string]string{"error": "I'm a teapot"})`,
      php: `http_response_code(418);
echo json_encode(["error" => "I'm a teapot"]);`,
      ruby: `render json: { error: "I'm a teapot" }, status: :teapot`,
      cloudflare: `return new Response(JSON.stringify({ error: "I'm a teapot" }), { status: 418 });`,
    },
  },
  {
    code: 422,
    ...
    examples: {
      ...
      flask: `return {"error": "Validation failed", "details": errors}, 422`,
      go: `w.WriteHeader(http.StatusUnprocessableEntity)
json.NewEncoder(w).Encode(map[string]interface{}{"error": "Validation failed"})`,
      php: `http_response_code(422);
echo json_encode(["error" => "Validation failed"]);`,
      ruby: `render json: { error: "Validation failed", details: errors }, status: :unprocessable_entity`,
      cloudflare: `return new Response(JSON.stringify({ error: "Validation failed" }), { status: 422 });`,
    },
  },
  {
    code: 429,
    ...
    examples: {
      ...
      flask: `from flask import make_response
resp = make_response({"error": "Too many requests"}, 429)
...
echo json_encode(["error" => "Too many requests"]);`,
      ruby: `render json: { error: "Too many requests" }, status: :too_many_requests, headers: { "Retry-After" => "60" }`,
      cloudflare: `return new Response(JSON.stringify({ error: "Too many requests" }), { status: 429, headers: { "Retry-After": "60" } });`,
    },
  },
  ...
  },
  {
    ...
    examples: {
      node: `// Usually automatic from reverse proxy on bad upstream`,
      flask: `# Usually automatic from reverse proxy`,
      go: `// Usually automatic from reverse proxy`,
      php: `// Usually automatic from reverse proxy`,
      ruby: `# Usually automatic from reverse proxy`,
      nginx: `# Usually raised automatically by Nginx/LB`,
      cloudflare: `// Usually automatic from bad origin`,
    },
  },
  {
    code: 503,
    ...
    examples: {
      ...
      flask: `from flask import make_response
resp = make_response({"error": "Service unavailable"}, 503)
resp.headers['Retry-After'] = '120'
return resp`,
      go: `w.Header().Set("Retry-After", "120")
w.WriteHeader(http.StatusServiceUnavailable)`,
      php: `http_response_code(503);
header('Retry-After: 120');
echo json_encode(["error" => "Service unavailable"]);`,
      ruby: `render json: { error: "Service unavailable" }, status: :service_unavailable, headers: { "Retry-After" => "120" }`,
      cloudflare: `return new Response(JSON.stringify({ error: "Service unavailable" }), { status: 503, headers: { "Retry-After": "120" } });`,
    },
  },
  {
    code: 504,
    ...
    examples: {
      node: `// Increase timeout or check upstream service`,
      flask: `PROPAGATE_EXCEPTIONS = True  # Log errors`,
      go: `client := &http.Client{Timeout: 30 * time.Second}`,
      php: `set_time_limit(60);  // Increase timeout`,
      ruby: `config.read_timeout = 60`,
      nginx: `proxy_read_timeout 60s;`,
      cloudflare: `// Increase request timeout in code`,
    },
  },
];
...
function searchByText(query, limit = 3) {
  ...
  const scored = HTTP_STATUS_DATA.map((entry) => {
    ...
    if (q.includes("timeout") || q.includes("timed out") || q.includes("slow")) {
      if (entry.code === 504) score += 5;
      if (entry.code === 408) score += 3;
    }
    if (q.includes("gateway") || q.includes("bad gateway") || q.includes("upstream")) {
      if (entry.code === 502) score += 5;
      if (entry.code === 504) score += 2;
    }
    if (q.includes("unavailable") || q.includes("down") || q.includes("maintenance") || q.includes("overload")) {
      if (entry.code === 503) score += 5;
    }
    if (q.includes("internal error") || q.includes("server error") || q.includes("crash") || q.includes("exception")) {
      if (entry.code === 500) score += 4;
    }
    if (q.includes("conflict") || q.includes("already exists") || q.includes("duplicate")) {
      if (entry.code === 409) score += 4;
    }
    if (q.includes("forbidden") || q.includes("permission") || q.includes("access denied")) {
      if (entry.code === 403) score += 4;
    }
    if (q.includes("bad request") || q.includes("invalid") || q.includes("malformed")) {
      if (entry.code === 400) score += 3;
    }
