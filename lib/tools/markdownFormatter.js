...
/* ============================
 *  HELPER: Mask style tags for Markdown linting
 * ============================ */

/**
 * Temporarily replace <style> tag contents with placeholder text
 * This prevents CSS syntax (like *{) from being interpreted as Markdown syntax
 */
function maskStyleTagsForLinting(text) {
  const styleTagRegex = /<style[^>]*>([\s\S]*?)<\/style>/gi
  let maskedText = text
  let styleIndex = 0

  maskedText = maskedText.replace(styleTagRegex, (match) => {
    // Replace with a placeholder that won't trigger markdown linting
    return `<!-- STYLE_PLACEHOLDER_${styleIndex++} -->`
  })
...
async function markdownHtmlFormatter(text, config) {
  ...
  try {
    ...
    if (showLinting) {
      ...
      // But mask <style> tags first to prevent CSS syntax from triggering Markdown warnings
      const textForLinting = maskStyleTagsForLinting(trimmed);

      if (containsMarkdownSyntax) {
        lintResult = lintMarkdown(textForLinting);
      } else if (isPureHtml || (result.includes('<') && result.includes('>'))) {
        lintResult = lintHtml(textForLinting);
      }
