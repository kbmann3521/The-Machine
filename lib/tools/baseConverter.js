...
function detectInputBase(input) {
  ...
  if (/[a-f]/.test(digitsOnly)) {
    return { base: 16, stripped: sign + toAnalyze, detected: true, reason: 'contains A-F' }
  }

  // Check if only binary digits (0-1)
  if (/^[01\s_]+$/.test(digitsOnly) && digitsOnly.length > 1) {
    return { base: 2, stripped: sign + toAnalyze, detected: true, reason: 'binary pattern' }
  }
  ...
  if (/^[0-7\s_]+$/.test(digitsOnly)) {
    ...
    if (digitsOnly.startsWith('0') && digitsOnly.length > 1) {
      return { base: 8, stripped: sign + toAnalyze, detected: true, reason: 'octal pattern', confidence: 'medium' }
    }
  }

  // Default to decimal
  return { base: 10, stripped: sign + toAnalyze, detected: true, reason: 'default' }
}
