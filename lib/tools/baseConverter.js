...
function detectInputBase(input) {
  ...
  if (trimmed.startsWith('0b')) {
    return { base: 2, stripped: trimmed.slice(2), detected: true, prefix: '0b' }
  }
  if (trimmed.startsWith('0o')) {
    return { base: 8, stripped: trimmed.slice(2), detected: true, prefix: '0o' }
  }
  if (trimmed.startsWith('0x')) {
    return { base: 16, stripped: trimmed.slice(2), detected: true, prefix: '0x' }
  }
  
  // Remove optional sign for pattern detection
  let toAnalyze = trimmed
  let isNegative = false
  if (trimmed.startsWith('-') || trimmed.startsWith('+')) {
    isNegative = trimmed.startsWith('-')
    toAnalyze = trimmed.substring(1)
  }
  
  // Remove decimal point for digit analysis
  const integerPart = toAnalyze.split('.')[0]
  const digitsOnly = integerPart.replace(/\s|_/g, '')
  
  // Check for hex digits (A-F)
  if (/[A-Fa-f]/.test(digitsOnly)) {
    return { base: 16, stripped: trimmed, detected: true, reason: 'contains A-F' }
  }
  
  // Check if only binary digits (0-1)
  if (/^[01\s_]+$/.test(digitsOnly) && digitsOnly.length > 1) {
    return { base: 2, stripped: trimmed, detected: true, reason: 'binary pattern' }
  }
  ...
  if (/^[0-7\s_]+$/.test(digitsOnly)) {
    ...
    if (digitsOnly.startsWith('0') && digitsOnly.length > 1) {
      return { base: 8, stripped: trimmed, detected: true, reason: 'octal pattern', confidence: 'medium' }
    }
  }
  
  // Default to decimal
  return { base: 10, stripped: trimmed, detected: true, reason: 'default' }
}
