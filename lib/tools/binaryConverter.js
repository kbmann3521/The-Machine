...
function binaryToText(binaryString, encoding = 'utf-8') {
  ...
  try {
    ...
    if (encoding === 'utf-16') {
      ...
      // ASCII and UTF-8 both work with String.fromCharCode for standard characters
      result = String.fromCharCode(...uint8Array)
    }
    ...
    // Check for non-printable characters and create a sanitized version
    const hasNonPrintable = /[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F-\x9F]/.test(result)

    // Create escaped representation for non-printable characters
    let escaped = ''
    let nonPrintableCount = 0
    for (let i = 0; i < result.length; i++) {
      const char = result[i]
      const code = char.charCodeAt(0)
      // Check if character is non-printable (control characters, etc.)
      if (/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F-\x9F]/.test(char)) {
        nonPrintableCount++
  ...
  try {
    ...
    if (mode === 'textToBinary') {
      ...
      return {
        ...
        escaped: conversionResult.escaped,
        hasNonPrintable: conversionResult.hasNonPrintable,
        nonPrintableCount: conversionResult.nonPrintableCount,
        input: input,
        charEncoding: encoding,
        formats: binaryFormats,
        metadata: {
