...
function verifyHS256Signature(rawHeader, rawPayload, signatureB64Url, secret) {
  try {
    if (!secret) {
      return {
        verified: null,
    ...
    if (typeof window !== 'undefined') {
      console.log('[JWT Debug] Server-side HS256 Verification:', {
...
// Phase 4: RS256 Signature Verification (Server-side, Node.js)
function verifyRS256Signature(rawHeader, rawPayload, signatureB64Url, publicKeyPem) {
  try {
    if (!publicKeyPem) {
      return {
        verified: null,
...
export function jwtDecoder(text, options = {}) {
  ...
  return {
    ...
    signatureVerification: (() => {
      // Phase 3-4: Signature Verification (HS256, RS256, none)
      const alg = headerParsed.value.alg

      if (alg === 'HS256') {
        const verificationResult = verifyHS256Signature(
          parts[0],
          parts[1],
          parts[2],
        ...
        const verificationResult = verifyRS256Signature(
          parts[0],
          parts[1],
          parts[2],
          options.publicKey
        ...
        return {
          algorithm: alg,
          verified: null,
          reason: `${alg} verification is not yet supported. Supported algorithms: HS256, RS256.`,
        }
      }
    })(),
