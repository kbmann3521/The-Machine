...
// Phase 6: EC (ECDSA) Signature Verification (Works in both Node.js and Browser)
// Supports ES256, ES384, ES512
// Uses Node.js crypto when available, falls back to Web Crypto API in browsers
function verifyECSignature(algorithm, rawHeader, rawPayload, signatureB64Url, publicKeyPem) {
  try {
    ...
    // Map EC algorithm to hash algorithm and curve name
    const algMap = {
      ES256: { hash: 'sha256', hashWeb: 'SHA-256', namedCurve: 'P-256' },
      ES384: { hash: 'sha384', hashWeb: 'SHA-384', namedCurve: 'P-384' },
      ES512: { hash: 'sha512', hashWeb: 'SHA-512', namedCurve: 'P-521' },
    }

    const algConfig = algMap[algorithm]
    if (!algConfig) {
      return {
        verified: false,
        reason: `Unsupported algorithm: ${algorithm}`,
    ...
    // Check if running in Node.js environment (crypto.createVerify available)
    const isNodeEnv = typeof crypto !== 'undefined' && typeof crypto.createVerify === 'function'

    if (isNodeEnv) {
      // Node.js path
      const verifier = crypto.createVerify(algConfig.hash)
      verifier.update(`${rawHeader}.${rawPayload}`)
      verifier.end()
      ...
      return {
        verified,
        reason: verified
          ? `${algorithm} (${algConfig.hashWeb} with ${algConfig.namedCurve} curve) signature matches token contents`
          : 'Signature does not match. The public key does not match the private key used to sign.',
      }
    } else {
      // Browser path - return message that browser verification is needed
      // (the browser component JWTDecoderOutput handles this with Web Crypto)
      return {
        verified: null,
        reason: `${algorithm} signature verification requires browser environment. Please use the UI to verify.`,
      }
    }
  } catch (error) {
    return {
