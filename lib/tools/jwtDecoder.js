...
export function jwtDecoder(text) {
  ...
  for (const part of parts) {
    if (!part) {
      return {
        error: 'Invalid JWT: one or more parts are empty',
        decoded: false,
      }
    }
    if (!/^[A-Za-z0-9_-]*$/.test(part)) {
      return {
        error: 'Invalid JWT: contains characters that are not base64url-safe (allowed: a-z, A-Z, 0-9, -, _)',
        decoded: false,
      }
    }
  }
  ...
  if (!headerDecoded.success) {
    return {
      error: `Failed to decode header: ${headerDecoded.error}`,
      decoded: false,
    }
  }
  ...
  if (!payloadDecoded.success) {
    return {
      error: `Failed to decode payload: ${payloadDecoded.error}`,
      decoded: false,
    }
  }

  // Phase 1.3: JSON Pretty-Print + JSON Validation
