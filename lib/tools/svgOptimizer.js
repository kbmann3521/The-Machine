...
function svgOptimizer(text, config = {}) {
  ...
  let normalizedSvg = svgWithoutCDATA
    .replace(/\s+/g, ' ')
    .replace(/>\s+</g, '><')
    .replace(/<!--.*?-->/g, '')
    .trim()

  // Restore CDATA blocks with original formatting
  normalizedSvg = restoreCDATA(normalizedSvg, cdataBlocks)
  ...
  if (outputFormat === 'pretty') {
    const svgWithCDATA = restoreCDATA(optimizedSvg, cdataBlocks)
    outputSvg = formatSVGPretty(svgWithCDATA)
  } else {
