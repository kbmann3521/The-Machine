...
const ATTRIBUTE_ORDER = {
  ...
  transform: 20,
  fill: 30, stroke: 30, 'stroke-width': 30, 'stroke-linecap': 30,
  opacity: 30, 'fill-opacity': 30, 'stroke-opacity': 30,
  'fill-rule': 30,
  filter: 40, mask: 40, 'clip-path': 40,
  'text-anchor': 50, 'font-size': 50, 'font-family': 50,
  role: 60, 'aria-label': 60, 'aria-labelledby': 60, 'aria-describedby': 60,
  'xmlns': 100, 'viewBox': 100, 'version': 100
}
...
function sortAttributes(attrString) {
  ...
  const sortedNames = Object.keys(attrs).sort((a, b) => {
    const orderA = ATTRIBUTE_ORDER[a] !== undefined ? ATTRIBUTE_ORDER[a] : 999
    const orderB = ATTRIBUTE_ORDER[b] !== undefined ? ATTRIBUTE_ORDER[b] : 999
    if (orderA !== orderB) return orderA - orderB
    return a.localeCompare(b)
  })

  return sortedNames.map(name => `${name}="${attrs[name]}"`).join(' ')
}

function parseElement(tagString) {
  const match = tagString.match(/^<(\/?)?(\w+)(.*?)(\/?)>$/)
  if (!match) return null

  const isClosing = !!match[1]
  const tagName = match[2]
  const attrPart = (match[3] || '').trim()
  const isSelfClosing = !!match[4]

  return { isClosing, tagName, attrPart, isSelfClosing }
}

function countAttributes(attrString) {
  return (attrString.match(/\w[\w-]*\s*=/g) || []).length
}

function shouldSelfClose(parsed, hasChildren) {
  if (parsed.isClosing) return false
  if (hasChildren) return false

  const attributeCount = countAttributes(parsed.attrPart)
  const structuralElements = ['svg', 'g', 'defs', 'mask', 'filter', 'clipPath', 'pattern', 'symbol']

  if (structuralElements.includes(parsed.tagName)) return false
  if (attributeCount > 3) return false

  return true
}

function formatSVGPretty(svg) {
  return formatSVGSemanticPretty(svg)
}

function formatSVGSemanticPretty(svg) {
  try {
    const normalizedSvg = svg.replace(/>\s+</g, '><').replace(/\s+/g, ' ').trim()
    ...
    while (currentIndex < normalizedSvg.length) {
      const tagStart = normalizedSvg.indexOf('<', currentIndex)

      if (tagStart === -1) {
        const textNode = normalizedSvg.substring(currentIndex).trim()
        if (textNode) {
          result += textNode + '\n'
          lastWasText = true
        }
      ...
      const textBefore = normalizedSvg.substring(currentIndex, tagStart).trim()
      if (textBefore && !lastWasClosingTag) {
        result += textBefore + '\n'
        lastWasText = true
      }

      const tagEnd = normalizedSvg.indexOf('>', tagStart)
      if (tagEnd === -1) break

      const tagString = normalizedSvg.substring(tagStart, tagEnd + 1)
      const parsed = parseElement(tagString)
      ...
      if (parsed.isClosing) {
        ...
        result += indentStr.repeat(depth) + tagString + '\n'
        lastWasClosingTag = true
        lastWasText = false

        if (parsed.tagName === 'defs') {
          result += '\n'
        }
      } else {
        const nextTagStart = normalizedSvg.indexOf('<', tagEnd + 1)
        const textAfter = nextTagStart > -1 ? normalizedSvg.substring(tagEnd + 1, nextTagStart).trim() : normalizedSvg.substring(tagEnd + 1).trim()
        const hasChildren = nextTagStart > -1 && !normalizedSvg.substring(nextTagStart, nextTagStart + 2).startsWith('</')

        if (parsed.isSelfClosing || (!hasChildren && textAfter === '')) {
          if (parsed.attrPart.trim()) {
            const sortedAttrs = sortAttributes(parsed.attrPart)
            const attributeCount = countAttributes(parsed.attrPart)

            if (attributeCount <= 3 && sortedAttrs.length < 60) {
              result += indentStr.repeat(depth) + `<${parsed.tagName} ${sortedAttrs}/>` + '\n'
            } else {
              result += indentStr.repeat(depth) + `<${parsed.tagName}\n`
              const attrNames = Object.keys(sortAttributes(parsed.attrPart).split(/\s+/).reduce((acc, attr) => {
                const kv = attr.split('=')
                acc[kv[0]] = true
                return acc
              }, {}))

              sortedNames.forEach(attr => {
                const match = parsed.attrPart.match(new RegExp(`${attr}="([^"]*)"`, 'i'))
                if (match) {
                  result += indentStr.repeat(depth + 1) + `${attr}="${match[1]}"\n`
                }
              })
          ...
          if (parsed.attrPart.trim()) {
            ...
            if (attributeCount <= 3 && sortedAttrs.length < 60) {
              ...
              const attrs = []
              let attrMatch
              while ((attrMatch = attrPattern.exec(parsed.attrPart)) !== null) {
              ...
              sortedAttrs.forEach((attr, idx) => {
                const isLast = idx === sortedAttrs.length - 1
                result += indentStr.repeat(depth + 1) + `${attr.name}="${attr.value}"` + (isLast ? '\n' : '\n')
              })
              result += indentStr.repeat(depth) + `>` + '\n'
            }
          } else {
