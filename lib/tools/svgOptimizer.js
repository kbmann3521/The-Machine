...
function normalizeSVG(svg) {
  ...
  let modified = svg.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, (match) => {
    cdataBlocks.push(match)
    return `__CDATA_MARKER_${blockIndex++}__`
  })
  ...
  modified = modified.replace(/__CDATA_MARKER_(\d+)__/g, (match, index) => {
    return cdataBlocks[parseInt(index)]
  })
