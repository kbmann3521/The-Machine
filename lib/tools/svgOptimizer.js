...
function extractCDATA(svg) {
  const cdataBlocks = []
  const placeholder = '\u0000CDATA_BLOCK_{index}\u0000'
  let modified = svg
  let blockIndex = 0

  const cdataPattern = /<!\[CDATA\[([\s\S]*?)\]\]>/g
  modified = modified.replace(cdataPattern, (match) => {
    cdataBlocks.push(match)
    return placeholder.replace('{index}', blockIndex++)
  })

  return { modified, cdataBlocks }
}

function restoreCDATA(svg, cdataBlocks) {
  let modified = svg
  cdataBlocks.forEach((block, index) => {
    const placeholder = '\u0000CDATA_BLOCK_' + index + '\u0000'
    modified = modified.replace(placeholder, block)
  })
  return modified
}
...
const ATTRIBUTE_ORDER = {
  'xmlns': 0, 'xmlns:xlink': 1, 'xmlns:svg': 2,
  'viewBox': 5,
  'id': 15, 'class': 16,
  'x': 20, 'y': 21,
  'cx': 20, 'cy': 21,
  'x1': 20, 'y1': 21, 'x2': 22, 'y2': 23,
  'width': 30, 'height': 31,
  'r': 32, 'rx': 32, 'ry': 33,
  'offset': 34, 'points': 35,
  'd': 40,
  'transform': 50,
  'fill': 60, 'stroke': 61, 'stroke-width': 62, 'stroke-linecap': 63, 'stroke-dasharray': 64, 'stroke-linejoin': 65,
  'opacity': 70, 'fill-opacity': 71, 'stroke-opacity': 72, 'fill-rule': 73,
  'filter': 80, 'mask': 81, 'clip-path': 82,
  'text-anchor': 90, 'font-size': 91, 'font-family': 92, 'font-weight': 93, 'font-style': 94,
  'href': 100, 'xlink:href': 100,
  'stdDeviation': 110, 'in': 111, 'result': 112,
  'stop-color': 113, 'stop-opacity': 114,
  'role': 130, 'aria-label': 131, 'aria-labelledby': 132, 'aria-describedby': 133,
  'version': 200
}
...
function tokenizeSVG(svg) {
  ...
  while (i < svg.length) {
    const openIdx = svg.indexOf('<', i)

    if (openIdx === -1) {
      const textContent = svg.substring(i).trim()
      if (textContent) {
    ...
    if (openIdx > i) {
      const textContent = svg.substring(i, openIdx)
      if (textContent.trim()) {
        tokens.push({ type: 'text', content: textContent })
      }
    }

    if (svg.substring(openIdx).startsWith('<![CDATA[')) {
      const cdataEndIdx = svg.indexOf(']]>', openIdx)
      if (cdataEndIdx !== -1) {
        const cdataContent = svg.substring(openIdx, cdataEndIdx + 3)
        tokens.push({ type: 'cdata', content: cdataContent, preserveFormat: true })
        i = cdataEndIdx + 3
        continue
      }
    }

    if (svg.substring(openIdx).startsWith('<!--')) {
      const commentEndIdx = svg.indexOf('-->', openIdx)
      if (commentEndIdx !== -1) {
        const commentContent = svg.substring(openIdx, commentEndIdx + 3)
    ...
    const closeIdx = svg.indexOf('>', openIdx)
    if (closeIdx === -1) break

    const tagContent = svg.substring(openIdx + 1, closeIdx)
...
function shouldFormatMultiLine(tagName, attrs, hasChildren) {
  const complexAttrs = attrs.some(a => a.name === 'd' || a.name === 'style')
  if (complexAttrs) return true

  if (attrs.length > 2) return true

  const attrStr = attrs.map(a => `${a.name}="${a.value}"`).join(' ')
  const totalLength = `<${tagName} ${attrStr}>`.length

  if (totalLength > 80) return true

  const mustExpandTags = ['svg']
  if (mustExpandTags.includes(tagName) && hasChildren) return true
  ...
  try {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === 'cdata') {
        ...
        const hasChildren = willHaveChildren
        const isMultiLine = shouldFormatMultiLine(token.tagName, token.attrs, hasChildren)
        ...
        const nextIsOpenTag = nextToken && nextToken.type === 'openTag'
        const nextIsCloseTag = nextToken && nextToken.type === 'closeTag'
        const shouldAddBlankLine = nextIsOpenTag && !nextIsCloseTag && depth === 1
...
function svgOptimizer(text, config = {}) {
  ...
  // Phase 1: Extract CDATA blocks before normalization to preserve formatting
  const { modified: svgWithoutCDATA, cdataBlocks } = extractCDATA(originalSvg)

  // Phase 1: Normalize (whitespace, comments) - keep CDATA blocks as placeholders during optimization
  const normalizedSvg = svgWithoutCDATA
    .replace(/\s+/g, ' ')
    .replace(/>\s+</g, '><')
    .replace(/<!--.*?-->/g, '')
    .trim()
  ...
  // Phase 3: Format output (optimizedSvg is always compact, with CDATA placeholders)
  const outputFormat = config.outputFormat || 'compact'
  let outputSvg = optimizedSvg

  if (outputFormat === 'pretty') {
    outputSvg = formatSVGPretty(optimizedSvg)
    outputSvg = restoreCDATA(outputSvg, cdataBlocks)
  } else {
