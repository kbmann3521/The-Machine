...
const ATTRIBUTE_ORDER = {
  id: 0, class: 1,
  x: 10, y: 10, cx: 10, cy: 10, r: 10, rx: 10, ry: 10,
  width: 10, height: 10, d: 10, points: 10,
...
function shouldFormatMultiLine(tagName, attrCount, attrStr) {
  const structuralElements = ['svg', 'g', 'defs', 'mask', 'filter', 'clipPath', 'pattern', 'symbol', 'style']
  if (structuralElements.includes(tagName)) return true
  if (attrCount > 3) return true
  ...
  try {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === 'openTag') {
        ...
        const isMultiLine = shouldFormatMultiLine(token.tagName, token.attrs.length, attrStr)
        ...
        if (token.selfClosing || (!willHaveChildren && nextToken?.type === 'closeTag')) {
          if (isMultiLine && token.attrs.length > 3) {
            result += currentIndent + `<${token.tagName}\n`
            token.attrs.forEach((attr, idx) => {
        ...
        if (structuralElements.includes(token.tagName) && nextToken) {
          result += '\n'
        }
        ...
        lines.forEach(line => {
          if (line.trim()) result += currentIndent + line + '\n'
        })
      }
    }
