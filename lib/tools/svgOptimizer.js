...
function svgOptimizer(text, config = {}) {
  ...
  // Recalculate stats with the final optimized SVG
  const finalStats = calculateOptimizationStats(originalSvg, optimizedSvg)
  finalStats.stepBytesRemoved = totalBytesRemoved
  finalStats.optimizationSteps = optimizationSteps
  ...
  const potentialOptimizations = {
    precisionReduction: {
      ...
      reason: finalStats.precision.originalDecimals && finalStats.precision.originalDecimals > 2
        ? `High precision decimals detected (${finalStats.precision.originalDecimals} decimals)`
        : null,
    ...
    pathMerging: {
      ...
      executed: false,
      reason: (finalStats.elements.byType.path || 0) > 1 ? `Multiple paths (${finalStats.elements.byType.path}) detected` : null
    },
    removeUnusedDefs: {
