...
function svgOptimizer(text, config = {}) {
  ...
  if (config.phase2 && config.phase2.enabled) {
    ...
    if (finalConfig.precisionReduction.enabled) {
      const blockedByFactors = []
      
      if (safetyFlags.hasText && finalConfig.precisionReduction.decimals < 3) {
        blockedByFactors.push('text (may cause font rendering issues)')
      }
      if (safetyFlags.hasFilters && finalConfig.precisionReduction.decimals < 2) {
        blockedByFactors.push('filters (sensitive to coordinate changes)')
      }
      if (safetyFlags.hasMasks && finalConfig.precisionReduction.decimals < 2) {
        blockedByFactors.push('masks (visual clipping may shift)')
      }

      const isPossible = stats.precision.originalDecimals !== null && stats.precision.originalDecimals > 0
      const isSafe = blockedByFactors.length === 0
      let executed = false
      let bytesRemoved = 0

      if (isSafe && isPossible && stats.precision.originalDecimals > finalConfig.precisionReduction.decimals) {
      ...
      stepResults.push({
        ...
        safe: isSafe,
        executed,
        blockedBy: isSafe ? undefined : blockedByFactors,
        reason: executed 
          ? `Reduced from ${stats.precision.originalDecimals} to ${finalConfig.precisionReduction.decimals} decimals`
          : isSafe 
          ? `No precision reduction needed (already at or below ${finalConfig.precisionReduction.decimals} decimals)`
          : `Blocked by: ${blockedByFactors.join(', ')}`,
        bytesRemoved
      })
    }
    ...
    if (finalConfig.pathMerging && finalConfig.pathMerging.enabled) {
      ...
      const isSafe = !safetyFlags.hasAnimations && !safetyFlags.hasFilters && !safetyFlags.hasMasks
      let executed = false
      let bytesRemoved = 0
      ...
      stepResults.push({
        ...
        safe: isSafe,
        executed,
        blockedBy: isSafe ? undefined : (safetyFlags.hasAnimations ? ['animations'] : []).concat(
          safetyFlags.hasFilters ? ['filters'] : [],
          safetyFlags.hasMasks ? ['masks'] : []
        ),
        reason: executed
          ? `Merged compatible paths, removed ${bytesRemoved} bytes`
          : isSafe
          ? `No mergeable paths found or no size reduction achieved`
          : `Blocked by: ${(safetyFlags.hasAnimations ? ['animations'] : []).concat(
              safetyFlags.hasFilters ? ['filters'] : [],
              safetyFlags.hasMasks ? ['masks'] : []
            ).join(', ')}`,
        bytesRemoved
    ...
    if (finalConfig.idCleanup && finalConfig.idCleanup.mode === 'minify' && analysis.references.idsDefined.length > 0) {
      ...
      stepResults.push({
        ...
        reason: executed
          ? `Minified ${analysis.references.idsDefined.length} IDs`
          : isSafe
  ...
  const potentialOptimizations = {
    ...
    pathMerging: {
      possible: (finalStats.elements.byType.path || 0) > 1,
      safe: true,
      executed: false
    },
    removeUnusedDefs: {
