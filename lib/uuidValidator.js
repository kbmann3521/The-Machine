...
function validateStructure(uuid) {
  ...
  if (![1, 3, 4, 5, 7].includes(versionNibble)) {
    errors.push(`Invalid version nibble: expected 1, 3, 4, 5, 6, or 7 but found '${versionNibble.toString(16).toLowerCase()}'`);
  }
...
function generateSummary(version, variant, valid, errors) {
  ...
  return `Valid ${variant} ${versionName}`;
}
...
function validate(input) {
  ...
  const wasNormalized = normalized !== input && normalized !== input.toLowerCase().replace(/-/g, '');
  ...
  if (!structureValidation.valid) {
    return {
      ...
      summary: generateSummary(null, null, false, structureValidation.errors),
    };
  }
