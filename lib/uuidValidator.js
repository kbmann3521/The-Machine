...
function extractTimestampV7(uuid) {
  // UUID v7 stores Unix timestamp in milliseconds as first 48 bits
  const bits = extractBits(uuid);
  if (!bits) return null;

  // Reconstruct the 48-bit timestamp from first 12 hex chars
  const timestampHex = bits.time_low + bits.time_mid;
  const milliseconds = parseInt(timestampHex, 16);

  if (milliseconds > 0 && milliseconds < Date.now() + 1000000) {
    const date = new Date(milliseconds);
    return date.toISOString();
  }
...
function validate(input) {
  ...
  if (version === 7) {
    const timestamp = extractTimestampV7(normalized);
    if (timestamp) {
      response.timestamp = timestamp;
    }
  }
