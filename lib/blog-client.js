...
export async function getPostById(postId) {
  const authHeader = await getAuthHeader()
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_SITE_URL || '')
  const getUrl = `${baseUrl}/api/blog/get-post?id=${postId}`
...
export async function getAllPosts() {
  const authHeader = await getAuthHeader()
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_SITE_URL || '')
  const getPostsUrl = `${baseUrl}/api/blog/get-posts`
...
export async function publishPost(postId) {
  const authHeader = await getAuthHeader()
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_SITE_URL || '')
  const publishUrl = `${baseUrl}/api/blog/publish-post`
...
export async function unpublishPost(postId) {
  const authHeader = await getAuthHeader()
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_SITE_URL || '')
  const unpublishUrl = `${baseUrl}/api/blog/publish-post`
