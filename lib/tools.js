...
function emailValidator(text) {
  ...
  const results = emails.map((email) => {
    ...
    // Words like "gadrgadbgoudbgu" have patterns like "gadr", "gadb", "godb" - gibberish patterns
    // We look for repeated trigrams (3-letter sequences) that appear 2+ times
    ...
    // If 3+ trigrams appear 2+ times, it's likely gibberish with repeated patterns
    const repeatedTrigrams = Array.from(trigrams.values()).filter(count => count >= 2).length
    if (repeatedTrigrams >= 3) {
      details.usernameHeuristics.push('Username appears to be gibberish or randomly generated')
    }

    // Also check for high consonant density in short stretches
    // Words with 4+ consonants in 5-char window = gibberish
