...
import { svgOptimizer as svgOptimizerModule } from './tools/svgOptimizer.js'
import {
...
Bob Johnson,bob@example.com,45,false`,
  ...
  'svg-optimizer': {
    ...
    configSchema: [],
    detailedDescription: {
      overview: 'The SVG Optimizer tool removes unnecessary content from SVG files, reducing file size while maintaining visual appearance. Provides detailed optimization statistics and structural analysis for informed decisions.',
      howtouse: [
        'Paste your SVG code',
        'The tool analyzes and optimizes it',
        'View optimization statistics showing size reduction and changes made',
        'Review the SVG structural analysis (elements, features detected)',
        'Check the change highlights to see exactly what was modified',
        'Copy the optimized SVG code',
      ],
      features: [
        'Minify SVG markup and remove unnecessary whitespace',
        'Remove comments and metadata',
        'Optimization statistics with file size comparison',
        'SVG structural analysis showing element types and features',
        'Detailed change highlights showing removed attributes and precision changes',
        'Trust indicators for confident optimization',
      ],
      usecases: [
        'Web icon optimization with confidence',
        'Reducing SVG file sizes with detailed insights',
        'Performance optimization with visibility into changes',
        'Cleaning SVG exports from design tools',
        'Verifying SVG structure before deployment',
      ],
    },
  },
  'unit-converter': {
...
export async function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'svg-optimizer': {
      const phase0_1Result = await svgOptimizerModule(inputText, config)
      if (config?.phase2 && config.phase2.enabled) {
        return applyPhase2Optimization(phase0_1Result, config.phase2.level, config.phase2.overrides)
      }
...
function validateSVG(svg) {
  ...
  try {
    ...
    // Check for namespace issues (non-standard is only a warning, not an error)
...
function trackReferences(svg) {
  ...
  try {
    // Extract all IDs defined in the SVG
...
function analyzeSVGStructure(svg) {
  // Note: analysis.elements counts element types for capability detection (what features exist in this SVG).
  // This is distinct from stats.elements.byType which tracks element changes for reporting.
  // Separation: analysis = what the SVG contains, stats = what changed during optimization.
  const analysis = {
    elements: {},
    features: {
      usesText: false,
...
function calculateOptimizationStats(originalSvg, optimizedSvg) {
  ...
  // Count attributes and track by element type
  // Attributes include: presentation attrs (fill, stroke, etc.), structural attrs (id, class, data-*),
  // transform attrs (transform, x, y, etc.), and namespace attrs (xmlns, xmlns:xlink, etc.)
  // This definition is consistent for deterministic comparisons across optimization phases.
  ...
  // Count attributes by element type - provides traceability for which elements had the most attributes
  // Useful for understanding optimization impact per element and identifying optimization opportunities
  const attrByElement = {}
  ...
  // Count elements by type - tracks all element tags and their frequency
  // Note: This is distinct from analysis.elements (which also counts elements but for structure analysis).
  // stats.elements.byType is used for change reporting. analysis.elements is used for capability detection.
  const elemByType = {}
  ...
  // Detect precision changes in numeric values (coordinates, transforms, etc.)
  // Currently tracks maximum decimal places across all numbers.
  // Phase 2+ enhancement: may differentiate between path precision, transform precision, and numeric attribute precision.
...
function generateDiffHighlights(originalSvg, optimizedSvg) {
  ...
  try {
    ...
    // Find removed elements (comments, metadata, etc.)
    ...
    // Detect precision changes in numbers
    ...
    // Determine overall change summary
    if (hasChanges) {
...
function detectNormalization(originalSvg, optimizedSvg) {
  ...
  // Check for space before closing tags
...
function svgOptimizer(text) {
  ...
  // Determine optimization result outcome
  ...
  // Build safety flags for Phase 2 logic
  // hasInternalRefs: IDs defined and referenced within the same SVG (e.g., gradients, masks, patterns)
  // hasExternalRefs: References to external resources (HTTP URLs, external files)
  ...
  // Detect potential optimizations for Phase 2 with justifications
...
const PHASE2_PRESETS = {
  ...
  balanced: {
    ...
    config: {
      ...
      shapeConversion: { enabled: true, allowedShapes: ['rect', 'circle', 'ellipse', 'line', 'polygon'] },
      pathMerging: { enabled: false },
      idCleanup: { enabled: true, mode: 'remove-unused' },
      textHandling: { mode: 'preserve' }
    }
  },
  aggressive: {
...
function checkSafetyGates(optimization, safetyFlags, config) {
  ...
  switch (optimization) {
    case 'precisionReduction':
      if (safetyFlags.hasText) {
        gating.allowed = false
        gating.reason = 'Text elements present: precision reduction may affect rendering'
        gating.skipped = true
      }
      break

    case 'shapeConversion':
      if (safetyFlags.hasAnimations || safetyFlags.hasScripts) {
        gating.allowed = false
        gating.reason = 'Animations or scripts detected: shape conversion may break functionality'
      }
      break
    ...
    case 'pathMerging':
      if (config.shapeConversion?.enabled !== true) {
        gating.allowed = false
        gating.reason = 'Path merging requires shape conversion to be enabled'
      }
      break
    ...
    case 'idCleanup':
      if (safetyFlags.hasBrokenReferences) {
        gating.allowed = false
        gating.reason = 'Broken ID references detected: ID cleanup not allowed'
      } else if (config.idCleanup?.mode === 'minify' && safetyFlags.hasExternalRefs) {
        gating.allowed = false
        gating.reason = 'External references detected: ID minification could break external links'
      }
      break
...
// Helper: Reduce precision in numbers
function reducePrecision(svg, decimals) {
  let reduced = svg
  const numberPattern = /(\d+\.\d+)/g

  reduced = reduced.replace(numberPattern, (match) => {
...
function convertShapesToPaths(svg, allowedShapes = ['rect', 'circle', 'ellipse', 'line', 'polygon']) {
  let converted = svg
  const shapesToConvert = allowedShapes.join('|')
  const shapePattern = new RegExp(`<(${shapesToConvert})\\s+([^>]*)\\s*/>`, 'gi')

  const convertedCount = (svg.match(shapePattern) || []).length

  // Simple conversion: replace with path placeholders (full SVG->Path math is complex)
  // In production, use proper SVG path conversion library
  if (convertedCount > 0) {
    converted = converted.replace(shapePattern, (match, tag, attrs) => {
      return `<path ${attrs} />`
    })
  }

  return { svg: converted, convertedCount }
}
...
function cleanupIDs(svg, mode = 'remove-unused', safetyFlags = {}) {
  let cleaned = svg
  const idPattern = /\bid="([^"]+)"/g
  const refPattern = /url\(#([^)]+)\)|xlink:href="#([^"]+)"|href="#([^"]+)"|<use[^>]+href="#([^"]+)"/g
  ...
  while ((match = refPattern.exec(svg)) !== null) {
    const refID = match[1] || match[2] || match[3] || match[4]
    usedIDs.add(refID)
  }

  let removedCount = 0

  if (mode === 'remove-unused') {
    // Remove unused IDs
    definedIDs.forEach((id) => {
      if (!usedIDs.has(id)) {
        cleaned = cleaned.replace(new RegExp(`\\sid="${id}"`, 'g'), '')
        removedCount++
      }
    })
  } else if (mode === 'minify' && !safetyFlags?.hasExternalRefs) {
    // Minify IDs with deterministic base-52
    const idMap = {}
    let minifyIndex = 0
    ...
    Object.entries(idMap).forEach(([oldID, newID]) => {
      cleaned = cleaned.replace(new RegExp(`id="${oldID}"`, 'g'), `id="${newID}"`)
      cleaned = cleaned.replace(new RegExp(`#${oldID}`, 'g'), `#${newID}`)
      removedCount++
    })
  }

  return { svg: cleaned, removedCount, usedCount: usedIDs.size }
}
...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}) {
  ...
  if (advancedOverrides.attributeCleanup !== undefined) {
    config.attributeCleanup = { ...config.attributeCleanup, enabled: advancedOverrides.attributeCleanup }
  }
  if (advancedOverrides.precisionReduction !== undefined) {
    config.precisionReduction = { ...config.precisionReduction, enabled: advancedOverrides.precisionReduction }
  }
  if (advancedOverrides.decimals !== undefined) {
    config.precisionReduction = { ...config.precisionReduction, decimals: advancedOverrides.decimals }
  }
  if (advancedOverrides.shapeConversion !== undefined) {
    config.shapeConversion = { ...config.shapeConversion, enabled: advancedOverrides.shapeConversion }
  }
  if (advancedOverrides.pathMerging !== undefined) {
    config.pathMerging = { ...config.pathMerging, enabled: advancedOverrides.pathMerging }
  }
  if (advancedOverrides.idCleanup !== undefined) {
    config.idCleanup = { ...config.idCleanup, mode: advancedOverrides.idCleanup, enabled: advancedOverrides.idCleanup !== 'preserve' }
  }
  if (advancedOverrides.textHandling !== undefined) {
    config.textHandling = { ...config.textHandling, mode: advancedOverrides.textHandling }
  }
  if (advancedOverrides.textToPathConfirmed !== undefined) {
    config.textHandling = { ...config.textHandling, userConfirmed: advancedOverrides.textToPathConfirmed }
  }
  ...
  const steps = [
    ...
    { name: 'precisionReduction', fn: () => reducePrecision(currentSvg, config.precisionReduction.decimals) },
    { name: 'shapeConversion', fn: () => convertShapesToPaths(currentSvg, config.shapeConversion.allowedShapes) },
    { name: 'pathMerging', fn: () => mergePaths(currentSvg) },
    { name: 'idCleanup', fn: () => cleanupIDs(currentSvg, config.idCleanup.mode, phase0_1Result.safetyFlags) },
    { name: 'textHandling', fn: () => textHandling(currentSvg, config.textHandling.mode, config.textHandling.userConfirmed) }
  ]

  for (const step of steps) {
    const gating = checkSafetyGates(step.name, phase0_1Result.safetyFlags, config)
    ...
    if (step.name === 'attributeCleanup') {
      stepEnabled = config.attributeCleanup?.enabled !== false
    } else if (step.name === 'precisionReduction') {
      stepEnabled = config.precisionReduction?.enabled !== false
    } else if (step.name === 'shapeConversion') {
      stepEnabled = config.shapeConversion?.enabled !== false
    } else if (step.name === 'pathMerging') {
      stepEnabled = config.pathMerging?.enabled !== false
    } else if (step.name === 'idCleanup') {
      stepEnabled = config.idCleanup?.enabled !== false
    } else if (step.name === 'textHandling') {
      stepEnabled = config.textHandling?.mode !== 'preserve'
    }
  ...
  const finalDiff = generateDiffHighlights(phase0_1Result.originalSvg, currentSvg)
