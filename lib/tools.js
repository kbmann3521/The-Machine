...
function parseCIDRNotation(input) {
  ...
  if (cidr > maxCIDR) return null

  return { base, cidr, isIPv4, normalized: `${base}/${cidr}` }
}

/**
 * Detects and parses IP range notation
 * Supports: A-B, A - B, A to B, A .. B, A…B (unicode ellipsis), A – B (en dash), A — B (em dash)
 */
function parseIPRange(input) {
  ...
  if (!isIPv4Start && !isIPv6Start) return null
...
function validateIPAddress(inputText, config = {}) {
  ...
  try {
    // Phase 2: Check for CIDR notation first
    ...
    return buildSingleIPResult(inputTrimmed, rawInput, config)
...
function buildSingleIPResult(ip, rawInput, config) {
  ...
  if (version === 4) {
    ...
    if (config.ipv6Expansion !== false) {
      results.expanded = expandIPv6(ip)
      results.compressed = compressIPv6(ip)
      results.hextets = getIPv6Hextets(ip)
    }
    if (config.ipv6Binary !== false) {
      results.ipv6Binary = ipv6ToBinary(ip)
      results.ipv6BinaryDotted = ipv6ToBinaryDotted(ip)
    }
  }

  // PTR records
  if (config.ptrRecord !== false) {
    const ptrData = generatePTR(ip)
    if (ptrData) {
...
function buildCIDRResult(cidrParsed, rawInput, config) {
  const { base, cidr, isIPv4, normalized } = cidrParsed
  ...
  if (isIPv4) {
    ...
    const networkAddress = getNetworkAddress(base, cidr)
    const broadcastAddress = getBroadcastAddress(base, cidr)
    const firstHost = getFirstHost(base, cidr)
    const lastHost = getLastHost(base, cidr)
    ...
    // ISSUE FIX #2 & #3: /32 special case handling
    // For /32: network = broadcast = host (no broadcast address in traditional sense)
    // isBroadcastAddress should be false for /32 (it's a host, not broadcast)
    ...
    // ISSUE FIX #1: Ensure integer values are never null
    const networkInt = networkAddress ? ipv4ToInteger(networkAddress) : null
    const broadcastInt = broadcastAddress ? ipv4ToInteger(broadcastAddress) : null
    ...
    // ISSUE FIX #4: Don't show RFC 1918 for public IPs
    const cleanClassification = { ...baseClassification }
    if (baseClassification.isPublic && baseClassification.rfc === 'RFC 1918') {
      cleanClassification.rfc = null
    }
    ...
    // IPv6 CIDR is more complex - for Phase 2, we'll include expanded/compressed forms
    results.cidr = {
      ...results.cidr,
...
function buildRangeResult(rangeParsed, rawInput, config) {
  const { start, end, startInt, endInt, isIPv4, isIPv6 } = rangeParsed
  ...
  if (isIPv4) {
    ...
    // Default assumption: if first 3 octets match, they're in the same /24
    ...
    if (startClassification.rfc && startClassification.rfc === endClassification.rfc) {
      // ISSUE FIX #4: Don't show duplicate RFC if it's already in classification type
