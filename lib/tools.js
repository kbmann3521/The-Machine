...
function buildSingleIPResult(ip, rawInput, config) {
  ...
  if (version === 4) {
    ...
    // Zone index parsing (fe80::1%eth0 format)
    ...
    if (getIPv4FromIPv6Mapped(ip)) {
      results.isIPv4Mapped = true
      results.mappedIPv4 = getIPv4FromIPv6Mapped(ip)
    }

    // Classify special IPv6 types
    const ipv6Special = classifyIPv6Special(ip)
    if (ipv6Special) {
...
function buildRangeResult(rangeParsed, rawInput, config) {
  const { start, end, startInt, endInt, isIPv4, isIPv6, separator } = rangeParsed
