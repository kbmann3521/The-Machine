...
function jsFormatter(text, config = {}) {
  ...
  try {
    ...
    let processedCode = trimmedText
    if (removeComments) {
      processedCode = removeJsComments(processedCode)
    }
    if (removeConsole) {
      processedCode = removeJsConsole(processedCode)
    }

    switch (mode) {
      case 'format':
        result.formatted = formatJavaScript(processedCode, config)
        if (showErrors) {
          result.errors = detectSyntaxErrors(processedCode)
        }
        if (showAnalysis) {
          result.analysis = analyzeJavaScript(processedCode)
        }
        if (showLinting) {
          result.linting = lintJavaScript(processedCode)
        }
        break

      case 'minify':
        result.minified = minifyJavaScript(processedCode, { compressCode })
        if (showErrors) {
          result.errors = detectSyntaxErrors(processedCode)
        }
        if (showAnalysis) {
      ...
      case 'obfuscate':
        result.obfuscated = obfuscateJavaScript(processedCode)
        if (showErrors) {
          result.errors = detectSyntaxErrors(processedCode)
        }
        if (showLinting) {
