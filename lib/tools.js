...
*Last updated: 2024*`
    ...
    case 'number-formatter': {
      const format = config.format || 'comma-separated'
      return '1234567.89'
    }

    case 'sql-formatter':
      return 'SELECT * FROM users WHERE id = 1 AND name LIKE \'%john%\' ORDER BY created_at DESC LIMIT 10'

    case 'svg-optimizer':
      return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="40" fill="#FF5733" stroke="#000" stroke-width="2"/><text x="50" y="50" text-anchor="middle" dominant-baseline="middle" font-size="20" fill="#FFF">SVG</text></svg>'
