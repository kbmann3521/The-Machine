...
function emailValidator(text) {
  ...
  const results = emails.map((email) => {
    ...
    // Check for excessive consonant clusters (unnatural patterns like "gdbg", "tdjf", etc.)
    // English allows max 3 consonants in a row (e.g., "strength"), but strings like "gdrgadbg" are gibberish
    const consonantClusters = localPart.match(/[bcdfghjklmnpqrstvwxyz]{4,}/gi) || []
    if (consonantClusters.length >= 2 || (consonantClusters.length === 1 && localPart.length > 15)) {
      details.usernameHeuristics.push('Username appears to be gibberish or randomly generated')
    }
