...
export async function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'xml-formatter':
      return await xmlFormatter(inputText, config)
    case 'yaml-formatter':
...
async function xmlFormatter(text, config) {
  ...
  try {
    ...
    if (mode === 'beautify' || mode === 'minify' || mode === 'clean') {
      // Import the new XML formatter module
      let processXmlTool
      try {
        const xmlFormatterModule = await import('./tools/xmlFormatter.js')
      ...
      if (showRepairInfo && pipelineResult.repairedXml !== text) {
        result.repairInfo = {
          wasRepaired: true,
          originalXml: pipelineResult.originalXml,
          repairedXml: pipelineResult.repairedXml,
        }
      }
