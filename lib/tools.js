...
function autoRepairXml(xml) {
  ...
  try {
    ...
    // Fix closing tags missing > (e.g., </title becomes </title>)
    preprocessed = preprocessed.replace(/<\/\w+(?!>)(\s*?)(?=<|$)/g, (match, ws) => {
      const tagName = match.match(/<\/(\w+)/)[1]
      return '</' + tagName + '>' + ws
    })

    // Fix opening tags missing > (e.g., <title attr="val" becomes <title attr="val">)
    preprocessed = preprocessed.replace(/<\w+([^<>]*?)(?<!>)(\s*?)(?=<|$)/g, (match, attrs, ws) => {
      if (match.includes('/>')) {
        return match // Already properly closed with />
      }
      // Get the tag name
