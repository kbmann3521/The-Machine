  ...
  if (mode === 'unused-only') {
    ...
    definedIDs.forEach((id) => {
      if (!usedIDs.has(id)) {
        ...
        // Match id attribute with any surrounding whitespace
        // Pattern: optional space/newline before, id="value", optional space after
        const pattern = new RegExp(`\\s*id="${escapedID}"\\s*`, 'g')
        const matches = cleaned.match(pattern)
        if (matches && matches.length > 0) {
          // Replace with single space to preserve attribute separation
          cleaned = cleaned.replace(pattern, ' ')
          removedCount++
        }
      }
