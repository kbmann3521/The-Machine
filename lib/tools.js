...
function classifyParameterValue(value) {
  ...
  // Test against regex patterns from the library
  const patternTests = [
    'email', 'url', 'jwt', 'jwt-loose', 'base64', 'uuid', 'integer',
    'hex-number', 'currency', 'phone', 'ipv4', 'ipv6', 'credit-card', 'ssn-us'
  ]
  ...
  for (const patternId of patternTests) {
    ...
    try {
      ...
      // For patterns with 'g' flag, test if it matches
      // For patterns without 'g', we test the full value
      let testString = value
      if (template.flags && template.flags.includes('g')) {
        // For global patterns, just test if there's a match anywhere
        ...
        // For non-global patterns, test as-is
        if (regex.test(testString)) {
          classifications.push({
            type: template.name,
            emoji: PATTERN_EMOJI_MAP[patternId] || '✓',
  ...
  // Special handling: Base64 JSON detection
  if (REGEX_PATTERN_TEMPLATES.base64) {
    const base64Regex = new RegExp(REGEX_PATTERN_TEMPLATES.base64.pattern, REGEX_PATTERN_TEMPLATES.base64.flags)
  ...
  // Special handling: JSON detection (not in patterns list)
...
function detectParameterRisks(paramKey, paramValue, classifications, isRedirectParam) {
  ...
  // SQL injection indicators - more precise detection
  const sqlInjectionPatterns = [
    /['"]?\s*or\s+1\s*=\s*1/i,           // or 1=1
    /['"]?\s*;\s*(drop|delete|truncate)/i, // ; DROP/DELETE/TRUNCATE
    /union\s+(all\s+)?select/i,           // UNION SELECT
    /exec\s*\(/i,                         // exec()
    /execute\s*\(/i,                      // execute()
    /sleep\s*\(\s*\d+\s*\)/i,            // sleep(n)
    /benchmark\s*\(/i,                    // benchmark()
    /xp_/i,                               // xp_ stored procedures
    /sp_/i,                               // sp_ stored procedures
    /admin['"]?\s*--/i,                   // admin'-- or admin"--
    /['"]?\s+and\s+\d+\s*=\s*\d+/i,      // and 1=1
    /decode\s*\(/i,                       // decode()
    /information_schema/i,                // information_schema access
  ]

  const hasSQLInjection = sqlInjectionPatterns.some(pattern => pattern.test(paramValue))
  if (hasSQLInjection) {
    risks.push({
