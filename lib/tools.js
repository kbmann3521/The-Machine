...
export const TOOLS = {
  ...
  'json-formatter': {
    ...
    configSchema: [
      {
        ...
        options: [
          ...
          { value: 'to-yaml', label: 'Convert to YAML' },
    ...
    detailedDescription: {
      overview: 'The JSON Formatter is a powerful tool for formatting, validating, repairing, and transforming JSON. It handles beautification, minification, validation, repair of common JSON errors, key sorting, flattening, null removal, and conversion to other formats.',
      howtouse: [
        'Paste your JSON code or data into the input field',
        'Enable or disable auto-repair (enabled by default) to fix broken JSON',
        'Select your desired mode: Beautify, Minify, Validate, Sort Keys, Flatten, Unflatten, Remove Nulls, or Convert to YAML',
  ...
  'yaml-formatter': {
    ...
    configSchema: [
      ...
      {
        id: 'jsonInput',
        label: 'JSON Input (for JSON → YAML)',
        type: 'textarea',
    ...
    detailedDescription: {
      ...
      howtouse: [
        ...
        'For JSON → YAML conversion, paste JSON in the JSON Input field',
      ...
      features: [
        ...
        'Convert YAML ↔ JSON bidirectionally',
  ...
  'markdown-html-formatter': {
    ...
    show_in_recommendations: true,
    detailedDescription: {
      overview: 'The Markdown + HTML Formatter is a versatile tool for converting, formatting, and beautifying both Markdown and HTML content. Supports converting between formats, beautifying with proper indentation, and minifying for production.',
      howtouse: [
      ...
      features: [
        ...
        'Fix and clean Markdown syntax with Remark',
      ...
      usecases: [
        ...
        'Clean and fix broken Markdown syntax',
...
export function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'yaml-formatter':
      let textToProcess = inputText
      if (config.jsonInput && config.jsonInput.trim()) {
        textToProcess = jsonToYaml(config.jsonInput)
      } else if (config.mode === 'to-json') {
        return yamlFormatter(inputText, config)
      }
      const finalConfig = { ...config }
      if (config.jsonInput && config.jsonInput.trim()) {
        finalConfig.mode = 'beautify'
      }
      return yamlFormatter(textToProcess, finalConfig)
    case 'url-parser':
...
function markdownHtmlFormatter(text, config) {
  const { convertTo = 'none', indent = '2spaces', minify = false } = config
  ...
  if (convertTo === 'html' && isMarkdown) {
    // Try to load marked (only works in server context)
    if (typeof window === 'undefined') {
      try {
        // Dynamic require in server context
...
function jsonToYAML(text, config) {
  ...
  try {
    const parsed = JSON.parse(trimmed)
    return jsonToYAMLString(parsed, 0)
  } catch (error) {
    return `Error: Invalid JSON format - ${error.message}`
  }
}
...
function jsonToYAMLString(obj, indent = 0) {
  ...
  if (Array.isArray(obj)) {
    ...
    const lines = obj.map(item => {
      const itemStr = jsonToYAMLString(item, indent + 2)
  ...
  const lines = keys.map(key => {
    const value = obj[key]
    const valueStr = jsonToYAMLString(value, indent + 2)
...
function yamlToJson(text) {
  try {
    let yaml
    try {
      yaml = dynamicRequire('yaml')
    } catch (e) {
      try {
        yaml = dynamicRequire('js-yaml')
        const parsed = yaml.load(text)
        return JSON.stringify(parsed, null, 2)
      } catch {
        return { error: 'YAML parsing package not available. Please use a simple YAML to test.' }
      }
    }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const parsed = yaml.load(text)
        try {
          const TOML = dynamicRequire('@iarna/toml')
          return TOML.stringify(parsed)
        } catch {
          return { error: '@iarna/toml package not available' }
        }
      } catch {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        return { error: 'YAML package not available' }
      }
    }
  } catch (error) {
...
function jsonToYaml(jsonText) {
  try {
    ...
    return { error: `Failed to convert JSON to YAML: ${error.message}` }
  }
}
