...
function smartJoinWords(text) {
  ...
  // This detects patterns of single letters/short segments and rejoins them intelligently
  let result = text
  ...
  result = result
    ...
    .map(line => {
      ...
      // Heuristic: average segment length < 3 chars and more than 50% are 1-2 chars
      const avgLength = segments.reduce((sum, w) => sum + w.length, 0) / segments.length
      const shortSegmentRatio = segments.filter(w => w.length <= 2).length / segments.length

      // If fragmentation is detected
      if (avgLength < 3 && shortSegmentRatio > 0.5) {
        // Rejoin segments, creating word breaks when we detect likely word boundaries
        let rejoinedWords = []
        let currentWord = ''
        ...
        for (let i = 0; i < segments.length; i++) {
          ...
          // Heuristic: if current segment starts with uppercase and previous segment ended with lowercase,
          // it's likely a new word boundary
          if (i > 0 && currentWord.length > 0 &&
              segment.length > 0 &&
              segment[0] === segment[0].toUpperCase() &&
              segment[0] !== segment[0].toLowerCase() &&
              currentWord[currentWord.length - 1] === currentWord[currentWord.length - 1].toLowerCase()) {
        ...
        // Only use the rejoined version if it's actually better
        if (rejoinedWords.length > 0 && rejoinedWords.length < segments.length) {
