...
function initializeESLint() {
  if (typeof window === 'undefined' && !eslintModule) {
    try {
      eslintModule = require('eslint')
    } catch (error) {
      console.error('Failed to load ESLint module:', error); // Log the error for debugging
      return false
    }
  }
  return eslintModule
