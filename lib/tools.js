...
try {
  ...
  // SQL formatter packages not available
}
...
try {
  ...
  // JavaScript formatter packages not available
}
...
export const TOOLS = {
  ...
  'string-reverse': {
    ...
    configSchema: [],
    detailedDescription: {
      overview: 'The String Reverse tool instantly reverses any text character by character. Works with any text input including words, sentences, and paragraphs.',
      howtouse: [
        'Enter or paste text into the input field',
        'The tool instantly reverses all characters',
        'View the reversed text in the output',
  ...
  'text-stats': {
    name: 'Text Stats',
    description: 'Analyze text with character, word, line, and paragraph counts',
    category: 'writing',
    inputTypes: ['text'],
    outputType: 'json',
    example: 'The quick brown fox jumps over the lazy dog.\n\nThis is another paragraph.',
    configSchema: [],
    detailedDescription: {
      overview: 'The Text Stats tool provides comprehensive text analysis including character counts, word counts, line counts, and paragraph counts with visual statistics.',
      howtouse: [
        'Paste or type text into the input field',
        'View comprehensive statistics instantly',
        'See character, word, line, and paragraph counts',
        'View visual representations of the data',
      ],
      features: [
      ...
      usecases: [
        ...
        'Content planning and requirements',
        'Writing analysis',
        'Document processing',
      ],
    },
...
export async function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'string-reverse':
      return stringReverse(inputText)
    case 'text-stats':
      return textStatsAnalysis(inputText)
    default:
...
function validateIPAddress(inputText, config = {}) {
  ...
  const ipVersion = config.ipVersion || 'auto'
  const validateIP = config.validateIP !== false
  const normalize = config.normalize || false
  const ipToInteger = config.ipToInteger || false
  const privatePublic = config.privatePublic || false
  ...
  try {
    ...
    if (!parsedIp && (ipVersion === 'auto' || ipVersion === 'ipv6')) {
      try {
    ...
    if (!parsedIp) {
      results.error = `Invalid IP address: "${ip}"`
      if (validateIP) {
    ...
    // Normalize the IP (only when normalize is explicitly checked)
    if (normalize) {
      results.normalized = parsedIp.toString()
    }

    // Convert to integer (IPv4 and IPv6)
    if (ipToInteger) {
      try {
...
function textStatsAnalysis(text) {
  if (!text) {
    return {
      characters: 0,
  ...
  // Sentence count (simple heuristic: . ! ? followed by space or end)
  const sentences = text.split(/[.!?]+/).filter(s => s.trim().length > 0).length

  // Averages
  const averageWordLength = words > 0 ? (charactersWithoutSpaces / words).toFixed(2) : 0
  const averageWordsPerLine = lines > 0 ? (words / lines).toFixed(2) : 0
...
function xmlFormatter(text, config) {
  ...
  try {
    ...
    // For beautify and minify modes, run the auto-repair pipeline
    if (mode === 'beautify' || mode === 'minify' || mode === 'clean') {
      // Step 1: Check if XML is valid
      let isValid = false
      try {
        if (xmlParser && xmlParser.validate(text.trim()) === true) {
          isValid = true
        }
      } catch (e) {
        isValid = false
      ...
      // Step 2: If not valid, attempt auto-repair
      if (!isValid) {
      ...
      switch (mode) {
        ...
        case 'minify':
          ...
          break
        case 'clean':
          modeResult = xmlClean(workingXml, indentSize, config)
          break
      }
    } else {
      // For conversion modes, apply cleanup then convert
      let cleanXml = applyXmlCleanup(text, config)
    ...
    // If mode result is a string, it's the formatted output
    if (typeof modeResult === 'string') {
      result = modeResult
    } else {
      // If mode result is an object, merge it with result
      result = modeResult
    }

    // Add repair info if requested (always include if toggle is on, regardless of whether repairs were made)
    if (showRepairInfo) {
      if (typeof result === 'string') {
        ...
        result.repaired = repairResult
      }
    }

    // Add validation and linting if requested (validate the repaired/processed XML, not the original)
    if (showValidation) {
      // For beautify/minify/clean modes, validate the workingXml (after repairs and cleanup)
      // For other modes, validate the working copy
      const xmlToValidate = (mode === 'beautify' || mode === 'minify' || mode === 'clean') ? workingXml : text
      if (typeof result === 'string') {
        result = { formatted: result, validation: xmlValidate(xmlToValidate) }
      } else {
        result.validation = xmlValidate(xmlToValidate)
      }
    }

    if (showLinting) {
      if (typeof result === 'string') {
        result = { formatted: result, linting: xmlLint(text) }
      } else {
...
function autoRepairXml(xml) {
  ...
  try {
    // Step 1: Fix unclosed tags (tags missing closing > or >/)
    // Look for patterns like <tag attr="value" at end of line followed by content
    repairedXml = repairedXml.replace(/<(\w+)((?:\s+[\w:-]+(?:=(?:"[^"]*"|'[^']*'))?)*)\s*\n/g, (match, tagName, attrs) => {
    ...
    // Step 2: Fix missing closing tags using a stack-based approach
    const openTags = []
    const allTags = []

    // Extract all tag information (position, type, name)
    const tagRegex = /<(\/?)(\w+)(?:\s+[^>]*)?(\/?)>/g
    let tagMatch

    while ((tagMatch = tagRegex.exec(repairedXml)) !== null) {
    ...
    if (openTags.length > 0) {
      ...
      repairedXml = repairedXml.trimEnd() + '\n' + closingTags
    }

    // Step 3: Try to parse the repaired XML with fast-xml-parser
    if (xmlParser) {
      try {
        const parsed = xmlParser.parse(repairedXml)
        const rebuilt = xmlParser.build(parsed, {
          indentBy: '\t',
          supressEmptyNode: false,
          ignoreAttributes: false,
        })

        if (rebuilt && rebuilt !== originalXml) {
          const repairDiffs = getXmlRepairDifferences(originalXml, rebuilt)
          return {
            xml: rebuilt,
            wasRepaired: true,
            method: 'auto-repair',
        ...
        // If parsing still fails after repairs, return the manually repaired version if it differs
        if (repairedXml !== originalXml) {
          const repairDiffs = getXmlRepairDifferences(originalXml, repairedXml)
          if (repairDiffs.length > 0) {
            return {
              xml: repairedXml,
              wasRepaired: true,
              method: 'auto-repair',
...
function xmlToToml(text) {
  try {
    ...
    // First convert XML to YAML
    const yamlResult = xmlToYaml(text)
    if (yamlResult.error) {
      return yamlResult
    }

    // Parse YAML and convert to TOML
    if (!yamlModule) {
      return { error: 'YAML parsing package not available. Please install yaml or js-yaml.' }
    }

    const parsed = yamlModule.parse ? yamlModule.parse(yamlResult) : yamlModule.load(yamlResult)
...
function scanSecurityIssues(code) {
  const issues = []

  const securityPatterns = [
    { pattern: /eval\s*\(/gi, name: 'eval()', severity: 'critical' },
    { pattern: /new\s+Function\s*\(/gi, name: 'Function constructor', severity: 'critical' },
    ...
    { pattern: /\.innerHTML\s*\+=/gi, name: 'innerHTML concatenation', severity: 'high' },
    { pattern: /dangerouslySetInnerHTML/gi, name: 'dangerouslySetInnerHTML', severity: 'high' },
    { pattern: /atob\s*\(/gi, name: 'atob() - Base64 decode', severity: 'medium' },
    { pattern: /btoa\s*\(/gi, name: 'btoa() - Base64 encode', severity: 'medium' },
  ]

  securityPatterns.forEach(({ pattern, name, severity }) => {
    if (pattern.test(code)) {
      issues.push({ pattern: name, severity })
    }
  })

  return {
    riskLevel: issues.length > 0 ? (issues.some(i => i.severity === 'critical') ? 'critical' : 'high') : 'safe',
    issues,
  }
}
...
export function getToolExample(toolId, config = {}) {
  switch (toolId) {
    ...
    case 'remove-extras': {
      ...
      if (config.filterCharacters === 'ascii-only') {
        return 'This text has spëcial chärs and ��mojis 😀 that need removing'
      }
      if (config.filterCharacters === 'keep-accents') {
        return 'This tëxt h��s àccénts and émojis 😀 and symbols #@$%'
      }
