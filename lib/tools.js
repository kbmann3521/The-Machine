...
function emailValidator(text) {
  ...
  const results = emails.map((email) => {
    ...
    if (dotCount > 2) {
      details.usernameHeuristics.push('Username has many dots')
    }

    // Check for randomly generated patterns (high entropy - lots of special chars)
    const specialCharCount = (localPart.match(/[^a-zA-Z0-9]/g) || []).length
    if (specialCharCount / localPart.length > 0.4) {
      details.usernameHeuristics.push('Username appears randomly generated')
    }

    // Check for bot-like strings (common patterns)
    ...
    if (details.issues.length > 0) {
      ...
      if (/^\d|^\d/.test(localPart) || /\d$/.test(localPart)) {
