...
function detectJWT(value) {
  ...
  const jwtLoosePattern = /^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/

  if (jwtPattern.test(value)) {
    return {
      type: 'JWT Token',
      emoji: '🔐',
      description: 'JSON Web Token (3-part format)'
    }
  }
  if (jwtLoosePattern.test(value)) {
    return {
      type: 'JWT Token',
      emoji: '🔐',
      description: 'JWT Token (2-part format)'
    }
  }
  return null
}
...
function detectCreditCard(value) {
  ...
  if (ccPattern.test(value.replace(/\s/g, ''))) {
    return {
      type: 'Credit Card',
      emoji: '💳',
...
function detectSQLInjection(value) {
  if (!value || typeof value !== 'string') return null
  const sqlPattern = /(?i)(union(\s+all)?\s+select|sleep\s*\(|benchmark\s*\(|or\s+1=1|';|--|#|\bselect\b|\bupdate\b|\bdrop\b|\binsert\b|\bdelete\b)/
...
function detectUUID(value) {
  ...
  if (uuidPattern.test(value)) {
    return {
      type: 'UUID',
      emoji: '🆔',
      description: 'UUID/GUID identifier'
    }
  }
  return null
}
...
function detectPureInteger(value) {
  ...
  if (pureIntegerPattern.test(value)) {
    return {
      type: 'Integer',
      emoji: '🔢',
