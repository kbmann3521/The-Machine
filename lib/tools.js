...
function smartJoinWords(text) {
  ...
  // Conservative approach: ONLY join when we have sequences of single-letter words
  // Example: "T h i s" (4 single letters) → "This"
  // But preserve: "This is text" (longer words stay separate)
  ...
  result = result
    .split('\n')
    .map(line => {
      // Split by spaces to analyze actual word lengths
      const words = line.split(/(\s+)/)  // Keep spaces in the split

      // Only process if we have a sequence of single-letter words
      // Strategy: look for patterns like [letter] [letter] [letter] and join them
      let processedWords = []
      let i = 0
      ...
      while (i < words.length) {
        if (i + 2 < words.length &&
          ...
          // Found: single letter + space + single letter
          // Check if this is part of a longer sequence
          let fragment = words[i]
          let j = i + 2

          // Keep joining while we find single-letter words
          ...
          // Add the final single letter if there is one
          if (j < words.length && words[j].length === 1) {
            fragment += words[j]
            j += 1
          }

          processedWords.push(fragment)
          i = j
        } else {
