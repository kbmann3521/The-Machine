  ...
  result = result
    ...
    .map(line => {
      ...
      if (avgLength < 3 && shortSegmentRatio > 0.5) {
        // Detect word boundaries by looking for capital letters followed by lowercase
        // Pattern: if a segment starts with a capital followed by lowercase, it's likely a word start
        let rejoinedWords = []
        let currentWord = ''

        for (let i = 0; i < segments.length; i++) {
          const segment = segments[i]
          const isLikelyWordStart = segment.length >= 2 &&
                                    segment[0] === segment[0].toUpperCase() &&
                                    segment[0] !== segment[0].toLowerCase() &&
                                    segment[1] === segment[1].toLowerCase()

          const isSingleCapital = segment.length === 1 &&
                                  segment[0] === segment[0].toUpperCase() &&
                                  segment[0] !== segment[0].toLowerCase()

          // Start a new word if we have a likely word boundary and we're not on the first segment
          if (i > 0 && currentWord.length > 0 && (isLikelyWordStart || isSingleCapital)) {
            rejoinedWords.push(currentWord)
            currentWord = segment
          } else {
            // Add segment to current word without extra space
            currentWord += segment
          }
        }
        ...
        // Only use the rejoined version if it's actually better (fewer segments means better)
