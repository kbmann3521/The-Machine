export const TOOLS = {
  ...
  'punycode-converter': {
    ...
    description: 'Encode or decode Punycode for internationalized domain names (auto-detects mode)',
    category: 'converter',
    inputTypes: ['text'],
    outputType: 'text',
    example: 'mañana',
    ...
    detailedDescription: {
      overview: 'The Punycode Converter tool encodes Unicode domain names to ASCII-compatible Punycode format and decodes them back. Automatically detects whether your input is Unicode or Punycode. Essential for internationalized domain names (IDNs) that contain non-ASCII characters.',
      howtouse: [
        'Enter a domain name or Punycode string',
        'Select "Auto-detect" to automatically convert based on input',
        'Or manually select "Encode to Punycode" for Unicode domains',
        'Or select "Decode from Punycode" to decode IDN domains',
        'View the converted domain',
      ],
      features: [
        'Auto-detect conversion mode based on input',
        'Encode internationalized domains to Punycode',
        'Decode Punycode to readable Unicode',
        'Support for all Unicode characters',
        'Works with domain names and individual labels',
      ],
      usecases: [
        ...
        'Converting between Unicode and ASCII domain formats',
      ],
    },
  },
  'binary-converter': {
...
function punycodeConverter(text, config) {
  try {
    ...
    if (!trimmedText) return { error: 'Input cannot be empty' }

    let mode = config.mode || 'auto'

    // Auto-detect mode based on input
    if (mode === 'auto') {
      mode = looksLikePunycode(trimmedText) ? 'decode' : 'encode'
    }

    if (mode === 'encode') {
      return { result: encodePunycode(trimmedText) }
    } else {
      return { result: decodePunycode(trimmedText) }
    }
  } catch (e) {
...
function looksLikePunycode(text) {
  // Check if text contains xn-- prefix which indicates punycode
  // Also verify it's ASCII-only (valid punycode should be ASCII)
  const lowerText = text.toLowerCase()
  if (!lowerText.includes('xn--')) {
    return false
  }
  // Valid punycode should be ASCII-only (no Unicode characters)
  return /^[a-zA-Z0-9\-.]*$/.test(text)
}

function encodePunycode(text) {
  // Handle domain names with multiple labels
  if (text.includes('.')) {
    return text.split('.').map(label => encodePunycodeLabel(label)).join('.')
  }
  return encodePunycodeLabel(text)
}

function decodePunycode(text) {
  // Handle domain names with multiple labels
  if (text.includes('.')) {
    return text.split('.').map(label => decodePunycodeLabel(label)).join('.')
  }
  return decodePunycodeLabel(text)
}

function encodePunycodeLabel(label) {
  if (!label) return label

  // Check if label only contains ASCII
  if (/^[a-zA-Z0-9-]*$/.test(label)) {
    return label
  }

  // Punycode encoding (RFC 3492)
  const codePoints = Array.from(label).map(c => c.charCodeAt(0))
  const basicCodePoints = []
  const nonBasicCodePoints = []

  codePoints.forEach((cp, idx) => {
    if (cp < 128) {
      basicCodePoints.push(idx)
    } else {
      nonBasicCodePoints.push(cp)
    }
  })

  if (nonBasicCodePoints.length === 0) {
    return label
  }

  let output = ''
  for (const idx of basicCodePoints) {
    output += label[idx]
  }
  ...
  let delta = 0
  let bias = 72
  const uniqueNonBasic = [...new Set(nonBasicCodePoints)].sort((a, b) => a - b)

  for (const cp of uniqueNonBasic) {
    delta += (cp - n) * (basicCodePoints.length + uniqueNonBasic.indexOf(cp) + 1)
    n = cp
    ...
    for (let j = 0; j < label.length; j++) {
      ...
      if (charCode < n) {
        delta++
        if (delta > 2147483647) return 'Overflow'
      }
      if (charCode === n) {
        let q = delta
        for (let k = 36; ; k += 26) {
          ...
          if (q < t) {
            output += String.fromCharCode(q + (q < 26 ? 97 : 22))
            break
          }
          output += String.fromCharCode(t + 97 - 1)
          q = Math.floor((q - t) / 26)
        }
        bias = adaptBias(delta, basicCodePoints.length + uniqueNonBasic.indexOf(cp) + 1, basicCodePoints.length === 0)
        delta = 0
      }
    }
  }
...
function adaptBias(delta, numPoints, isFirst) {
  delta = isFirst ? Math.floor(delta / 700) : Math.floor(delta / 2)
  ...
  return k + Math.floor(26 * delta / (delta + 38))
}
...
function decodePunycodeLabel(label) {
  ...
  const encoded = label.substring(4)
  const lastDashIndex = encoded.lastIndexOf('-')
  let basic = ''
  let output = []

  if (lastDashIndex > 0) {
    basic = encoded.substring(0, lastDashIndex)
  ...
  let i = 0
  let bias = 72
  let pos = lastDashIndex > 0 ? lastDashIndex + 1 : 0
  ...
  while (pos < encoded.length) {
    ...
    while (pos < encoded.length) {
      const c = encoded.charCodeAt(pos++) - 48
      const digit = c < 10 ? c : (c < 1039 ? c - 39 : -1)
    ...
    bias = adaptBias(delta, len, oldi === 0)
  }
