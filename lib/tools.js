...
Bob Johnson,bob@example.com,45,false`,
  ...
  'time-normalizer': {
    ...
    description: 'Parse and normalize dates from multiple formats (timestamps, ISO 8601, human-readable)',
    category: 'converter',
    inputTypes: ['text'],
    outputType: 'json',
    ...
    detailedDescription: {
      overview: 'The Time Normalizer tool accepts dates in any format and normalizes them to a standardized representation. Supports Unix timestamps (seconds/milliseconds), ISO 8601, and human-readable date strings.',
      howtouse: [
        'Enter a date/time in any supported format',
        'Tool automatically detects the format',
        'View the normalized date in multiple formats',
      ],
      features: [
...
function timeNormalizer(text, config = {}) {
  ...
  if (!trimmed) {
    return { error: 'Parsing failed — expected (ISO, timestamp, YYYY-MM-DD HH:mm:ss)' }
  }
  ...
  if (/^\d+$/.test(trimmed)) {
    ...
    // Timestamps in seconds are typically 10 digits (1970-2286)
    // Timestamps in milliseconds are typically 13 digits (1970-2286)
    if (timestamp.toString().length === 10) {
      ...
      // Try to parse as seconds if reasonable
  ...
  // Step 3: Try RFC 3339 format (similar to ISO but stricter)
  if (!dt || !dt.isValid) {
  ...
  // Step 4: Try YYYY-MM-DD HH:mm:ss format
  if (!dt || !dt.isValid) {
  ...
  // Step 5: Try date-only YYYY-MM-DD
  if (!dt || !dt.isValid) {
  ...
  // Get timezone config
  const inputTimezone = config.inputTimezone || 'auto'
  const outputTimezone = config.outputTimezone || 'UTC'

  // If input is timestamp, dt is already in UTC, so we can skip the inputTimezone override
  const isTimestamp = detectedFormat.includes('Unix Timestamp')

  // If inputTimezone is not 'auto' and it's not a timestamp, interpret the parsed time as being in that timezone
  let dtInInputZone = dt
  if (inputTimezone !== 'auto' && !isTimestamp) {
    // The dt was parsed without timezone info, so treat its local time as being in the input timezone
    dtInInputZone = dt.setZone(inputTimezone, { keepLocalTime: true })
  } else if (inputTimezone !== 'auto' && isTimestamp) {
    // For timestamps, apply the input timezone as the offset for display purposes
    dtInInputZone = dt
  }
  ...
  return {
    ...
    inputTimezone: inputTimezone === 'auto' ? 'Auto' : inputTimezone,
    ...
    timeDifference: `${timeDifferenceHours >= 0 ? '+' : ''}${timeDifferenceHours} hours`,
    dayBoundaryShift,
...
const TOOL_EXAMPLES = {
  ...
  'time-normalizer': () => ['2024-01-15 14:30:00'],
  'base-converter': () => ['255'],
