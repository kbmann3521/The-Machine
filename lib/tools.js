...
function autoRepairXml(xml) {
  ...
  try {
    ...
    try {
      const doc = libxmljs2.parseXml(xml, { recover: true })
      const repaired = doc.toString()

      if (repaired && repaired !== originalXml) {
        const repairs = getXmlRepairDifferences(originalXml, repaired)
        return {
          xml: repaired,
          wasRepaired: true,
          method: 'libxmljs2-recovery',
          repairs: repairs,
        }
      }
    } catch (e) {
      // If libxmljs2 fails, return original
      return { xml: originalXml, wasRepaired: false, method: null, repairs: [] }
    }
