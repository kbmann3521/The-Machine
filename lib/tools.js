...
function xmlFormatter(text, config) {
  ...
  try {
    ...
    // Add validation and linting if requested
    if (showValidation) {
      if (typeof result === 'string') {
        result = { formatted: result, validation: xmlValidate(text) }
      } else {
        result.validation = xmlValidate(text)
      }
    }
...
function autoRepairXml(xml) {
  ...
  try {
    // Step 1: Repair common XML formatting issues
    const repairs = []

    // Fix unclosed tags by finding tags without closing >
    const unclosedTagRegex = /(<\w+[^>]*[^/>])\s*\n/g
    let match
    let hasUnclosedTags = false
    while ((match = unclosedTagRegex.exec(repairedXml)) !== null) {
      const tag = match[1]
    ...
    // Fix missing closing brackets in tags
    if (repairedXml.includes('<') && !repairedXml.match(/>.*$/)) {
      // Find incomplete tags at end of lines and close them
      repairedXml = repairedXml.replace(/(<\w+[^>]*[^/>])\s*\n/g, (match, tag) => {
        if (!tag.endsWith('>') && !tag.endsWith('/')) {
          return tag + '>\n'
        }
        return match
    ...
    // Fix missing closing tags using a stack-based approach
    const openTags = []
    const tagRegex = /<\/?\w+[^>]*>/g
    let tagMatch
    let lastIndex = 0
    ...
    // Step 2: Try to parse the repaired XML with fast-xml-parser
    if (xmlParser) {
      try {
        ...
        // If parsing still fails after repairs, return the manually repaired version
