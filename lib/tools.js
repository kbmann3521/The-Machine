...
function emailValidator(text) {
  ...
  const results = emails.map((email) => {
    ...
    // Check for high entropy/nonsensical pattern
    // High entropy = lots of consonants with no vowels, or random char combinations
    const vowelsInLocal = (localPart.match(/[aeiou]/gi) || []).length
    const vowelRatioLocal = vowelsInLocal / localPart.length

    // If less than 15% vowels in local part AND has special chars, it's likely gibberish
    const specialCharsInLocal = (localPart.match(/[^a-zA-Z0-9.-]/g) || []).length
    if (vowelRatioLocal < 0.15 && specialCharsInLocal >= 2) {
      details.usernameHeuristics.push('Username appears to be gibberish or randomly generated')
    }

    // Check for mixed case with numbers and special chars (high entropy pattern like "aBc123!@#")
    ...
    // 4+ different character types + longer username = suspicious
    ...
    // Check if TLD is rarely used (less common ones)
    ...
    if (MailChecker && details.issues.length === 0 && !isDisposable) {
      try {
    ...
    details.domainExists = null // Will be fetched via API
    details.mxRecords = []
    ...
    // HIGH-RISK PENALTIES (Severe — remove 60–100 points)
    if (details.issues.length > 0) {
      // Syntax errors = guaranteed bad
      score = 0
      details.penalties.push({ issue: 'Syntax invalid', deduction: 100 })
    } else if (isDisposable) {
      // Disposable/temporary domain = guaranteed bad
      score = 0
      details.penalties.push({ issue: 'Disposable/temporary email domain', deduction: 100 })
    } else {
      // Domain has no MX or fallback A record (checked via DNS lookup later)
      // This will be handled after DNS lookup if available

      // Spammy TLD detection (*.xyz, *.buzz, .top, .tk, .ml, .ga, .cf, .club, .cricket)
      ...
      // MEDIUM-RISK PENALTIES (lower deliverability — remove 20–40 points)
      if (roleMatch) {
        score = Math.max(0, score - 30)
        details.penalties.push({ issue: 'Role-based email (admin@, support@, hr@, etc.)', deduction: 30 })
      }

      // Username looks automated (test, test123, noreply, etc.)
      ...
      // Gibberish or nonsensical patterns (very long, repeated chars, high entropy)
      // Use sliding scale based on length - longer gibberish is more suspicious
      ...
      if (gibberishHeuristics.length > 0) {
        // Sliding scale penalty based on length
        // Shorter gibberish (15-20 chars): light penalty (-10 to -15)
        // Medium gibberish (20-30 chars): moderate penalty (-20 to -25)
        // Long gibberish (30+ chars): heavy penalty (-30 to -40)
        let gibberishPenalty = -10 // Base penalty for gibberish

        if (localPart.length > 30) {
          // Long gibberish: steep penalty
          gibberishPenalty = Math.min(-40, -10 - Math.floor((localPart.length - 30) / 2))
        } else if (localPart.length > 20) {
          // Medium gibberish: moderate penalty scaling with length
          gibberishPenalty = -10 - Math.floor((localPart.length - 20) / 2)
        }
        // For lengths 15-20, keep base penalty of -10
      ...
      // Local part has unusual formatting (excessive dashes, dots, etc.)
      ...
      // Domain is known free-mail provider - Tiered scoring based on trust/reliability
      const domainLower = domain.toLowerCase()

      // Tier 1: Premium Free Providers (Minimal/No Penalty or Bonus)
      if (domainLower === 'gmail.com') {
        // Gmail is the most trusted consumer email provider - no penalty, slight bonus
        score = Math.min(100, score + 5)
        details.bonuses.push({ feature: 'Gmail (trusted provider)', bonus: 5 })
      }
      // Tier 1: Standard Premium Providers (Small Penalty)
      ...
      // Tier 2: Medium-Risk Free Providers (Moderate Penalty)
      ...
      // Tier 3: High-Risk Free Providers (Heavy Penalty)
      ...
      // Non-standard/non-ICANN TLD (lower penalty than high-risk)
    ...
    // BONUSES (Optional — add 5–10 points)
    // These would require DNS TXT record lookups for SPF/DKIM/DMARC
    // For now, we'll skip these bonuses as they require additional API calls
    // They can be added later if DNS infrastructure supports TXT lookups
    ...
    // Keep the actual calculated score (0-100)
    details.deliverabilityScore = score

    // Determine final validity
    const basicValid = details.issues.length === 0
    details.valid = basicValid && (MailChecker ? (isDisposable ? false : mailcheckerValid) : true)
  ...
  return {
    ...
    mailcheckerAvailable: !!MailChecker,
