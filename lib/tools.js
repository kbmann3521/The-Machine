...
function mathEvaluator(text) {
  try {
    ...
    const trimmed = text.trim()
    if (!trimmed) {
      return { expression: text, error: 'Empty expression' }
    }

    // Whitelist of allowed functions for Phase 1
    ...
    // Parse expression to AST - this validates syntax and structure
    const node = mathjs.parse(trimmed)

    // Validate that only allowed functions are used (deterministic safety check)
    ...
    // Deterministic output: ensure consistent numeric representation
    const finalResult = typeof result === 'number' && !isNaN(result) ? result : null

    if (finalResult === null) {
      return { expression: text, error: 'Invalid result type' }
    }

    return { expression: text, result: finalResult, error: null }
  } catch (e) {
    ...
    if (errorMsg.includes('Unexpected')) {
      ...
      errorMsg = `Undefined variable or function: ${errorMsg}`
    } else if (errorMsg.includes('Division by zero')) {
      errorMsg = 'Division by zero'
    }
    return { expression: text, error: errorMsg }
  }
}
