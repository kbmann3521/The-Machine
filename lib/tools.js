...
const { svgOptimizer: svgOptimizerModule, validateSVG, calculateOptimizationStats, generateDiff, formatSVGPretty, formatSVGCompact, analyzeSVGStructure, lintSVG, trackReferences } = require('./tools/svgOptimizer.js')
import {
...
export const TOOLS = {
  ...
  'image-toolkit': {
    name: 'Image Toolkit',
    description: 'Resize images with fine control over dimensions and quality',
    ...
    detailedDescription: {
      overview: 'The Image Toolkit provides powerful image resizing with fine-grained control. Adjust width, height, scale, and quality with interactive sliders to optimize your images.',
      howtouse: [
        'Upload or drag and drop an image into the input area',
        'Adjust width and height sliders to your desired dimensions',
        'Use the scale slider to resize by percentage (10-200%)',
        'Adjust the quality slider for compression control (10-100%)',
        'View the resized image, file sizes, and download or copy as needed',
      ],
      features: [
        'Width and height sliders for precise dimension control',
        'Scale percentage slider for proportional resizing',
        'Quality slider for compression optimization',
        'Real-time file size calculation and display',
        'Auto-detection of original image dimensions',
        'Support for common image formats',
        'HTML code snippet generation',
      ],
      usecases: [
        ...
        'Preparing images for responsive design',
      ],
    },
  },
  'base64-converter': {
...
function textAnalyzer(text, analyzeType) {
  ...
  if (analyzeType === 'stats' || analyzeType === 'both') {
    ...
    const minSentenceLength = Math.min(...sentenceWordCounts, 0)
    ...
    const minWordLength = Math.min(...wordLengths, 0)
...
function xmlFormatter(text, config) {
  ...
  try {
    ...
    if (mode === 'beautify' || mode === 'minify') {
      ...
      switch (mode) {
        case 'to-json':
          convertedResult = xmlToJson(xmlForConversion, { preserveXmlStructure: config?.preserveXmlStructure === true })
          break
        case 'to-yaml':
          convertedResult = xmlToYaml(xmlForConversion, config?.preserveXmlStructure === true ? 'lossless' : 'lossy')
          break
        case 'to-toml':
          convertedResult = xmlToToml(xmlForConversion, { preserveXmlStructure: config?.preserveXmlStructure === true })
          break
        case 'xpath':
...
function xmlFormatterLegacy(text, config) {
  ...
  try {
    ...
    if (mode === 'beautify' || mode === 'minify') {
      ...
      switch (mode) {
        case 'to-json':
          modeResult = xmlToJson(cleanXml, { preserveXmlStructure: config?.preserveXmlStructure === true })
          break
        case 'to-yaml':
          modeResult = xmlToYaml(cleanXml, config?.preserveXmlStructure === true ? 'lossless' : 'lossy')
          break
        case 'to-toml':
          modeResult = xmlToToml(cleanXml, { preserveXmlStructure: config?.preserveXmlStructure === true })
          break
        case 'xpath':
...
function transformXmlToHumanFriendlyJson(obj) {
  // Reuse the same transformation logic as YAML (it's format-agnostic)
...
function transformXmlToHumanFriendlyYaml(obj, parentKey = null, isArrayElement = false) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    // Note: value can be string, number, or boolean depending on parseTagValue setting
    if (key === '#text') {
      if (typeof value === 'string') {
        textContent = value
        // FIX #1: Detect CDATA by content, not key name
        // CDATA often arrives as #text with HTML/XML markup
        // Check for HTML entities or XML markup to identify CDATA
    ...
    // CDATA should always be string, but handle edge cases
    if (key === '#cdata') {
      if (typeof value === 'string') {
        textContent = value
      } else {
    ...
    if (Array.isArray(value)) {
      ...
      // e.g., templates { template: [...] } should become templates [...]
      if (typeof childTransformed === 'object' && !Array.isArray(childTransformed)) {
        ...
        if (childKeys.length === 1) {
          ...
          if (onlyKey === singularOfParent) {
            // FLATTEN: Convert to array if not already (handles both single objects and arrays)
  ...
  if (textContent) {
    ...
    // FIX #4: Short-circuit CDATA before mixed-content logic
    // CDATA is never prose—it always goes straight to content key
    // This ensures indentation is preserved and never flattened
    if (hasCDATA) {
    ...
    if (hasChildElements) {
      ...
      // Keep path as separate field (Option B: Structured API-friendly approach)
      // Message stays clean: just the text content without path manipulation
      result.message = textContent.replace(/\s+/g, ' ').trim()
    } else if (hasAttributes) {
      // Element has ONLY attributes + text (no child elements, no CDATA—already handled above)
      // Preserve text content with semantic naming: 'name' for maintainer, 'value' default
      if (textContent) {
        const textKey = getSemanticTextKey(parentKey, false)
        result[textKey] = normalizeValue(textContent)
      }
    } else {
      // Element is ONLY text (no attributes, no children, no CDATA—already handled above)
      return normalizeValue(textContent)
    }
  }

  // Final cleanup: Remove XML artifact keys that shouldn't be in human-friendly YAML
  // These should have been handled above, but we filter them as a safety net
  delete result['#text']
  delete result['#cdata']
...
// Determine semantic key name based on parent context
...
// Helper: Get plural form of singular key (feature->features, protocol->protocols)
...
// Helper: Get singular form of plural key (features->feature, protocols->protocol)
...
// Helper: Normalize scalar values (handles ISSUE #6: empty strings -> null)
...
// Helper: Sort object keys semantically for human-friendly reading
...
function semanticallySortObject(obj) {
  ...
  // Root-level structure: version → environment → application → features → server → logging → limits → templates → advanced
  // Within sections: identity (name) → config (enabled) → path → descriptive (description)
  const fieldCategories = {
    // ROOT LEVEL - specific priorities for each top-level element
    // 0. Version (appears first in metadata)
    version: { priority: 0, fields: ['version'] },
    // 0.5. Environment (appears second in metadata)
    environment: { priority: 0.5, fields: ['environment'] },
    // 1. Core configuration (application)
    coreConfig: { priority: 1, fields: ['application'] },
    // 1.5. Feature flags (early, before infrastructure)
    features: { priority: 1.5, fields: ['features'] },
    // 2. Server (first infrastructure element)
    server: { priority: 2, fields: ['server'] },
    // 2.3. Logging (after server)
    logging: { priority: 2.3, fields: ['logging'] },
    // 2.6. Limits/constraints
    limits: { priority: 2.6, fields: ['limits', 'thresholds'] },
    // 3. Templates/Content
    templates: { priority: 3, fields: ['templates'] },
    // 3.5. Advanced/Optional features
    advanced: { priority: 3.5, fields: ['advanced', 'experimental', 'debug', 'optional'] },

    // WITHIN SECTIONS - consistent semantic ordering
    // 10. Identity fields first within a section (name, key, id)
    identity: { priority: 10, fields: ['name', 'id', 'key', 'identifier', 'title'] },
    // 11. Configuration/Status fields (enabled, active, level)
    config: { priority: 11, fields: ['enabled', 'disabled', 'active', 'status', 'state', 'type', 'kind', 'level'] },
    // 12. Path/location fields (path, host, port, email, units)
    path: { priority: 12, fields: ['path', 'location', 'url', 'host', 'port', 'email', 'units', 'value'] },
    // 13. Descriptive fields (description, message, content)
    descriptive: { priority: 13, fields: ['description', 'label', 'summary', 'comment', 'doc', 'documentation', 'info', 'details', 'message', 'text', 'content'] },
    // 14. Reference/Relational fields (maintainer, author, owner)
...
// Helper: Normalize CDATA block indentation
// FIX #2: Rebase to column 0 completely
// - Removes ALL leading indentation from XML source
// - YAML handles visual indentation later via block scalar (|-)
// - Never re-indent after normalization
function normalizeCDATAIndentation(cdataText) {
  ...
  // This is critical: YAML will handle the visual indentation
...
function xmlToToml(xmlString, options = {}) {
  try {
    ...
    if (preserveStructure) {
      ...
      // Extract root element (configuration, root, etc.)
