...
function extractNumberFromString(str) {
  ...
  } else if (dotCount > 1 || (dotCount === 1 && commaCount === 0)) {
    // Multiple dots OR single dot with no commas: dots are thousands, keep as is
    numberStr = mantissaPart.replace(/\./g, '').replace(',', '.')
  } else if (commaCount > 1 || (commaCount === 1 && dotCount === 0)) {
    // Multiple commas OR single comma with no dots: commas are thousands
