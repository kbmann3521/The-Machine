...
function isLikelyHeaderRow(firstRow, delimiter) {
  const cells = firstRow.split(delimiter).map(c => c.trim())

  // Check if mostly non-numeric
  const numericCount = cells.filter(c => !isNaN(c) && c !== '').length
  if (numericCount > cells.length * 0.5) return false

  // Check for common header naming patterns
  const validNamePattern = /^[a-zA-Z_][a-zA-Z0-9_\s\-]*$/
  const validHeaders = cells.filter(c => validNamePattern.test(c) && c.length > 0)
  if (validHeaders.length < cells.length * 0.7) return false

  // Check for duplicate headers (unlikely in a real header)
  const uniqueCount = new Set(cells.filter(c => c !== '')).size
  if (uniqueCount < cells.filter(c => c !== '').length * 0.8) return false
