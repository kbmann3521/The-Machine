...
function unitConverter(text, config) {
  ...
  const detectedUnit = detectUnitFromText(text, type)
  if (!detectedUnit) {
    return { error: `Could not detect unit for ${type}` }
  }

  const unitData = typeData.units[detectedUnit]
  if (!unitData) {
    return { error: `Unknown unit: ${detectedUnit}` }
  }
  ...
  for (const [targetUnit, factor] of Object.entries(conversionsTo)) {
    if (typeof factor === 'function') {
      results[targetUnit] = factor(value)
    } else {
      results[targetUnit] = value * factor
    }
  }

  const inputUnitFullName = abbrvToFullName[detectedUnit] || detectedUnit
  const inputUnitFullNamePluralized = pluralizeUnitName(inputUnitFullName, value)

  const pluralizedAbbrToFullName = {}
  for (const [abbr, fullName] of Object.entries(abbrvToFullName)) {
    pluralizedAbbrToFullName[abbr] = fullName
  }

  return {
    input: value,
    inputUnit: detectedUnit,
    inputUnitFull: inputUnitFullName,
    inputUnitFullPluralized: inputUnitFullNamePluralized,
    type: type,
    results: results,
    abbrvToFullName: pluralizedAbbrToFullName,
    pluralizeUnitName: pluralizeUnitName,
    message: `Converted ${value} ${detectedUnit}`
  }
}
