...
export const TOOLS = {
  ...
  'barcode-generator': {
    ...
    description: 'Free, deterministic 1D barcode generation (UPC-A) from numeric input with customizable size and format',
    toolGroup: 'Media & Visual',
    category: 'generator',
    inputTypes: ['text'],
    outputType: 'json',
    example: '012345678901',
    configSchema: [
      { id: 'format', label: 'Format', type: 'select', default: 'upca', options: [
        {value: 'upca', label: 'UPC-A'},
      ]},
      { id: 'width', label: 'Bar Width (px)', type: 'number', default: 2, min: 1, max: 10 },
      { id: 'height', label: 'Height (px)', type: 'number', default: 50, min: 20, max: 500 },
      { id: 'includeText', label: 'Include Text', type: 'toggle', default: true },
      { id: 'ignoreLeadingZeros', label: 'Ignore Extra Leading Zeros', type: 'toggle', default: true, description: 'Automatically trim extra zeros to find a valid 11 or 12 digit UPC-A code' }
    ],
    show_in_recommendations: true,
    detailedDescription: {
      overview: 'The Barcode Generator creates scannable 1D barcodes from numeric input or uploaded spreadsheets. Currently supports UPC-A format. You can enter codes manually (one per line) or upload a CSV/XLSX file to generate barcodes in bulk with attached metadata. Generates clean SVG vector output for high-quality printing and display.',
      howtouse: ['Enter the 11 or 12 digit UPC-A code (one per line) or upload a CSV/XLSX file', 'If using a spreadsheet, select the column containing the barcode/UPC', 'Adjust the bar width and total height', 'Toggle whether to include human-readable text below the barcode', 'View the generated barcodes with their associated row data in the output panel', 'Download or copy the SVG output'],
      features: ['Generate UPC-A barcodes from numeric input or spreadsheets (CSV, XLSX)', 'Support for multiple codes and bulk processing', 'Automatic column detection for barcode data in files', 'Display row information alongside each generated barcode', 'Customizable bar width and total height', 'Optional human-readable text', 'High-quality SVG vector output', 'Automatic checksum calculation and validation'],
      usecases: ['Generate barcodes for product labels', 'Create scannable codes for inventory management', 'Retail packaging design', 'Asset tracking and logistics', 'Educational demonstrations of barcode formats'],
    },
  },
  'json-formatter': {
    name: 'JSON Formatter',
...
export async function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'barcode-generator': {
      const { barcodeGenerator } = await import('./tools/barcodeGenerator.js')
      return await barcodeGenerator(inputText, config)
    }
    default:
