...
const { svgOptimizer: svgOptimizerModule, validateSVG, calculateOptimizationStats, generateDiff, formatSVGPretty, formatSVGCompact } = require('./tools/svgOptimizer.js')
import {
  ...
  if (mode === 'unused-only') {
    ...
    definedIDs.forEach((id) => {
      if (!usedIDs.has(id)) {
        ...
        // Remove the id attribute itself, handling various spacing patterns
        // Pattern matches: optional-space + id="value" + optional-space
        // But preserve at least one space between attributes if needed
        const beforeReplace = cleaned
        cleaned = cleaned.replace(new RegExp(`\\s+id="${escapedID}"`, 'g'), '')
        // If nothing was replaced, try without requiring preceding space
        if (cleaned === beforeReplace) {
          cleaned = cleaned.replace(new RegExp(`id="${escapedID}"\\s*`, 'g'), '')
        }
        removedCount++
      }
    })
...
// Helper: Remove empty group elements
function removeEmptyGroupsFromSvg(svg) {
  ...
  try {
    const before = result.length
    const emptyGroupPattern = /<g\b[^>]*>\s*<\/g>/gi
    let match
    const emptyGroups = []

    while ((match = emptyGroupPattern.exec(svg)) !== null) {
      emptyGroups.push(match[0])
      emptyGroupsRemoved++
    }

    result = result.replace(emptyGroupPattern, '')
    bytesRemoved = before - result.length
  } catch (error) {
    // Silently fail
  }

  return { result, bytesRemoved, removedCount: emptyGroupsRemoved }
}
