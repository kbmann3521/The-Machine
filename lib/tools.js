...
function smartJoinWords(text) {
  ...
  // Only targets actual fragmented sequences, not normal text
  let result = text
  ...
  result = result
    .split('\n')
    .map(line => {
      // Strategy: Find and fix only sequences that are clearly fragmented
      // A fragmented sequence is: a series of 1-2 char segments separated by spaces
      // Example: "T h i s" or "c a m e" but NOT "This is text"

      // Match pattern: sequences of very short segments (1-2 chars) separated by spaces
      // This regex finds: single letter OR two letters, with spaces between multiple such segments
      const fragmentedPattern = /([a-z])\s+(?=[a-z]\s+|[a-z]$)/gi

      // Only process if we find at least 2 fragmented segments in a row
      let processedLine = line

      // More targeted approach: look for sequences of single/double char segments
      // Pattern: word that's 1-2 chars, followed by space, followed by another 1-2 char word
      // But only join if there are MULTIPLE such patterns in a row (indicating OCR fragmentation)

      const words = processedLine.split(/\s+/)
      const isHighlyFragmented = words.filter(w => w.length <= 2).length > words.length * 0.6 && words.length >= 4

      if (isHighlyFragmented) {
        // Only apply joining to sequences of short segments
        // Join only when we have multiple consecutive very short words
        processedLine = processedLine.replace(/\b([a-z])\s+([a-z])\s+([a-z])/gi, (match, g1, g2, g3) => {
          return g1 + g2 + g3
        })

        // Second pass: join remaining pairs of single letters that are obviously fragmented
        // But be conservative: only join if surrounded by other fragmented-looking text
        processedLine = processedLine.replace(/\b([a-z])\s+([a-z])\b(?=\s+[a-z]\s)/gi, '$1$2')
      }
