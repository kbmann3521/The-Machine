  ...
  'http-status-lookup': {
    ...
    detailedDescription: {
      ...
      features: [
        ...
        'Code examples for Node.js, Next.js, FastAPI, and Nginx',
...
export async function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'http-status-lookup':
      // Temporary inline handler
      const code = inputText.trim()
      const statusMap = {
        '200': { code: 200, name: 'OK', category: '2xx Success', description: 'The request succeeded' },
        '404': { code: 404, name: 'Not Found', category: '4xx Client Error', description: 'The requested resource could not be found' },
        '500': { code: 500, name: 'Internal Server Error', category: '5xx Server Error', description: 'The server encountered an error' },
      }
      return statusMap[code] || { error: `Status code ${code} not found` }
