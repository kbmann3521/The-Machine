...
// NOTE: complexity.nodes counts expression structure nodes (Option A approach)
// This ensures same input always produces same complexity regardless of numeric mode
function calculateComplexity(node) {
  ...
  function traverse(n, depth = 0) {
    ...
    // Count meaningful structure nodes only
    // Only count nodes that directly represent expression structure
    ...
    if (Array.isArray(n)) {
      n.forEach(item => traverse(item, isStructuralNode ? depth + 1 : depth))
      return
    }
    ...
    for (const key in n) {
      if (n.hasOwnProperty(key) && typeof n[key] === 'object') {
        const value = n[key]
        if (Array.isArray(value)) {
          value.forEach(item => traverse(item, isStructuralNode ? depth + 1 : depth))
        } else {
          traverse(value, isStructuralNode ? depth + 1 : depth)
        }
      }
    }
  }
