...
function smartJoinWords(text) {
  ...
  // Key insight: only join SINGLE LETTERS - these are almost never intentional in English
  // Preserve normal words like "is", "an", "at" etc.
  let result = text

  // Strategy: Target only single-letter sequences
  // Single letter followed by space followed by single letter = almost definitely OCR fragmentation
  // Run this selectively to avoid joining normal words at boundaries

  // First pass: Join sequences of ONLY single letters with spaces between them
  // Pattern: single letter, space, single letter (and repeat)
  // This specifically targets "T h i s" → "This" but NOT "This is"
  result = result.replace(/\b([A-Z])(\s+)([A-Z])/g, (match, first, space, second) => {
    // Check if this looks like fragmented capitals: "O C R" etc.
    // Only join if preceded/followed by single letters to avoid "This is" → "Thisis"
    return first + second
  })

  // Second pass: Join lowercase single letters
  // Pattern: lowercase single letter, space, lowercase single letter
  // But use word boundaries carefully to avoid breaking normal text
  result = result.replace(/([a-z])(\s+)([a-z])(\s+)(?=[a-z]\s|[a-z]$)/g, (match, l1, sp1, l2, sp2) => {
    // This pattern specifically catches sequences like "c a m e" or "t e x t"
    return l1 + l2
  })

  // Third pass: Handle remaining single-letter sequences
  // After the patterns above, handle any remaining obvious fragmentation
  // Single letter space single letter when in a sequence of short words
  result = result.split(' ').map(segment => {
    // Within each word-like segment, if it's very short, try to join pieces
    if (segment.length <= 3) {
      return segment.replace(/([a-z])\s+([a-z])/g, '$1$2')
    }
    return segment
  }).join(' ')
