    ...
    if (ipToInteger) {
      try {
        if (detectedVersion === 'IPv4' && ipNum) {
          ...
          const integerValue = ipv4.getValue().valueOf()
          results.integer = integerValue
          results.integerHex = '0x' + integerValue.toString(16).toUpperCase().padStart(8, '0')
          results.integerBinary = integerValue.toString(2).padStart(32, '0').replace(/(.{8})/g, '$1.')
        } else if (detectedVersion === 'IPv6' && ipNum) {
          const ipv6 = ipNum.IPv6.fromString(ip)
          const integerValue = ipv6.getValue().valueOf()
          results.integer = integerValue.toString()
          results.integerHex = '0x' + integerValue.toString(16).toUpperCase()
          results.integerBinary = integerValue.toString(2)
        }
      } catch (e) {
