...
function timeNormalizer(text) {
  ...
  if (/^\d+$/.test(trimmed)) {
    ...
    if (timestamp.toString().length === 10) {
      dt = DateTime.fromSeconds(timestamp, { zone: 'system' })
      detectedFormat = 'Unix Timestamp (seconds)'
    } else if (timestamp.toString().length === 13) {
      dt = DateTime.fromMillis(timestamp, { zone: 'system' })
      detectedFormat = 'Unix Timestamp (milliseconds)'
    } else if (timestamp > 0) {
      // Try to parse as seconds if reasonable
      const asSeconds = DateTime.fromSeconds(timestamp, { zone: 'system' })
      if (asSeconds.isValid) {
        dt = asSeconds
        detectedFormat = 'Unix Timestamp (seconds)'
      } else {
        const asMillis = DateTime.fromMillis(timestamp, { zone: 'system' })
        if (asMillis.isValid) {
          dt = asMillis
          detectedFormat = 'Unix Timestamp (milliseconds)'
        }
  ...
  if (!dt) {
    dt = DateTime.fromISO(trimmed, { zone: 'system' })
    if (dt.isValid) {
  ...
  if (!dt || !dt.isValid) {
    ...
    if (ymdPattern.test(trimmed)) {
      dt = DateTime.fromFormat(trimmed, 'yyyy-MM-dd HH:mm:ss', { zone: 'system' })
      if (dt.isValid) {
  ...
  if (!dt || !dt.isValid) {
    ...
    if (dateOnlyPattern.test(trimmed)) {
      dt = DateTime.fromFormat(trimmed, 'yyyy-MM-dd', { zone: 'system' })
      if (dt.isValid) {
  ...
  if (!dt || !dt.isValid) {
    ...
    for (const { format, name } of timePatterns) {
      dt = DateTime.fromFormat(trimmed, format, { zone: 'system' })
      if (dt.isValid) {
        detectedFormat = name
        break
      }
  ...
  // Ensure dt is in system timezone
  if (!dt.zone.isValid || dt.zone.name === 'UTC') {
    dt = dt.setZone('system', { keepLocalTime: false })
  }

  // Generate output
  const iso = dt.toISO()
  const local = dt.toLocaleString(DateTime.DATETIME_MED_WITH_SECONDS)
  const unixSeconds = Math.floor(dt.toMillis() / 1000)
  const unixMillis = dt.toMillis()
