...
function xmlFormatter(text, config) {
  ...
  try {
    ...
    const { processXmlTool, xmlToJson, xmlToYaml, xmlToToml, xmlXPath } = xmlFormatterModule
    ...
    if (mode === 'beautify' || mode === 'minify') {
      ...
      // For conversion modes, use the new pipeline to get cleaned XML, then convert
      const formatOptions = {
        formatMode: 'none',
        indentSize: config.indentSize || '2',
        removeDeclaration: config.removeXMLDeclaration || false,
      ...
      // Use cleaned/formatted XML for conversion
...
// Legacy XML formatter code removed - all functionality moved to lib/tools/xmlFormatter.js

function urlToolkit_DELETED(text, config) {
  // BROKEN FUNCTION - DO NOT USE
  // Try block body removed
  try {
    const url = new URL('')

    // URL processing code continues �� validate → cleanup → format
    if (mode === 'beautify' || mode === 'minify') {
      // Continue with URL processing (old XML code removed - now in lib/tools/xmlFormatter.js)
      // All remaining URL analysis code continues below
    }
  } catch (error) {
    return null
  }
...
// All legacy XML formatter code has been removed and moved to lib/tools/xmlFormatter.js
