...
function detectUnitFromText(text, type) {
  ...
  const conversions = {
    ...
    data: { b: 'b', B: 'B', KB: 'KB', MB: 'MB', GB: 'GB', TB: 'TB', PB: 'PB' }
  }

  const typeUnits = conversions[type]
  if (!typeUnits) return null

  // Try exact matches first
  for (const token of tokens) {
    if (typeUnits[token]) return token
  }

  // Try case-insensitive matches
  for (const token of tokens) {
    const lower = token.toLowerCase()
    if (typeUnits[lower]) return lower
  }

  // Try unit name to abbreviation mapping
  for (const token of tokens) {
    const abbr = unitNameToAbbr[token]
    if (abbr && typeUnits[abbr]) return abbr
  }
  ...
  for (const token of tokens) {
    ...
    for (const unitAbbr of Object.keys(typeUnits)) {
      if (unitAbbr.length < 2) continue

      const distance = levenshteinDistance(token, unitAbbr)
      if (distance <= 2 && distance < bestMatch.distance) {
        bestMatch = { unit: unitAbbr, distance }
      }
    }

    if (bestMatch.unit) return bestMatch.unit
  }
