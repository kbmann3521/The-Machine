...
function removeJsComments(code) {
  ...
  result = result.replace(/\/\*[\s\S]*?\*\//g, '')
  // Remove inline comments that are part of statements
  result = result.replace(/\/\*[\s\S]*?\*\//g, '')
  return result
}

function removeJsConsole(code) {
  // Remove console.log/warn/error/info statements (entire line)
  let result = code.replace(/^\s*console\.(log|warn|error|info|debug|trace)\([^)]*\);?\s*$/gm, '')
  // Also handle multi-line console statements
  result = result.replace(/console\.(log|warn|error|info|debug|trace)\([^)]*(?:\([^)]*\))*[^)]*\);?/g, '')
  return result
}
...
function jsFormatter(text, config = {}) {
  ...
  try {
    ...
    switch (mode) {
      case 'format': {
        // For format, only remove console/comments if requested (happens after format)
        let formatted = formatJavaScript(trimmedText, config)
        if (removeComments) {
        ...
        if (showLinting) {
          result.linting = lintJavaScript(formatted)
        }
        break
      }

      case 'minify': {
        // For minify, apply comment/console removal first, then minify
        let preprocessed = trimmedText
        if (removeComments) {
          preprocessed = removeJsComments(preprocessed)
        }
        if (removeConsole) {
          preprocessed = removeJsConsole(preprocessed)
        }
        result.minified = minifyJavaScript(preprocessed, { compressCode })
        if (showErrors) {
          result.errors = detectSyntaxErrors(trimmedText)
        }
        if (showAnalysis) {
        ...
        if (showLinting) {
          result.linting = lintJavaScript(result.minified)
        }
        break
      }

      case 'obfuscate': {
        // For obfuscate, apply comment/console removal first
        let preprocessed = trimmedText
        if (removeComments) {
          preprocessed = removeJsComments(preprocessed)
        }
        ...
        result.obfuscated = obfuscateJavaScript(preprocessed)
        if (showErrors) {
          result.errors = detectSyntaxErrors(trimmedText)
        }
        if (showLinting) {
          result.linting = lintJavaScript(result.obfuscated)
        }
        break
      }
...
async function lintJavaScript(code) {
  ...
  try {
    ...
    const messages = linter.verify(code, { rules }, { filename: 'input.js' })
