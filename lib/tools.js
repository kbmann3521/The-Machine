...
function emailValidator(text) {
  ...
  const results = emails.map((email) => {
    ...
    const vowelsInLocal = (localPart.match(vowelPattern) || []).length
    ...
    const vowelPattern = /[aeiou]/gi
    const vowelCount = (domain.match(vowelPattern) || []).length
    const vowelRatio = vowelCount / domain.replace(/\./g, '').length
    if (vowelRatio < 0.15) {
    ...
    if (details.issues.length > 0) {
      ...
      if (gibberishHeuristics.length > 0) {
        ...
        details.penalties.push({ issue: 'Username contains gibberish or nonsensical patterns', deduction: 35 })
      }
      ...
      // Domain is known free-mail provider (gmail, yahoo, outlook) - optional, light penalty
      const freeMailProviders = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com', 'aol.com', 'mail.com', 'protonmail.com']
      if (freeMailProviders.includes(domain.toLowerCase())) {
        score = Math.max(0, score - 10)
        details.penalties.push({ issue: 'Known free-mail provider', deduction: 10 })
      }
      ...
      // Local part starts or ends with number
      if (/^\d/.test(localPart) || /\d$/.test(localPart)) {
        score = Math.max(0, score - 5)
        details.penalties.push({ issue: 'Username starts/ends with number', deduction: 5 })
      }
