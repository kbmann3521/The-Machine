...
function numberFormatter(text, config) {
  ...
  if (typeof sanitizedInput === 'object' && Array.isArray(sanitizedInput)) {
    ...
    return {
      type: 'bulk',
      results: sanitizedInput.map(num => {
...
function extractNumberFromString(str) {
  ...
  // Remove commas and periods that are likely thousand separators
  // Keep track of last occurrence of . or ,
  const lastDot = str.lastIndexOf('.')
  const lastComma = str.lastIndexOf(',')

  let numberStr = str
  if (lastDot > lastComma) {
    // Likely decimal separator is period
    numberStr = str.replace(/,/g, '')
  } else if (lastComma > lastDot) {
    // Likely decimal separator is comma
    numberStr = str.replace(/\./g, '').replace(',', '.')
  } else {
    // Remove both if ambiguous
    numberStr = str.replace(/[,.]/g, '')
  }
