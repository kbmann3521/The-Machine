...
function cleanupIDs(svg, mode = 'preserve', safetyFlags = {}, idGraph = null) {
  ...
  if (mode === 'unused-only') {
    ...
    // CRITICAL SEQUENCE:
    // 1. Build mapping
    // 2. Rewrite ALL references FIRST (before removing anything)
    // 3. Then rewrite definitions
    // 4. Then remove unused IDs if any
    // 5. Verify zero broken references
    ...
    Object.entries(idMap).forEach(([oldID, newID]) => {
      ...
      // Rewrite references in url(), xlink:href, href, aria-labelledby, aria-describedby
      cleaned = cleaned.replace(new RegExp(`url\\(#${escapedID}\\)`, 'g'), `url(#${newID})`)
      cleaned = cleaned.replace(new RegExp(`xlink:href="#${escapedID}"`, 'g'), `xlink:href="#${newID}"`)
      cleaned = cleaned.replace(new RegExp(`href="#${escapedID}"`, 'g'), `href="#${newID}"`)
      cleaned = cleaned.replace(new RegExp(`aria-labelledby="${escapedID}"`, 'g'), `aria-labelledby="${newID}"`)
      cleaned = cleaned.replace(new RegExp(`aria-describedby="${escapedID}"`, 'g'), `aria-describedby="${newID}"`)
    })
    ...
    // Step 4: Verify zero broken references (safety check)
    const verifyGraph = buildIdGraph(cleaned)
    if (verifyGraph.unused.size > 0) {
      // This should never happen, but log it for debugging
      console.warn('[SVG Optimizer] ID minification created unused IDs (this is a bug):', Array.from(verifyGraph.unused))
    }
  }
