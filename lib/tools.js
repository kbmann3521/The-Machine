...
function quickFixCommonErrors(code) {
  let fixed = code
  let wasFixed = false
...
async function autoRepairJavaScript(code) {
  ...
  try {
    let codeToTry = originalCode

    const quickFix = quickFixCommonErrors(codeToTry)
    if (quickFix.wasFixed) {
      codeToTry = quickFix.code
    }

    if (prettierModule) {
      try {
        const repaired = await prettierModule.format(codeToTry, { parser: 'babel' })
        if (repaired && repaired !== originalCode) {
          const repairs = getRepairDifferences(originalCode, repaired)
          return {
            code: repaired,
            wasRepaired: true,
            method: quickFix.wasFixed ? 'quick-fix + prettier' : 'prettier',
            repairs: repairs,
          }
        }
      } catch (e) {
    ...
    const babelRepaired = repairWithBabel(codeToTry)
    if (babelRepaired && babelRepaired !== originalCode) {
      const repairs = getRepairDifferences(originalCode, babelRepaired)
      return {
        code: babelRepaired,
        wasRepaired: true,
        method: quickFix.wasFixed ? 'quick-fix + babel-recovery' : 'babel-recovery',
        repairs: repairs,
      }
    }

    if (quickFix.wasFixed && codeToTry !== originalCode) {
      return {
        code: codeToTry,
        wasRepaired: true,
        method: 'quick-fix',
        repairs: getRepairDifferences(originalCode, codeToTry),
      }
    }
