...
function cleanupIDs(svg, mode = 'preserve', safetyFlags = {}, idGraph = null) {
  ...
  if (mode === 'unused-only') {
    ...
    Object.entries(idMap).forEach(([oldID, newID]) => {
      ...
      // Strategy: Replace ALL occurrences of "#oldID" in quoted attributes, and url(#oldID) patterns
      // This is more comprehensive and handles any attribute that references the ID
      // Match: "#oldID" (in any attribute context)
      cleaned = cleaned.replace(new RegExp(`"#${escapedID}"`, 'g'), `"#${newID}"`)
      // Match: url(#oldID) without quotes (common in CSS and SVG attributes)
