...
const { svgOptimizer: svgOptimizerModule, validateSVG, calculateOptimizationStats, generateDiffHighlights } = require('./tools/svgOptimizer.js')
import {
...
function checkSafetyGates(optimization, safetyFlags, config) {
  ...
  switch (optimization) {
    case 'precisionReduction':
      // Precision reduction is now attribute-aware, so it's safe to apply even with text
      // Only block if there are complex text layouts that shouldn't be touched
      if (safetyFlags.hasText && (safetyFlags.usesMasks || safetyFlags.hasFilters)) {
        gating.allowed = false
        gating.reason = 'Text with masks/filters detected: precision reduction may affect complex rendering'
        gating.skipped = true
      }
      break
...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}) {
  ...
  const steps = [
    ...
    { name: 'precisionReduction', fn: () => reducePrecision(currentSvg, config.precisionReduction.decimals, phase0_1Result.safetyFlags) },
  ...
  for (const step of steps) {
    ...
    try {
      ...
      } else if (step.name === 'idCleanup' && result.removedCount === 0 && config.idCleanup?.mode === 'minify') {
