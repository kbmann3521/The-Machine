  ...
  'js-formatter': {
    ...
    detailedDescription: {
      ...
      howtouse: [
        ...
        'Select your desired mode (Format, Minify, Obfuscate, Lint, or Analyze)',
        'Customize formatting options based on your preferences',
        'View formatted output with detailed insights',
        'Copy results or view errors and warnings',
      ],
      features: [
...
export function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'js-formatter':
      return jsFormatter(inputText, config)
...
function validateIPAddress(inputText, config = {}) {
  ...
  try {
    ...
    results.version = detectedVersion

    // Normalize the IP
    if (normalize || ipToInteger) {
      results.normalized = parsedIp.toString()
    }

    // Convert to integer (IPv4 only)
    if (ipToInteger && detectedVersion === 'IPv4') {
      const parts = parsedIp.parts || []
      const integer = (parts[0] << 24) + (parts[1] << 16) + (parts[2] << 8) + parts[3]
      results.integer = integer
      results.integerHex = '0x' + integer.toString(16).toUpperCase()
      results.integerBinary = parts.map(p => p.toString(2).padStart(8, '0')).join('.')
    }
    ...
    results.error = error.message || 'Failed to parse IP address'
    results.isValid = false
    return results
  }
}
...
function jsFormatter(text, config = {}) {
  const {
    mode = 'format',
    indentSize = '2',
    useSemicolons = true,
  ...
  try {
    ...
    switch (mode) {
      case 'format':
        result.formatted = formatJavaScript(trimmedText, config)
        if (showErrors) {
          result.errors = detectSyntaxErrors(trimmedText)
        }
        if (showAnalysis) {
          result.analysis = analyzeJavaScript(trimmedText)
        }
        break

      case 'minify':
        result.minified = minifyJavaScript(trimmedText, { compressCode })
        if (showErrors) {
          result.errors = detectSyntaxErrors(trimmedText)
        }
        break
      ...
      case 'lint':
        result.linting = lintJavaScript(trimmedText)
        if (showErrors) {
          result.errors = detectSyntaxErrors(trimmedText)
        }
        if (showAnalysis) {
      ...
      case 'analyze':
        result.analysis = analyzeJavaScript(trimmedText)
        result.security = scanSecurityIssues(trimmedText)
        if (showErrors) {
          result.errors = detectSyntaxErrors(trimmedText)
        }
        break
...
function formatJavaScript(code, config) {
  if (!prettierModule) {
  ...
  try {
    ...
    const formatted = prettierModule.format(code, options)
...
function minifyJavaScript(code, config = {}) {
  if (!terserModule) {
    return fallbackJsMinify(code)
  }

  try {
    const result = terserModule.minify(code, {
      compress: config.compressCode ? { passes: 2 } : true,
      mangle: true,
...
function lintJavaScript(code) {
  ...
  if (/console\.\w+/g.test(code)) {
    warnings.push({
      level: 'info',
      message: 'Code contains console statements',
  ...
  if (/\bvar\s+\w+/g.test(code)) {
    warnings.push({
      ...
      suggestion: 'Prefer const or let for better scoping',
    })
  }
  ...
  if (/\beval\s*\(/g.test(code)) {
    warnings.push({
      level: 'error',
      message: 'Using eval() - serious security risk',
      suggestion: 'Avoid eval() at all costs',
    })
  }
  ...
  lines.forEach((line, idx) => {
    ...
    if (trimmed && !trimmed.endsWith(';') && !trimmed.endsWith('{') && !trimmed.endsWith('}') && !trimmed.endsWith(',')) {
      if (!/^\s*(\/\/|\/\*|\*|return|break|continue|case)/.test(line)) {
        // Skip comments and control flow keywords
      }
    }
  })
...
export function autoDetectToolConfig(toolId, inputText) {
  ...
  switch (toolId) {
    ...
    case 'sql-formatter': {
      // Auto-detect if SQL is already formatted
      const hasSqlNewlines = /\n/.test(trimmed)
      const hasSqlIndentation = /^\s+/.test(trimmed)
      const language = detectSqlDialect(trimmed)
      const config = {
        language: language || 'postgresql',
        showLint: true,
        showTableAnalysis: true,
      }
      if (hasSqlNewlines || hasSqlIndentation) {
        config.minify = false
      } else {
        config.minify = false
      }
