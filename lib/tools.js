...
function emailValidator(text) {
  ...
  const results = emails.map((email) => {
    ...
    // 8. Calculate Deliverability Score (0-100)
    let score = 100

    // Deductions
    if (details.issues.length > 0) score = 0 // Syntax errors = invalid
    if (isDisposable) score = 0 // Disposable = invalid
    if (hasBadReputation) score = Math.max(0, score - 25) // Bad reputation
    if (roleMatch) score = Math.max(0, score - 25) // Role-based (warning)
    if (!tldIsIcannApproved && hasValidTld) score = Math.max(0, score - 15) // Non-standard TLD
    if (details.usernameHeuristics?.length > 0) score = Math.max(0, score - (details.usernameHeuristics.length * 5)) // Username heuristics
    if (details.domainHeuristics?.length > 0) score = Math.max(0, score - (details.domainHeuristics.length * 5)) // Domain heuristics

    // Score ranges
    if (score > 85) details.deliverabilityScore = 100 // Fully valid
    else if (score > 60) details.deliverabilityScore = 75 // Valid but with flags
    else if (score > 30) details.deliverabilityScore = 50 // Valid but risky
    else details.deliverabilityScore = 0 // Invalid
