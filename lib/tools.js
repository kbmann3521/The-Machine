...
function xmlBeautify(text, indentSize, config) {
  try {
    const cleanText = config?.removeXMLDeclaration ? text.replace(/<\?xml[^?]*\?>/, '') : text
    ...
    while (i < trimmed.length) {
      ...
      if (!inCDATA && char === '<') {
        if (buffer.trim()) {
          formatted += indentSize.repeat(indent) + buffer.trim() + '\n'
          buffer = ''
        }
        ...
        if (closeIdx !== -1) {
          ...
          if (!tag.startsWith('</') && !tag.endsWith('/>') && !tag.startsWith('<?') && !tag.startsWith('<!')) {
            indent++
          }
    ...
    function xmlToYamlRecursive(str, level = 0) {
      ...
      while ((match = regex.exec(str)) !== null) {
        ...
        if (content) {
          ...
          if (hasChildren) {
            yaml += '\n' + xmlToYamlRecursive(content, level + 1)
          } else {
            yaml += ' "' + content + '"\n'
          }
        } else {
