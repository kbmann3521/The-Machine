...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}, config = {}) {
  ...
  for (const step of steps) {
    ...
    try {
      ...
      // Rebuild ID graph after mutations to keep downstream steps synchronized
      // This ensures removeEmptyGroups, removeUnusedDefs, and idCleanup always use current state
      if (step.name === 'idCleanup' || step.name === 'removeUnusedDefs' || step.name === 'attributeCleanup') {
