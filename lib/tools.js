  ...
  try {
    ...
    while ((match = elementPattern.exec(svg)) !== null) {
      ...
      if (tag) {
        ...
        countedElements.add(tag)
      }
    }

    // Check for specific features
    analysis.hasDefs = /<defs\b/.test(svg)
    analysis.hasText = /<text\b/.test(svg)
    analysis.hasMask = /<mask\b/.test(svg)
    analysis.hasFilter = /<filter\b/.test(svg)
    analysis.hasUse = /<use\b/.test(svg)

    // Extract IDs
    const idPattern = /\bid="([^"]+)"/g
    while ((match = idPattern.exec(svg)) !== null) {
      analysis.ids.push(match[1])
    }

    // Detect fonts used
    const fontPattern = /font-family="([^"]+)"/g
    while ((match = fontPattern.exec(svg)) !== null) {
      const fonts = match[1].split(',').map(f => f.trim())
      fonts.forEach(font => {
        if (!analysis.fonts.includes(font)) {
          analysis.fonts.push(font)
        }
      })
    }

    // Warnings
    if (analysis.hasText && analysis.fonts.length === 0) {
      analysis.warnings.push('Text elements found without font-family definitions')
    }
    if (analysis.ids.length > 20) {
      analysis.warnings.push(`Many IDs detected (${analysis.ids.length}) - verify if all are needed`)
    }
    if (analysis.hasMask && analysis.hasFilter) {
      analysis.warnings.push('Both masks and filters detected - verify performance')
    }
  } catch (error) {
...
function calculateOptimizationStats(originalSvg, optimizedSvg, analysis) {
  const originalSize = new Blob([originalSvg]).size
  const optimizedSize = new Blob([optimizedSvg]).size
  const reductionPercent = originalSize > 0 ? ((originalSize - optimizedSize) / originalSize * 100).toFixed(1) : 0
  ...
  // Detect precision changes (e.g., coordinates reduced)
  const originalCoords = (originalSvg.match(/[\d.]+/g) || []).filter(n => n.includes('.'))
  const optimizedCoords = (optimizedSvg.match(/[\d.]+/g) || []).filter(n => n.includes('.'))
  const precisionReduced = originalCoords.length > optimizedCoords.length

  return {
    originalSize,
    optimizedSize,
    reductionPercent: parseFloat(reductionPercent),
    attributesRemoved: Math.max(0, originalAttrs - optimizedAttrs),
    precisionReduced,
    bytesRemoved: originalSize - optimizedSize
  }
}
...
function generateDiffHighlights(originalSvg, optimizedSvg) {
  ...
  try {
    ...
    // Silently fail on diff generation
  }
...
function svgOptimizer(text) {
  ...
  // Phase 0: Basic optimization (existing logic)
  ...
  const stats = calculateOptimizationStats(originalSvg, optimizedSvg, analysis)
