...
function smartJoinWords(text) {
  ...
  // This detects patterns of single letters/short segments followed by space and joins them
  let result = text
  ...
  result = result
    .split('\n')
    .map(line => {
      // Check if line has the pattern of fragmented words
      // Pattern: capital letter followed by space, then lowercase, then space
      // e.g., "T h i s" or "Th is tex t"

      // Only apply if we detect fragmentation pattern (short segments separated by spaces)
      const words = line.split(/\s+/).filter(w => w.length > 0)

      if (words.length > 1) {
        // Check if this looks like fragmented text
        // Heuristic: average word length < 3 chars and many short segments
        ...
        // If fragmentation is detected (avg < 3 chars, and more than 50% are 1-2 chars)
        if (avgLength < 3 && shortWordRatio > 0.5) {
          // Try to rejoin fragments into words based on capital letters
          let rejoinedWords = []
          let currentWord = ''
          ...
          for (let i = 0; i < words.length; i++) {
            ...
            // If word starts with capital (except first word), it's likely a new word
            if (i > 0 && word.length > 0 && word[0] === word[0].toUpperCase() && word[0] !== word[0].toLowerCase()) {
              if (currentWord.length > 0) {
                rejoinedWords.push(currentWord)
              }
              currentWord = word
            } else {
              currentWord += word
          ...
          // Only use the rejoined version if we actually rejoined something
          if (rejoinedWords.length < words.length) {
            line = rejoinedWords.join(' ')
          }
        }
      }
...
function filterToAsciiWithAccents(text) {
  ...
  // Remove other special Unicode
  let result = text

  // Unicode categories we want to remove (symbols, emojis, control chars)
  // Keep: ASCII letters/numbers, accented Latin letters, whitespace, basic punctuation
  result = result.replace(/[^\x20-\x7E\u00C0-\u00FF\n\t]/g, '')
...
function filterToBasicPunctuation(text) {
  ...
  // Remove everything except: letters, numbers, whitespace, and basic punctuation
