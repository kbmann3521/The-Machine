...
function numberFormatter(text, config) {
  ...
  return formatSingleNumber(sanitizedInput, {
    decimals,
    thousandSeparator,
    decimalSeparator,
    negativeStyle,
...
function extractNumberFromString(str) {
  ...
  // Determine which separator is decimal by looking at position and count
  ...
  // Count digits after the last separator
  const digitsAfterLastSeparator = lastSeparatorPos > -1
    ? mantissaPart.substring(lastSeparatorPos + 1).replace(/\D/g, '').length
    : 0
  ...
  if (lastDot === -1 && lastComma === -1) {
    ...
    // Only commas: if 3 or fewer digits after, it's thousands; else it's decimal
    numberStr = digitsAfterLastSeparator <= 3
      ? mantissaPart.replace(/,/g, '')
      : mantissaPart.replace(',', '.')
  } else if (lastComma === -1) {
    // Only periods: if 3 or fewer digits after, it's thousands; else it's decimal
    numberStr = digitsAfterLastSeparator <= 3
      ? mantissaPart.replace(/\./g, '')
      : mantissaPart
  } else {
    // Both exist: rightmost is decimal
