...
function unitConverterTool(inputText, config = {}) {
  ...
  const { value, rawUnit } = extractValueAndUnit(rawInput)
  const normalizedUnit = normalizeUnit(rawUnit)
  const category = getCategoryForUnit(normalizedUnit)

  if (!category) {
    throw new Error(
      `Unsupported or unknown unit "${rawUnit}". Try something like "100 pounds", "250 cm", or "5 ft".`,
    )
  }

  let base
  try {
    base = mathUnit(`${value} ${normalizedUnit}`)
  } catch (err) {
    throw new Error(`Could not parse "${value} ${normalizedUnit}" as a valid unit.`)
  }
  ...
  const conversions = targetUnits.map(unitName => {
    try {
      const converted = base.to(unitName)
