    ...
    if (config.precision !== null && typeof finalResult === 'number') {
      ...
      if (wasPrecisionRounded) {
        if (config.rounding === 'floor') {
          warnings.push('Result was floored to an integer')
        } else if (config.rounding === 'ceil') {
          warnings.push('Result was ceiled to an integer')
        } else {
          warnings.push(`Result was rounded to ${config.precision} decimal place${config.precision === 1 ? '' : 's'} using ${config.rounding}`)
        }
      }
    }
...
// Phase 4: Engine-agnostic complexity based on expression structure (Option A)
// Same input always produces same complexity, independent of numeric mode (float vs bignumber)
// Counts meaningful AST nodes: literals, variables, operators, functions
function calculateComplexity(node) {
  ...
  function traverse(n, depth = 0) {
    ...
    // Count only semantically meaningful nodes
    // Skip internal mathjs metadata nodes to ensure engine-agnostic consistency
    const isMeaningfulNode =
      n.type === 'ConstantNode' ||
      n.type === 'SymbolNode' ||
      n.type === 'OperatorNode' ||
      n.type === 'FunctionNode' ||
    ...
    if (Array.isArray(n)) {
      n.forEach(item => traverse(item, depth + (isMeaningfulNode ? 1 : 0)))
      return
    }
    ...
    for (const key in n) {
      if (n.hasOwnProperty(key) && typeof n[key] === 'object') {
        const value = n[key]
        if (Array.isArray(value)) {
          value.forEach(item => traverse(item, depth + (isMeaningfulNode ? 1 : 0)))
        } else {
          traverse(value, depth + (isMeaningfulNode ? 1 : 0))
        }
      }
    }
  }
...
function generateStabilityMetadata(config, wasPrecisionRounded, wasNotationApplied, finalResult, floatPrecisionWarnings) {
  ...
  // 1. Rounding actually occurred (wasPrecisionRounded = true)
  // 2. AND a rounding mode other than half-up is active
  // 3. AND precision is defined
  // 4. AND the value is on a half-boundary (half-up/half-even sensitive)
  //    OR the rounding mode is floor/ceil (directional transforms always sensitive when rounding occurs)
  const isSensitiveToRoundingMode =
...
// CONSISTENCY RULE: reductions should only exist if:
...
// This ensures reductions are semantically meaningful and not empty metadata
