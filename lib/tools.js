...
export function getToolExample(toolId, config = {}) {
  switch (toolId) {
    ...
    case 'find-replace': {
      const find = config.find || 'the'
      const caseSensitive = config.caseSensitive
      if (find && find.toLowerCase() !== 'the') {
        const text = `Sample text with "${find}" in it multiple times. Another "${find}" here.`
        return text
      }
    ...
    case 'regex-tester': {
      ...
      if (pattern && pattern.toLowerCase().includes('email')) {
        return 'Contact us at john@example.com or jane@example.com for more info'
      }
      if (pattern && pattern.toLowerCase().includes('url')) {
        return 'Visit https://example.com or http://test.org today'
      }
      return 'The phone numbers are: 555-1234 and 555-5678'
    }
    ...
    case 'timezone-converter': {
      const fromTz = config.fromTimezone || 'UTC'
      const toTz = config.toTimezone || 'America/New_York'
    ...
    case 'keyword-extractor': {
      const minFrequency = parseInt(config.minFrequency) || 1
      if (minFrequency > 1) {
        return 'Python Python Python is great. Python is powerful. Data data analysis is important. Data science data skills are needed.'
      }
      return 'Machine learning and artificial intelligence are transforming industries. Deep learning networks enable new applications in computer vision and natural language processing.'
    }
    ...
    case 'http-header-parser': {
      const hasAuth = config.parseAuth
      if (hasAuth) {
