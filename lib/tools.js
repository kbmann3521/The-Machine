...
function markdownHtmlFormatter(text, config) {
  ...
  if (convertTo === 'html' && isMarkdown) {
    try {
      ...
      throw new Error('marked library not available: ' + e.message)
    }
    ...
    try {
      ...
      throw new Error('turndown library not available: ' + e.message)
    }
  }
  ...
  if (minify) {
    ...
    // Beautify
    if (result.includes('<') && result.includes('>')) {
      // HTML beautification using js-beautify
      try {
        ...
        throw new Error('js-beautify library not available: ' + e.message)
      }
    } else {
      ...
      lines.forEach((line) => {
        ...
        // Preserve blank lines in markdown, but not excessive ones
        if (!trimmed) {
          if (!prevWasEmpty) {
            formatted.push('')
            prevWasEmpty = true
  ...
  return {
    output: result,
    inputType: isHtml && !isMarkdown ? 'html' : isMarkdown && !isHtml ? 'markdown' : 'mixed',
    convertedFrom: convertTo !== 'none' ? (isHtml ? 'html' : 'markdown') : null,
    convertedTo: convertTo !== 'none' ? convertTo : null,
    minified: minify,
  }
}
