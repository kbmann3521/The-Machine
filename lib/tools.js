...
export async function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'email-validator':
      return await emailValidator(inputText)
...
async function emailValidator(text) {
  ...
  const validTLDs = new Set([
    ...
    'shop', 'store', 'cloud', 'tech', 'digital', 'media', 'news', 'tv'
  ])
  ...
  const results = await Promise.all(emails.map(async (email) => {
    ...
    // 7. DNS/MX Record Lookup (async)
    if (details.issues.length === 0 && hasValidTld) {
      try {
        const addresses = await dns.resolveMx(domain)
        if (addresses && addresses.length > 0) {
          details.domainExists = true
          details.mxRecords = addresses.map(record => ({
            priority: record.priority,
            hostname: record.exchange
          }))
        } else {
          details.domainExists = false
          details.issues.push('No MX records found for domain')
        }
      } catch (error) {
        details.domainExists = false
        details.issues.push('Domain does not exist or has no MX records')
      }
    }
    ...
    if (!details.domainExists && hasValidTld) score = Math.max(0, score - 50) // No domain
