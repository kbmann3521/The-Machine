...
function mathEvaluator(text, numericConfig = {}) {
  try {
    ...
    const trimmed = text.trim()
    if (!trimmed) {
      return { expression: text, error: 'Empty expression', diagnostics: { isValid: false, warnings: ['Empty expression'], functionsUsed: [], variables: [], complexity: { nodes: 0, depth: 0 }, numeric: config } }
    }
    ...
    // Phase 3: Apply numeric mode configuration
    const originalConfig = mathjs.config({
      number: config.mode === 'big' ? 'BigNumber' : 'number',
      precision: config.mode === 'big' ? 64 : undefined
    })
