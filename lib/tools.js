...
function mathEvaluator(text, numericConfig = {}) {
  try {
    ...
    const trimmed = text.trim()
    if (!trimmed) {
      return { expression: text, error: 'Empty expression', diagnostics: { isValid: false, warnings: ['Empty expression'], functionsUsed: [], variables: [], complexity: { nodes: 0, depth: 0 }, numeric: { mode: config.mode, precision: config.precision, rounding: config.rounding, notation: config.notation, precisionRounded: false } } }
    }
    ...
    const expressionStructure = buildExpressionStructure(node)
    const reductions = generateReductions(trimmed, finalResult, floatPrecisionWarnings, complexity, config)
    const structureSummary = generateStructureSummary(expressionStructure, complexity, warnings, usedFunctions)
...
// Phase 5: Generate deterministic evaluation trace from AST
// Walks the AST to explain evaluation order and operations
function generateEvaluationTrace(node) {
  const steps = []
  let stepCounter = 0
  ...
  function describeNode(n) {
    ...
    switch (n.type) {
      ...
      case 'FunctionNode':
        return `function ${n.fn.name || n.fn}`
  ...
  function categorizeOperator(op) {
    if (['+', '-', '*', '/'].includes(op)) return 'arithmetic'
    if (['^', '**'].includes(op)) return 'exponentiation'
  ...
  function traverse(n, depth = 0) {
    ...
    if (n.type === 'ConstantNode') {
      ...
      steps.push({
        step: ++stepCounter,
        type: 'symbol',
        description: `Variable: ${n.name}`,
        name: n.name,
        category: 'operand',
        depth: depth
      })
    } else if (n.type === 'OperatorNode') {
      // Traverse operands first
      ...
      steps.push({
        ...
        type: 'operation',
        operator: n.op,
        description: `Operation: ${n.op}`,
        category: categorizeOperator(n.op),
        operandCount: n.args ? n.args.length : 0,
        depth: depth
      })
      ...
      steps.push({
        ...
        category: categorizeFunctionName(fnName),
        argCount: n.args ? n.args.length : 0,
        depth: depth
      })
    } else if (n.type === 'ParenthesisNode') {
      if (n.content) traverse(n.content, depth)
...
function categorizeFunctionName(fn) {
  const trigFunctions = ['sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'sinh', 'cosh', 'tanh']
  const logFunctions = ['log', 'log10', 'log2', 'exp']
  const rootFunctions = ['sqrt', 'cbrt']

  if (trigFunctions.includes(fn)) return 'trigonometric'
  if (logFunctions.includes(fn)) return 'logarithmic'
  if (rootFunctions.includes(fn)) return 'root'
  if (['floor', 'ceil', 'round', 'trunc', 'abs', 'sign'].includes(fn)) return 'rounding'
  if (['min', 'max', 'pow'].includes(fn)) return 'aggregate'
  return 'other'
}

// Generate human-readable evaluation summary
function generateEvaluationSummary(expression, complexity, config) {
  const parts = []

  // Evaluation order
  if (complexity.depth <= 1) {
    parts.push('The expression was evaluated left-to-right following standard operator precedence.')
  } else {
    parts.push(`The expression was evaluated following standard operator precedence with ${complexity.depth} level${complexity.depth === 1 ? '' : 's'} of nesting.`)
  }

  // Numeric method
  if (config.mode === 'bignumber') {
    parts.push('Exact decimal arithmetic was performed using BigNumber precision.')
  } else {
    parts.push('Decimal arithmetic was performed using standard floating-point (IEEE 754) numbers.')
  }

  // Precision note
  if (config.precision !== null) {
    parts.push(`The result was rounded to ${config.precision} decimal place${config.precision === 1 ? '' : 's'} using ${getRoundingModeName(config.rounding)} rounding.`)
  }

  return parts.join(' ')
}

// Helper to get rounding mode name
function getRoundingModeName(mode) {
  const names = {
    'half-up': 'half-up (traditional)',
    'half-even': 'half-even (banker\'s)',
    'floor': 'floor',
    'ceil': 'ceiling'
  }
