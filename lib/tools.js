...
*Last updated: 2024*`,
  ...
  'ip-address-toolkit': {
    ...
    description: 'Validate, convert, analyze IPv4/IPv6, IP ranges, and CIDR subnets with advanced diagnostic tools',
    category: 'network',
    inputTypes: ['text'],
    outputType: 'json',
...
function buildSingleIPResult(ip, rawInput, config) {
  ...
  if (isValidIPv4(ip)) {
    ...
    // Special case: IPv4 with leading zeros (e.g., "010.000.000.001")
    // These are syntactically valid but discouraged
    if (ip.includes('.') && !ip.includes(':')) {
      const parts = ip.split('.')
      if (parts.length === 4) {
        // Try normalizing by removing leading zeros
        const normalized = parts.map(part => {
          const num = parseInt(part, 10)
          return !Number.isNaN(num) && num >= 0 && num <= 255 ? num.toString() : null
        })

        if (normalized.every(p => p !== null)) {
          // The address is valid except for leading zeros
          const normalizedIP = normalized.join('.')
    ...
    // If still not determined, fallback to format detection
    if (!isValid) {
  ...
  // Include diagnostics (for both invalid and valid IPs with warnings)
  let errorData = null
  if (isValid && version === 4) {
    // Valid IPv4 - check for warnings like leading zeros
    errorData = detectIPv4Errors(ip)
  } else if (!isValid) {
    // Invalid IP - get errors
    if (ip.includes(':')) {
      errorData = detectIPv6Errors(ip)
    } else {
      errorData = detectIPv4Errors(ip)
    }
  }

  // Include diagnostics if there are any issues
  if (errorData) {
    const hasIssues = (errorData.errors && errorData.errors.length > 0) ||
                      (errorData.warnings && errorData.warnings.length > 0) ||
                      (errorData.tips && errorData.tips.length > 0)
