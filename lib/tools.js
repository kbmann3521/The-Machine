...
function mathEvaluator(text) {
  try {
    ...
    const trimmed = text.trim()
    if (!trimmed) {
      return { expression: text, error: 'Empty expression', diagnostics: { isValid: false, warnings: ['Empty expression'], functionsUsed: [], variables: [], complexity: { nodes: 0, depth: 0 } } }
    }
    ...
    const implicitMultWarnings = detectImplicitMultiplication(trimmed)

    // Parse expression to AST - validates syntax
    let node = null
    try {
      node = mathjs.parse(trimmed)
    } catch (parseError) {
    ...
    // Extract functions and variables for diagnostics
    ...
    // If unknown functions, still return with error but include diagnostics
    if (unknownFunctions.length > 0) {
      return {
        expression: text,
    ...
    const scope = createRestrictedScope(allowedFunctions, mathjs)
    let result = null
    let hasUndefinedVariables = false
    try {
      result = compiled.evaluate(scope)
    } catch (evalError) {
      let errorMsg = evalError.message || 'Evaluation error'

      // Check for undefined variables
      if (errorMsg.includes('not defined') || errorMsg.includes('is undefined')) {
        hasUndefinedVariables = true
        // Extract variable name if possible
        const varMatch = errorMsg.match(/([a-zA-Z_][a-zA-Z0-9_]*) is not defined|Unknown symbol '([^']*)'/)
        if (varMatch) {
        ...
        // Return error with diagnostics instead of result
        return {
          expression: text,
          error: errorMsg,
          diagnostics: {
      ...
      // Check for division by zero - improved detection
    ...
    const finalResult = typeof result === 'number' && !isNaN(result) && isFinite(result) ? result : null
...
function extractFunctionNames(node) {
  ...
  function traverse(n) {
    ...
    // Traverse all properties that might contain nodes
    if (n.args && Array.isArray(n.args)) {
      n.args.forEach(arg => traverse(arg))
    }
    if (n.expr) traverse(n.expr)
...
function detectImplicitMultiplication(input) {
  const warnings = []

  // Pattern: digit followed by letter (e.g., 2x, 3a)
  if (/\d+[a-zA-Z]/.test(input)) {
    warnings.push("Expression appears to use implicit multiplication (e.g., '2x'). Explicit '*' is required.")
  }

  // Pattern: digit followed by opening paren (e.g., 2(, 3()
  if (/\d+\(/.test(input)) {
    warnings.push("Expression appears to use implicit multiplication (e.g., '2(4+1)'). Explicit '*' is required.")
  }

  // Pattern: closing paren followed by opening paren (e.g., (a)(b))
  if (/\)\s*\(/.test(input)) {
    warnings.push("Expression appears to use implicit multiplication (e.g., '(a)(b)'). Explicit '*' is required.")
  }

  // Pattern: function name followed by digit or paren without explicit mult (e.g., sqrt(4)2 or sin2)
  if (/[a-zA-Z]+\([^)]*\)\d+/.test(input) || /[a-zA-Z]+\s+\(/.test(input)) {
    warnings.push("Expression appears to use implicit multiplication. Explicit '*' is required.")
  }

  // Pattern: closing paren followed by digit (e.g., (4)2)
  if (/\)\d+/.test(input)) {
    warnings.push("Expression appears to use implicit multiplication (e.g., '(4)2'). Explicit '*' is required.")
  }
...
function calculateComplexity(node) {
  ...
  function traverse(n, depth = 0) {
    ...
    if (n.expr) traverse(n.expr, depth + 1)
    if (n.left) traverse(n.left, depth + 1)
    if (n.right) traverse(n.right, depth + 1)
    if (n.condition) traverse(n.condition, depth + 1)
    if (n.consequent) traverse(n.consequent, depth + 1)
    if (n.alternate) traverse(n.alternate, depth + 1)
  }
...
export { validateIPAddress, timeNormalizer }
