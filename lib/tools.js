...
function emailValidator(text) {
  ...
  const results = emails.map(email => {
    ...
    // 1. Basic syntax validation - check for required components
    const basicRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    if (!basicRegex.test(trimmedEmail)) {
      details.issues.push('Invalid email format: missing @ or domain components')
      return details
    }
    ...
    // 3. Check for spaces and invalid characters
    ...
    if (MailChecker) {
      try {
        ...
        // Determine if it's flagged as disposable/bad reputation
        // mailchecker returns false for disposable and bad domains
        if (!mailcheckerValid && details.issues.length === 0) {
          // If mailchecker says invalid but we passed syntax checks,
          // it's likely disposable or bad reputation
          const isKnownDisposable = [
            'mailinator', 'yopmail', '10minutemail', 'temp-mail',
            'guerrillamail', 'anonymousemail', 'cock.li', 'tempmail',
            'throwaway', 'mailnesia', '10min', 'guerrillamail'
          ].some(disposable => domain.includes(disposable))

          if (isKnownDisposable) {
            isDisposable = true
            details.issues.push('Disposable/temporary email domain detected')
          } else {
            hasBadReputation = true
            details.issues.push('Domain has poor reputation or is on blocklist')
          }
        }
      } catch (error) {
        // Fallback if mailchecker fails
        mailcheckerValid = false
      }
    }

    // Determine final validity
    // Valid if: basic syntax passes, no format issues, and (mailchecker OK or mailchecker unavailable)
    const basicValid = details.issues.length === 0 && basicRegex.test(trimmedEmail)
    details.valid = basicValid && (MailChecker ? mailcheckerValid : true)

    // Add metadata flags
    if (isDisposable) details.isDisposable = true
    if (hasBadReputation) details.hasBadReputation = true
    if (roleMatch) details.roleBasedEmail = true
