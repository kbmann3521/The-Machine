  ...
  if (mode === 'unused-only') {
    ...
    definedIDs.forEach((id) => {
      if (!usedIDs.has(id)) {
        ...
        // Remove id attribute - handle both: " id="value"" and "id="value""
        // First try with preceding space
        let before = cleaned.length
        cleaned = cleaned.replace(new RegExp(`\\s+id="${escapedID}"`, 'g'), '')
        // If nothing changed, try without requiring preceding space
        if (cleaned.length === before) {
          cleaned = cleaned.replace(new RegExp(`id="${escapedID}"`, 'g'), '')
        }
        removedCount++
      }
    })
