...
function xmlFormatter(text, config) {
  ...
  try {
    ...
    if (mode === 'beautify' || mode === 'minify' || mode === 'clean') {
      // Step 1: Auto-repair using tolerant parsing (always attempt, even if valid)
      ...
      // Step 4: Beautify, minify, or clean using pretty-data
      switch (mode) {
        case 'beautify':
      ...
      // For conversion modes, attempt auto-repair then cleanup and convert
      repairResult = autoRepairXml(text)
      if (repairResult.wasRepaired) {
        workingXml = repairResult.xml
      }
