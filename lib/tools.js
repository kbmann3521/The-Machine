...
function autoRepairXml(xml) {
  ...
  try {
    ...
    // If already valid, don't attempt repair (avoid corrupting comments/CDATA)
    if (isValid) {
      return { xml: originalXml, wasRepaired: false, method: null, repairs: [] }
    }
    ...
    // Find all tag-like structures and fix incomplete ones
    // Match: < followed by optional /, word chars, optional attributes, up to next < or end of string
    const lines = preprocessed.split('\n')
    for (let i = 0; i < lines.length; i++) {
      let line = lines[i]

      // Fix opening/closing tags missing closing >
      // Look for < followed by content that doesn't end with > or />
      let fixed = line.replace(/<([^<]*?)(?<![>/])(\s*)$/g, (match, content, ws) => {
        // If line doesn't have opening < before this position, it's incomplete
        if (!content.includes('>')) {
          return '<' + content.trim() + '>' + ws
        }
        return match
      })
