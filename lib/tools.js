...
function urlToolkit(text, config) {
  ...
  try {
    ...
    try {
      ...
      if (normalizeUrl) {
        try {
          result.normalization.safe = normalizeUrl(trimmed, {
            stripProtocol: false,
            stripWWW: false,
      ...
      if (normalizeUrl) {
        try {
          result.normalization.aggressive = normalizeUrl(trimmed, {
            stripProtocol: false,
            stripWWW: true,
            removeQueryParameters: [/^utm_/, /^fbclid$/, /^gclid$/, /^msclkid$/, /^mc_eid$/],
      ...
      // Fallback: Basic normalization if normalize-url not available
      if (!normalizeUrl) {
