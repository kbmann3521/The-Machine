...
const { svgOptimizer: svgOptimizerModule, validateSVG, calculateOptimizationStats, generateDiffHighlights, formatSVGPretty, formatSVGCompact } = require('./tools/svgOptimizer.js')
import {
...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}, config = {}) {
  ...
  let finalOutputSvg = currentSvg

  if (outputFormat === 'pretty') {
    const { formatSVGPretty } = require('./tools/svgOptimizer.js')
    finalOutputSvg = formatSVGPretty(currentSvg)
  } else {
    const { formatSVGCompact } = require('./tools/svgOptimizer.js')
