...
export const TOOLS = {
  ...
  'xml-formatter': {
    ...
    configSchema: [
      ...
      {
        id: 'trimWhitespace',
        label: 'Trim Whitespace Between Tags',
        type: 'toggle',
        default: false,
      },
      {
...
function xmlFormatter(text, config) {
  ...
  try {
    ...
    // Handle beautify/minify/clean with the new pipeline
    if (mode === 'beautify' || mode === 'minify' || mode === 'clean') {
      ...
      const formatOptions = {
        formatMode: mode === 'clean' ? 'none' : mode,
        ...
        trimWhitespaceBetweenTags: config.trimWhitespace || false,
        indentSize: config.indentSize || '2',         // 👈 NEW
      ...
      // Include repair info if it was performed
      if (pipelineResult.repairInfo) {
        result.repairInfo = {
          wasRepaired: pipelineResult.repairInfo.wasRepaired,
          originalXml: pipelineResult.repairInfo.originalXml,
          repairedXml: pipelineResult.repairInfo.repairedXml,
          diff: pipelineResult.repairInfo.diff,
          summary: pipelineResult.repairInfo.summary,
        }
      }

      if (showLinting && pipelineResult.lintWarnings.length > 0) {
        result.linting = {
          warnings: pipelineResult.lintWarnings.map((msg) => ({
            message: msg,
            line: 0,
          })),
          total: pipelineResult.lintWarnings.length,
        }
      }
      ...
      // For conversion modes, use legacy implementation
      return xmlFormatterLegacy(text, config)
    }
  } catch (error) {
    return { error: error.message }
