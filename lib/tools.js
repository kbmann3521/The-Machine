  ...
  try {
    ...
    if (config.precision !== null && typeof finalResult === 'number') {
      ...
      if (wasPrecisionRounded) {
        const placeWord = config.precision === 1 ? 'place' : 'places'
        const roundingMsg = config.rounding === 'floor'
          ? 'Result was floored to an integer'
          : config.rounding === 'ceil'
          ? 'Result was ceiled to an integer'
          : `Result was rounded to ${config.precision} decimal ${placeWord} using ${config.rounding}`
        warnings.push(roundingMsg)
      }
    }
