...
function timeNormalizer(text, config = {}) {
  ...
  // Pattern 1: With explicit date and time+tz (e.g., "2024-01-15 1pm PST" or "Jan 15 2024 1:15am EST")
  const dateTimeTzPattern = /^(\d{4}-\d{2}-\d{2}|\w+\s+\d{1,2}\s+\d{4})\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)\s*([a-z]{2,4})$/
  ...
  if (dateTimeTzMatch && !dt) {
    ...
    if (ianaZone) {
      try {
        let formatStr = dateStr.includes('-') ? 'yyyy-MM-dd' : 'MMM d yyyy'
        const hour24 = parseInt(hour) + (ampm === 'pm' && parseInt(hour) !== 12 ? 12 : 0) - (ampm === 'am' && parseInt(hour) === 12 ? 12 : 0)
        const min = minute ? parseInt(minute) : 0
        const timeStr = `${dateStr} ${String(hour24).padStart(2, '0')}:${String(min).padStart(2, '0')}`
        formatStr = dateStr.includes('-') ? 'yyyy-MM-dd HH:mm' : 'MMM d yyyy HH:mm'
        dt = DateTime.fromFormat(timeStr, formatStr, { zone: ianaZone })
        if (dt.isValid) {
