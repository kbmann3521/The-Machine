...
function buildSingleIPResult(ip, rawInput, config) {
  ...
  if (config.ptrRecord !== false) {
    const ipForPTR = version === 6 && results.normalizedWithoutZone ? results.normalizedWithoutZone : ip
  ...
  if (config.classification !== false) {
    const ipForClassification = version === 6 && results.normalizedWithoutZone ? results.normalizedWithoutZone : ip
...
function buildCIDRResult(cidrParsed, rawInput, config) {
  ...
  if (isIPv4) {
    ...
    let networkAddress = getNetworkAddress(base, cidr)
    let broadcastAddress = getBroadcastAddress(base, cidr)
    let firstHost = getFirstHost(base, cidr)
    let lastHost = getLastHost(base, cidr)
    ...
    // Special handling for /32: it's a single host, not a network
    if (cidr === 32) {
      // For /32, everything is the IP itself
      networkAddress = base
      broadcastAddress = base
      firstHost = base
      lastHost = base
    }
    ...
    // Convert to integers (should never be null after above handling)
