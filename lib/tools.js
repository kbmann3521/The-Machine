...
function urlToolkit(text, config) {
  // Fix only literal "\n<word>" sequences before removing backslashes
  let cleanedText = text
    .replace(/\\n([a-zA-Z0-9_-]+)/g, '$1')  // Fix escaped newline keys: \nkey → key
    .replace(/\\/g, '')                     // Remove all remaining backslashes
    .replace(/\n+/g, '')                    // Remove all real newlines
    .trim()
...
function analyzeQueryParamDeepDive(url) {
  ...
  try {
    ...
    for (const [rawKey, value] of params) {
      // Clean the key: trim, remove leading/trailing newlines, and strip leftover "n" prefix from backslash removal
      const cleanKey = rawKey
        .trim()
        .replace(/^\n+|\n+$/g, '')
        .replace(/^n(?=[a-zA-Z0-9_-])/, '')  // Remove leading "n" only if followed by alphanumeric, underscore, or hyphen
...
function analyzeParamKey(key, values) {
  // Clean the key: trim, remove any leading/trailing newlines, and strip leftover "n" prefix from backslash removal
