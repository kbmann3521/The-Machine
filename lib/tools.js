...
function asciiUnicodeConverter(text, config) {
  if (config.mode === 'toCode') {
    const conversions = text.split('').map(c => ({ char: c, code: c.charCodeAt(0) }))
    const compiledCodes = conversions.map(c => c.code).join(' ')
    return {
      summary: compiledCodes,
  ...
  try {
    ...
    if (codes.length === 0) {
      return {
        summary: '',
        conversions: [],
        message: 'No numeric codes found'
      }
    }
    const result = codes.map(c => String.fromCharCode(parseInt(c))).join('')
    return {
      summary: result,
      conversions: codes.map((code, idx) => ({ code: parseInt(code), char: String.fromCharCode(parseInt(code)) }))
    }
  } catch (e) {
    return {
      summary: '',
...
export function getToolExample(toolId, config = {}) {
  switch (toolId) {
    ...
    case 'base64-converter': {
      ...
      return mode === 'encode' ? 'Hello World' : 'SGVsbG8gV29ybGQ='
    }
    case 'csv-json-converter':
      return 'name,age\nJohn,30\nJane,25'
    case 'timestamp-converter':
    ...
    case 'css-formatter':
      return '.container{color:red;padding:10px;}'
    case 'xml-formatter':
      return '<root><item>test</item></root>'
    case 'js-formatter':
      return 'const x={a:1,b:2}'
    case 'binary-converter':
      return '255'
    case 'color-converter':
      return '#FF5733'
    case 'regex-tester':
      return 'test123test456'
    case 'yaml-formatter':
      return 'server:\n  port: 8080'
    case 'url-toolkit':
      return 'https://example.com:8080/path?id=123#section'
    case 'jwt-decoder':
