...
function detectParameterRisks(paramKey, paramValue, classifications, isRedirectParam) {
  ...
  // SQL injection indicators - use regex library patterns
  if (REGEX_PATTERN_TEMPLATES['sql-injection']) {
    try {
      const sqlRegex = new RegExp(REGEX_PATTERN_TEMPLATES['sql-injection'].pattern, REGEX_PATTERN_TEMPLATES['sql-injection'].flags)
      if (sqlRegex.test(paramValue)) {
        risks.push({
          type: 'SQL Injection Risk',
          severity: 'high',
          description: `Parameter contains potential SQL injection patterns (${REGEX_PATTERN_TEMPLATES['sql-injection'].description})`
        })
      }
    } catch (e) {
      // Skip if pattern fails to compile
    }
  }
