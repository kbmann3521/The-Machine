...
function mathEvaluator(text) {
  try {
    ...
    // Normalize expression: use the trimmed version
...
function extractFunctionNames(node) {
  ...
  function traverse(n) {
    ...
    // Check for FunctionNode type
    if (n.type === 'FunctionNode') {
      // Handle both n.fn.name and direct name property
      const funcName = n.fn && n.fn.name ? n.fn.name : (n.name || null)
      if (funcName) {
    ...
    if (n.object) traverse(n.object)
    if (n.index) traverse(n.index)
  }
...
function detectImplicitMultiplication(input, allowedFunctions = []) {
  ...
  if (/\d+[a-zA-Z]/.test(input) && !functionPattern.test(input)) {
    warnings.push("Implicit multiplication detected. Expression was evaluated as multiplication.")
  }
...
function calculateComplexity(node) {
  ...
  function traverse(n, depth = 0) {
    ...
    // Traverse all child node properties
    // Binary operators
    ...
    // Array/object access
    if (n.object) traverse(n.object, depth + 1)
    if (n.index) traverse(n.index, depth + 1)

    // Assignment and other
    if (n.value) traverse(n.value, depth + 1)
  }
