...
function checkSafetyGates(optimization, safetyFlags, config) {
  ...
  switch (optimization) {
    ...
    // Path merging: ONLY block if shape conversion is disabled (impossible operation)
    case 'pathMerging':
      if (config.shapeConversion?.enabled !== true) {
        gating.allowed = false
        gating.reason = 'Path merging requires shape conversion to be enabled (impossible operation)'
        gating.skipped = true
      }
      break
...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}, config = {}) {
  ...
  if (removeEmptyGroupsIdx >= 0 && removeEmptyGroupsAfterIdIdx >= 0) {
    ...
    // If the first pass removed nothing but the second pass did, replace with combined message
    if ((beforeResult.removedCount === 0 || !beforeResult.executed) && afterResult.executed && afterResult.removedCount > 0) {
      // Remove the first (empty) result
      stepResults.splice(removeEmptyGroupsIdx, 1)

      // Update the second result with combined info
      ...
      if (updatedResult >= 0) {
        ...
        stepResults[updatedResult].message = `${afterResult.removedCount} empty groups removed (after ID cleanup)`
      }
    }
  }
