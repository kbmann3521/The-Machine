...
function autoRepairXml(xml) {
  ...
  try {
    ...
    // Fix tags missing closing > (e.g., <name>John</name becomes <name>John</name>)
    preprocessed = preprocessed.replace(/(<\/?\w+[^/>]*)\s*(?=<|$)/g, (match) => {
    ...
    // Fix unclosed tags at end of string
    preprocessed = preprocessed.replace(/(<\w+[^/>]*)\s*$/g, (match) => {
      if (!match.trim().endsWith('>') && !match.trim().endsWith('/')) {
        return match.trim() + '>'
      }
    ...
    try {
      ...
      // If parsing still fails after preprocessing, return original
      return { xml: originalXml, wasRepaired: false, method: null, repairs: [] }
    }
