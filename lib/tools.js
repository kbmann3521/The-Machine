...
function validateIPAddress(inputText, config = {}) {
  ...
  try {
    // Import isHostname from ipInputDetection
    const { isHostname } = require('./ipInputDetection')
...
function buildHostnameResult(hostname, rawInput, config) {
  ...
  const isValid = hostname === 'localhost' || (
    labels.every(label =>
      label.length > 0 &&
      label.length <= 63 &&
    ...
    results.diagnostics = {
      ...
      tips: [
        ...
        },
        {
          ...
          message: 'DNS resolution will be performed to resolve hostname to IP address(es)',
          severity: 'info',
        },
      ],
    }
...
function buildCIDRResult(cidrParsed, rawInput, config) {
  ...
  if (isIPv4) {
    ...
    if (cidr === 32) {
      ...
      // /0 to /30: normal case - first host is network + 1, last host is broadcast - 1
