...
function encodePunycodeLabel(label) {
  ...
  // Copy basic ASCII characters to output
  ...
  while (handledCases < label.length) {
    let nextDelta = Infinity

    // Find next code point
    for (let i = 0; i < label.length; i++) {
      const code = label.charCodeAt(i)
      if (code >= n && code < nextDelta) {
        nextDelta = code
      }
    }

    delta += (nextDelta - n) * (handledCases + 1)
    n = nextDelta
    ...
    for (let i = 0; i < label.length; i++) {
      ...
      if (code < n) {
        ...
        for (let k = BASE; ; k += BASE) {
          ...
          q -= t
          q = Math.floor(q / (BASE - t))
        }

        bias = Math.floor(
          ((handledCases + 1) * DAMP) /
          (Math.floor(delta / 2) + delta)
        )
        delta = 0
        handledCases++
      }
...
function decodePunycodeLabel(label) {
  ...
  const encoded = label.substring(4).toLowerCase()
  let n = INITIAL_N
  let bias = INITIAL_BIAS
  let i = 0
  let output = []
  ...
  if (lastDashIndex > 0) {
    output = encoded.substring(0, lastDashIndex).split('')
  }
  ...
  while (pos < encoded.length) {
    ...
    for (let k = BASE; ; k += BASE) {
      ...
      let code = encoded.charCodeAt(pos++) - 48
      let digit = code < 10 ? code : code < 39 ? code - 39 : code
    ...
    if (n > 1114111) return 'Invalid codepoint'
