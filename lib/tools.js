...
export function getToolExample(toolId, config = {}) {
  switch (toolId) {
    ...
    case 'find-replace': {
      const findText = config.findText || 'hello'
      if (findText && findText.trim()) {
        return `This ${findText} example shows ${findText} text. The ${findText} will be highlighted and replaced.`
      }
      return 'Find and replace this text with your own content here.'
    }
    ...
    case 'timezone-converter': {
      const fromTz = config.fromTimezone || 'UTC'
      const toTz = config.toTimezone || 'EST'
      if (fromTz === 'UTC' && toTz === 'EST') {
        return '2024-01-15 14:30:00 UTC'
      }
      return `2024-01-15 14:30:00 (${fromTz})`
    }
