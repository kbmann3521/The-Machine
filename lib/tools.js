...
function svgOptimizer(text) {
  ...
  const safetyFlags = {
    ...
    hasExternalRefs: analysis.references.idsReferenced.length > 0,
...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}) {
  ...
  const phase2Result = {
    ...
    optimizationResult: failedStep ? 'blocked' : currentSvg === phase0_1Result.optimizedSvg ? 'no_changes' : 'changes_applied',
    blockedReason: failedStep ? blockedReason : null,
    failedStep,
    stats: finalStats,
    diff: finalDiff,
    phase2Applied: !failedStep && currentSvg !== phase0_1Result.optimizedSvg,
    phase2Config: config,
