...
function autoRepairXml(xml) {
  ...
  try {
    ...
    // Check for common repair patterns before attempting parse
    // Look for lines ending with incomplete tags
    const hasIncompleteTag = /<[^<>]*$/m.test(originalXml)

    // First, check if the XML is already valid (try standard strict parsing)
    ...
    // If already valid AND no obvious incomplete tags, skip repair
    if (isValid && !hasIncompleteTag) {
      return { xml: originalXml, wasRepaired: false, method: null, repairs: [] }
    }

    // Pre-process to fix obvious markup issues before tolerant parsing
    let preprocessed = xml

    // Fix incomplete tags line by line
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Check if line ends with an incomplete tag (starts with < but doesn't end with > or />)
      const incompleteTagMatch = trimmed.match(/<[^<>]*$/)
      if (incompleteTagMatch) {
        const tag = incompleteTagMatch[0]
        // Only fix if it's a real tag (has word character or slash after <)
        if (tag.match(/<\/?[\w:]/)) {
          const indent = line.match(/^(\s*)/)[1]
          // Add closing >
          const trailing = line.slice(trimmed.length)
          line = indent + trimmed + '>' + trailing
          hasChanges = true
        }
      }
    ...
    preprocessed = lines.join('\n')

    // Only repair if preprocessing changed something
    if (hasChanges && preprocessed !== originalXml) {
      let repairedXml = preprocessed

      // Try parsing the preprocessed version to ensure it's valid
      try {
        ...
        // If still invalid after preprocessing, return original
        return { xml: originalXml, wasRepaired: false, method: null, repairs: [] }
      }
    }
