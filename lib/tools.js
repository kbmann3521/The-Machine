...
function validateCSV(text, delimiter, headerRowIndex) {
  ...
  rowsWithMismatch.forEach(mismatch => {
    const diff = mismatch.actual - mismatch.expected
    ...
    warnings.push({
      ...
      severity: severity,
      message: `Row ${mismatch.row + 1} has ${mismatch.actual} columns (expected ${mismatch.expected})`,
      description: `This row has ${direction} columns. This may cause data to shift or be dropped. Check for unquoted delimiters or missing quotes.`,
      row: mismatch.row,
    })
  })

  // Check for invisible characters in headers
  if (headerRowIndex >= 0 && lines[headerRowIndex]) {
    const headerLine = lines[headerRowIndex]
    const headers = headerLine.split(delimiter)
    headers.forEach((h, idx) => {
      ...
      if (h !== trimmed) {
        warnings.push({
          type: 'whitespace',
          severity: 'warning',
          message: `Header ${idx + 1} has leading/trailing whitespace`,
          description: `"${h}" should be "${trimmed}". This can cause column matching issues.`,
          row: headerRowIndex,
          column: idx,
        })
      }
      ...
      if (/[\u200B\u200C\u200D\uFEFF]/.test(h)) {
        warnings.push({
          ...
          description: `Zero-width spaces or other hidden characters detected. These should be removed.`,
          row: headerRowIndex,
          column: idx,
        })
      }
    ...
    if (line.includes('"') && idx < lines.length - 1) {
      ...
      if (quoteCount % 2 !== 0) {
        warnings.push({
          ...
          description: `This field spans multiple lines. Verify this is intentional.`,
          row: idx,
        })
      }
    }
