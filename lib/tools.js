...
function imageToolkit(imageData, config) {
  ...
  if (!imageData) {
    return {
      error: 'Please upload an image. Click the image upload area above to select an image.',
      mode,
    }
  }
  ...
  if (mode === 'resize') {
    return {
      mode: 'resize',
      message: 'Image loaded. Configure resize settings and the image will be resized.',
      imageData: imageData,
      resizeMode: config.resizeMode || 'dimensions',
      width: config.width || 800,
      height: config.height || 600,
