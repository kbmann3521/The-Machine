...
function mathEvaluator(text, numericConfig = {}) {
  try {
    ...
    let expressionStructure, reductions, structureSummary
    try {
      ...
      reductions = []
      structureSummary = { shape: 'Unknown', nestingDepth: 0, nodeCount: 0, functions: [], hasWarnings: warnings.length > 0 }
    }
...
function generateStructureSummary(structure, complexity, warnings, functionsUsed) {
  ...
  const nodeCount = countStructureNodes(structure)
...
// Export new Phase 5 helper functions for use in components if needed
export { calculateNestingDepth, countStructureNodes }
