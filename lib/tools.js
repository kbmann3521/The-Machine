...
export const TOOLS = {
  ...
  'barcode-generator': {
    ...
    show_in_recommendations: true,
    detailedDescription: {
      overview: 'The Barcode Generator creates scannable 1D barcodes from numeric input. Currently supports UPC-A format. Enter one code per line to generate multiple barcodes at once. Generates clean SVG vector output for high-quality printing and display.',
      howtouse: ['Enter the 11 or 12 digit UPC-A code (one per line)', 'Adjust the bar width and total height', 'Toggle whether to include human-readable text below the barcode', 'View the generated barcodes in the output panel', 'Download or copy the SVG output'],
      features: ['Generate UPC-A barcodes from numeric input', 'Support for multiple codes (one per line)', 'Customizable bar width and total height', 'Optional human-readable text', 'High-quality SVG vector output', 'Automatic checksum calculation if 11 digits are provided', 'Validation of input length and characters'],
