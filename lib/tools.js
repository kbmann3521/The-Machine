...
function mathEvaluator(text, numericConfig = {}) {
  try {
    ...
    const stabilityMetadata = generateStabilityMetadata(config, wasPrecisionRounded, wasNotationApplied)
...
function generateStabilityMetadata(config, wasPrecisionRounded, wasNotationApplied) {
  return {
    precisionSensitive: config.precision !== null && wasPrecisionRounded,
    roundingSensitive: config.precision !== null && wasPrecisionRounded && (config.rounding !== 'half-up'),
