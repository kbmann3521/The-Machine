  ...
  if (minify) {
    ...
    if (result.includes('<') && result.includes('>')) {
      ...
      try {
        ...
        // Fallback to simple minification if html-minifier fails
        result = result.replace(/>\s+</g, '><').replace(/\s+/g, ' ').trim()
      }
    } else {
      // Markdown minification - remove extra whitespace
      result = result
        .replace(/\n\n+/g, '\n')
        .replace(/^\s+/m, '')
        .replace(/\s+$/m, '')
    ...
    if (result.includes('<') && result.includes('>')) {
      ...
      // Markdown formatting - simple beautification
      const lines = result.split('\n')
      const formatted = []
