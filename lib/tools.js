...
function analyzeParamKey(key, values) {
  ...
  values.forEach((value, index) => {
    ...
    const risks = detectParameterRisks(key, value, classifications, isRedirectParam, index)
...
function detectYouTubeVideo(value) {
  if (!value || typeof value !== 'string') return null
  const youtubePattern = /(?:v=|youtu\.be\/)([A-Za-z0-9_-]{11})/
  const match = youtubePattern.exec(value)
  if (match && match[1]) {
    return {
      type: 'YouTube Video',
      emoji: '▶️',
      description: `YouTube video ID: ${match[1]}`
    }
  }
  return null
}
