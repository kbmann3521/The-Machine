...
async function jsFormatter(text, config = {}) {
  ...
  try {
    ...
    switch (mode) {
      case 'format': {
        result.formatted = await formatJavaScript(trimmedText, config)
        if (showErrors) {
...
function repairWithEslint(code) {
  if (!eslintModule) {
    return null
  }
  ...
  try {
    ...
    const results = linter.verify(code, {}, { fix: true })

    if (results.length > 0 && results[0].fix) {
      return results[0].fix.range
    }
    return null
  } catch (error) {
    return null
...
async function autoRepairJavaScript(code) {
  ...
  try {
    if (prettierModule) {
      try {
        ...
        if (repaired && repaired !== originalCode) {
          return { code: repaired, wasRepaired: true, method: 'prettier' }
        }
      } catch (e) {
    ...
    if (babelRepaired && babelRepaired !== originalCode) {
      return { code: babelRepaired, wasRepaired: true, method: 'babel-recovery' }
    }

    const eslintRepaired = repairWithEslint(code)
    if (eslintRepaired && eslintRepaired !== originalCode) {
      return { code: eslintRepaired, wasRepaired: true, method: 'eslint-fix' }
    }

    return { code: originalCode, wasRepaired: false, method: null }
  } catch (error) {
    return { code: originalCode, wasRepaired: false, method: null }
  }
}
...
async function formatJavaScript(code, config) {
  if (!prettierModule) {
    return { formatted: fallbackJsFormat(code, config), wasRepaired: false }
  }
  ...
  try {
    ...
    return { formatted: formatted && typeof formatted === 'string' ? formatted : fallbackJsFormat(code, config), wasRepaired: false }
  } catch (error) {
    return { formatted: fallbackJsFormat(code, config), wasRepaired: false }
  }
}
