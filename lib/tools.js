...
function encodePunycodeLabel(label) {
  ...
  const basicCodePoints = codePoints.filter(cp => cp < 128)

  if (basicCodePoints.length === codePoints.length) {
    return label
  }

  let output = ''
  let basicLength = basicCodePoints.length

  if (basicLength > 0) {
    output = label.substring(0, basicLength) + '-'
  }
  ...
  const nonBasicCodePoints = codePoints.filter(cp => cp >= 128).sort((a, b) => a - b)
  const uniqueNonBasic = [...new Set(nonBasicCodePoints)]

  for (const cp of uniqueNonBasic) {
    delta += (cp - n) * (output.length === 0 ? 1 : uniqueNonBasic.indexOf(cp) + 1)
    n = cp
    ...
    for (let i = 0; i < label.length; i++) {
      ...
      if (charCode === n) {
        ...
        bias = Math.floor(delta / (i + 1 === basicLength ? 700 : 1280))
        delta = 0
      }
    }
  }
...
function decodePunycodeLabel(label) {
  ...
  const basicCodePointCount = encoded.lastIndexOf('-')
  let basic = ''
  let output = ''

  if (basicCodePointCount > 0) {
    basic = encoded.substring(0, basicCodePointCount)
    output = basic
  }
  ...
  for (let pos = basicCodePointCount > 0 ? basicCodePointCount + 1 : 0; pos < encoded.length; ) {
    let oldi = i
    let w = 1
    let k = 36
    ...
    while (pos < encoded.length) {
      ...
      const digit = c < 10 ? c : c - 39
      if (digit < 0 || digit >= 36) break
      i += digit * w
      const t = k <= bias ? 1 : k >= bias + 26 ? 26 : k - bias
      if (digit < t) {
        bias = Math.floor(bias + (i - oldi) * 1000 / (k - t + i - oldi))
        w = 1
        k = 36
        i = 0
        oldi = 0
