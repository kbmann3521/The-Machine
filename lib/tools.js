    ...
    case 'text-toolkit':
      const toolkitResult = {
        ...
        cleanText: cleanText(inputText, {
          ...
          removeTimestamps: config?.removeTimestamps || false,
          ...
          flattenToSingleLine: config?.flattenToSingleLine || false,
        }),
...
function cleanText(text, config = {}) {
  ...
  // 6. Remove timestamps from logs (optional)
  ...
  // 9. Handle excessive spacing (tabs, multiple spaces, OCR artifacts)
  if (config.compressSpaces !== false) {
    result = compressExcessiveSpacing(result)
  }

  // 10. Remove excessive line breaks
  if (config.removeBlankLines !== false) {
    result = result
      .split('\n')
  ...
  // 11. Compress consecutive blank lines to max 2
  if (config.compressLineBreaks !== false) {
  ...
  // 13. Flatten to single line (if requested)
  if (config.flattenToSingleLine === true) {
    result = result
      .split('\n')
      .map(line => line.trim())
...
function removeInvisibleUnicodeChars(text) {
  ...
  result = result.replace(/\uFEFF/g, '') // Zero Width No-Break Space

  // Word joiner and other invisible formatting
  result = result.replace(/\u2060/g, '') // Word Joiner
  result = result.replace(/\u061C/g, '') // Arabic Letter Mark
  result = result.replace(/\u180E/g, '') // Mongolian Vowel Separator
...
function removeLogTimestamps(text) {
  ...
  result = result.replace(/\[\d{1,2}:\d{2}(?::\d{2})?\]\s*/gm, '')
  ...
  // Username or speaker prefixes like "Kyle:" or "[Kyle]"
  result = result.replace(/^\[?[A-Za-z_][A-Za-z0-9_]*\]?\s*:\s*/gm, '')
...
function fixPunctuationSpacing(text) {
  ...
  // Ensure space after punctuation (add space if missing)
