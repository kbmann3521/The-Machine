...
function mathEvaluator(text, numericConfig = {}) {
  try {
    ...
    // Phase 3: Use appropriate mathjs instance based on numeric mode
    const activeInstance = config.mode === 'big' ? mathjsBigNumber : mathjs
    if (config.mode === 'big' && !mathjsBigNumber) {
      return {
        expression: trimmed,
    ...
    if (config.mode === 'big' && result && typeof result.toNumber === 'function') {
    ...
    // Phase 3: Apply precision rounding if specified
    let displayedResult = finalResult
    ...
    if (config.precision !== null && typeof finalResult === 'number') {
      ...
      displayedResult = roundedResult.value
      if (wasPrecisionRounded) {
        warnings.push(`Result was rounded to ${config.precision} decimal place${config.precision === 1 ? '' : 's'}`)
      }
    }

    // Phase 3: Format result based on notation setting
    let formattedResult = displayedResult
    if (config.notation === 'scientific' && Math.abs(displayedResult) >= 1000) {
      formattedResult = displayedResult.toExponential()
    }
