...
export const TOOLS = {
  ...
  'base64-converter': {
    ...
    configSchema: [
      ...
      {
        id: 'charEncoding',
        label: 'Character Encoding (Encode)',
        type: 'select',
        options: [
      ...
      {
        id: 'urlSafe',
        label: 'URL-Safe Output (Encode)',
        type: 'checkbox',
        default: false,
      },
      {
        id: 'removePadding',
        label: 'Remove Padding (Encode)',
        type: 'checkbox',
        default: false,
      },
      {
        id: 'lineWrap',
        label: 'Line Wrap 76 chars (MIME)',
        type: 'checkbox',
        default: false,
      },
      {
        id: 'robustMode',
        label: 'Robust Mode (Decode)',
        type: 'checkbox',
        default: false,
      },
      {
        id: 'autoFixPadding',
        label: 'Auto-Fix Padding (Decode)',
        type: 'checkbox',
        default: true,
      },
    ],
...
function base64ConverterTool(inputText, config = {}) {
  ...
  const removePadding = config.removePadding || false
  const lineWrap = config.lineWrap || false
  const robustMode = config.robustMode || false
  const autoFixPadding = config.autoFixPadding !== false
...
function base64Encoder(text, options = {}) {
  try {
    ...
    const removePadding = options.removePadding || false
    const lineWrap = options.lineWrap || false
    ...
    // Determine output
    let output = base64
    if (urlSafe && removePadding) {
      output = urlSafeNoPadding
    } else if (urlSafe) {
      output = urlSafeBase64
    ...
    if (lineWrap) {
      if (urlSafe && removePadding) {
        output = wrappedUrlSafe.replace(/=+/g, '')
      } else if (urlSafe) {
        output = wrappedUrlSafe
      } else if (removePadding) {
        output = wrapped.replace(/=+/g, '')
      } else {
        output = wrapped
      }
    }

    const inputBytes = Buffer.from(text, 'utf-8').length
    const outputBytes = Buffer.from(output, 'utf-8').length
    ...
    return {
      ...
      // Stats
      stats: {
        inputSize: `${inputBytes} byte${inputBytes !== 1 ? 's' : ''}`,
        outputSize: `${outputBytes} byte${outputBytes !== 1 ? 's' : ''}`,
        encodingUsed: charEncoding.toUpperCase(),
        detectedMode: 'Encode',
        compressionRatio: ((outputBytes / inputBytes) * 100).toFixed(2) + '%',
      },
    }
  } catch (error) {
    return {
...
function base64Decoder(text, options = {}) {
  ...
  const autoFixPadding = options.autoFixPadding !== false
  ...
  try {
    ...
    return {
      ...
      stats: {
        inputSize: `${Buffer.from(cleanText, 'utf-8').length} byte${Buffer.from(cleanText, 'utf-8').length !== 1 ? 's' : ''}`,
        outputSize: `${Buffer.from(decodedText, 'utf-8').length} byte${Buffer.from(decodedText, 'utf-8').length !== 1 ? 's' : ''}`,
    ...
    if (error.message.includes('is not a valid base64')) {
      ...
      errorMsg = 'Length is not divisible by 4'
      if (!robustMode) {
        errorMsg += '. Enable "Auto-fix Padding" to try fixing it automatically.'
      }
    }
    ...
    return {
      ...
      output: '',
      input: text,
      suggestion: !robustMode ? 'Try enabling "Robust Mode" or "Auto-fix Padding"' : null,
    }
  }
}
