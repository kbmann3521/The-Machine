...
function emailValidator(text) {
  ...
  const results = emails.map((email) => {
    ...
    // Normal human names have 30-50% vowels. Gibberish has <20% vowels.
    ...
    // Check for excessive consonant clusters (unnatural patterns)
    // Split by vowels and look at consonant sequences
    const consonantSequences = (localPart.toLowerCase().split(/[aeiou]/) || []).filter(s => s.length > 0)

    // Count sequences of 3+ consonants
    const longConsonantClusters = consonantSequences.filter(seq => seq.length >= 3).length
    // If 2+ long clusters OR 1 cluster in short username = gibberish
    if (longConsonantClusters >= 2) {
      details.usernameHeuristics.push('Username appears to be gibberish or randomly generated')
    } else if (longConsonantClusters === 1 && localPart.length <= 15) {
      // In a short username, even one 3+ consonant cluster is suspicious
      details.usernameHeuristics.push('Username appears to be gibberish or randomly generated')
    }
    ...
    if (details.issues.length > 0) {
      ...
      if (gibberishHeuristics.length > 0) {
        // Sliding scale penalty based on length - longer gibberish gets harsher penalty
        // 15-20 chars: -10 penalty
        // 25 chars: -12 penalty
        // 30 chars: -15 penalty
        // 40 chars: -20 penalty
        // 50 chars: -25 penalty
        // 60+ chars: -35 penalty (hard cap)

        // Linear scaling: -10 - (length - 15) * 0.5, capped at -35
