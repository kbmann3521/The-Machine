...
function checkSafetyGates(optimization, safetyFlags, config) {
  ...
  switch (optimization) {
    case 'precisionReduction':
      // Precision reduction gating based on decimals and features
      // Allow safe reductions (3+ decimals) even with text
      // Block aggressive reductions (< 2 decimals) with masks/filters
      const targetDecimals = config.precisionReduction?.decimals || 3

      if (safetyFlags.hasText && targetDecimals < 3) {
        gating.allowed = false
        gating.reason = 'Text detected: precision reduction below 3 decimals may affect font rendering'
        gating.skipped = true
      } else if (safetyFlags.hasMasks && targetDecimals < 2) {
        gating.allowed = false
        gating.reason = 'Masks detected: precision reduction below 2 decimals may shift visual clipping'
        gating.skipped = true
      } else if (safetyFlags.hasFilters && targetDecimals < 2) {
        gating.allowed = false
        gating.reason = 'Filters detected: precision reduction below 2 decimals may affect filtering'
        gating.skipped = true
      }
      break
...
function cleanupIDs(svg, mode = 'preserve', safetyFlags = {}) {
  let cleaned = svg
  ...
  if (mode === 'unused-only') {
    ...
    // Minify IDs: rewrite all IDs with shorter names and update all references
    // This ensures no broken references
    const idMap = {}
    let minifyIndex = 0
    ...
    // Map ALL IDs (both used and unused) to minified names
    definedIDs.forEach((id) => {
      const newID = minifyID(minifyIndex++)
      idMap[id] = newID
    })

    // Replace all ID definitions and all references with new minified IDs
    Object.entries(idMap).forEach(([oldID, newID]) => {
      const escapedID = oldID.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&')
      cleaned = cleaned.replace(new RegExp(`id="${escapedID}"`, 'g'), `id="${newID}"`)
      cleaned = cleaned.replace(new RegExp(`#${escapedID}`, 'g'), `#${newID}`)
      removedCount++
    })
  }
...
function removeEmptyGroupsFromSvg(svg) {
  let result = svg
  const before = result.length
  let removedCount = 0
  ...
  try {
    ...
    while (changed && iterations < maxIterations) {
      ...
      // Remove groups with no attributes and no content: <g></g> or <g> </g> or <g>\n</g>
      const emptyGroupPattern = /<g\s*>\s*<\/g>/gi
      const afterEmpty = result.replace(emptyGroupPattern, '')
      ...
      // Matches <g> (with optional whitespace, no attributes) followed by content, then </g>
      // Uses non-greedy matching to handle the innermost <g></g> pair first
      // [\s\S]*? matches any character including newlines, non-greedily
      const attributelessGroupPattern = /<g\s*>([\s\S]*?)<\/g>/gi
      const afterFlatten = result.replace(attributelessGroupPattern, '$1')
...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}, config = {}) {
  ...
  const steps = [
    ...
    { name: 'removeUnusedDefs', fn: () => ({ svg: removeUnusedDefsFromSvg(currentSvg, phase0_1Result.analysis.references) }) },
    { name: 'removeEmptyGroups', fn: () => {
      const result = removeEmptyGroupsFromSvg(currentSvg)
    ...
    { name: 'idCleanup', fn: () => cleanupIDs(currentSvg, phase2Config.idCleanup.mode, phase0_1Result.safetyFlags) },
  ...
  // RE-ANALYZE ON OPTIMIZED SVG: Critical fix to ensure analysis reflects the final state
  // This ensures linting, references, and hints are accurate for the outputted SVG
  ...
  try {
    ...
    // Fall back to original analysis if re-analysis fails
  }
