export const TOOLS = {
  'text-toolkit': {
    ...
    detailedDescription: {
      overview: 'The Text Toolkit combines multiple text analysis and transformation tools into one unified interface. Instantly get word counts, text analysis, reversals, slugs, case conversions, and whitespace visualization all from a single input.',
      howtouse: [
        'Paste or type your text into the input field',
        'The toolkit automatically runs all analysis tools',
        'View results from each tool in organized sections:',
        '  • Word Counter: statistics on words, characters, sentences',
        '  • Text Analyzer: readability scores and text metrics',
        '  • Case Converter: uppercase transformation',
        '  • Slug Generator: URL-friendly slug format',
        '  • Reverse Text: character-reversed version',
        '  �� Whitespace Visualizer: show hidden whitespace',
        '  • Sort Lines: alphabetically sorted lines',
        'Copy individual results or the complete output',
      ],
      features: [
        'Multiple text analysis tools in one unified interface',
        'Real-time analysis of all metrics',
      ...
      usecases: [
        'Comprehensive text analysis for content creators',
        'Developer debugging and text transformation',
        'SEO optimization with slug generation',
        'Readability assessment for target audiences',
        'Data analysis and text processing',
        'Format validation and text cleanup',
      ],
    },
  },
  'image-resizer': {
  ...
  'escape-unescape': {
    ...
    detailedDescription: {
      overview: 'The Escape/Unescape tool converts text between plain and escaped formats across JavaScript, JSON, HTML, and CSV standards. Choose which escaping standard to apply or reverse.',
      howtouse: [
        'Paste your text into the input field',
        'Select "Escape" to encode special characters or "Unescape" to decode them',
        'Select the escape type (JavaScript, JSON, HTML, or CSV) that matches your use case',
        'View the converted result',
      ...
      usecases: [
        'Preparing text for JSON payloads or APIs',
        'Escaping quotes and special chars for JavaScript code',
        'Converting special characters to HTML entities',
        'Creating valid CSV files with quoted fields',
  ...
  'sort-lines': {
    ...
    detailedDescription: {
      overview: 'The Sort Lines tool organizes lines of text in various orders: alphabetically, numerically, by length, or reversed. Perfect for organizing lists, data cleanup, and sorting content with flexible options.',
      howtouse: [
  ...
  switch (toolId) {
    case 'text-toolkit':
      const toolkitResult = {
        ...
        removeExtras: removeExtras(inputText, {
          trimSpaces: config?.trimSpaces !== false ? true : false,
          removeBlankLines: config?.removeBlankLines !== false ? true : false,
          removeDuplicateLines: config?.removeDuplicateLines || false,
...
export function getToolExample(toolId, config = {}) {
  switch (toolId) {
    ...
    case 'remove-extras': {
      const parts = []
      if (config.trimSpaces) {
        parts.push('  Text with leading and trailing spaces  ')
      }
      if (config.removeBlankLines) {
        parts.push('Line one\n\n\nLine two\n\n\nLine three')
      }
      ...
      if (parts.length === 0) {
        return '  This   text   has   extra    spaces\n\n\nand multiple line breaks  '
      }
      return parts.join('\n')
    }
...
export function autoDetectToolConfig(toolId, inputText) {
  ...
  switch (toolId) {
    ...
    case 'json-formatter':
      if (isJsonMinified(trimmed)) {
        return { mode: 'beautify' }
      } else if (isJsonFormatted(trimmed)) {
        return { mode: 'minify' }
      }
      return null

    case 'xml-formatter':
      if (isFormattedXml(trimmed)) {
        return { mode: 'minify' }
      } else {
        return { mode: 'beautify' }
      }
    ...
    case 'css-formatter':
      return isMinifiedCss(trimmed) ? { mode: 'beautify' } : { mode: 'minify' }
