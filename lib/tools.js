export const TOOLS = {
    ...
    configSchema: [
      ...
      },
      {
        ...
        options: [
          ...
          { value: 'alpha', label: 'Alphabetic (A-Z, a-z)' },
          { value: 'numeric', label: 'Numeric (0-9)' },
          { value: 'hex', label: 'Hexadecimal (0-F)' },
      ...
      {
        id: 'customCharset',
        label: 'Custom Characters (if custom selected)',
        type: 'text',
  ...
  'variable-name-generator': {
    ...
    description: 'Generate random variable names in different naming conventions',
    category: 'generator',
    inputTypes: ['text'],
    outputType: 'text',
    configSchema: [
  ...
  'function-name-generator': {
    name: 'Function Name Generator',
    description: 'Generate meaningful function names based on action verbs',
    category: 'generator',
    inputTypes: ['text'],
    outputType: 'text',
    configSchema: [
  ...
  'api-endpoint-generator': {
    ...
    description: 'Generate realistic API endpoint paths and routes',
    category: 'generator',
    inputTypes: ['text'],
    outputType: 'text',
    configSchema: [
...
export function runTool(toolId, inputText, config, inputImage = null) {
  switch (toolId) {
    ...
    case 'random-string-generator':
      return randomStringGenerator(config)
    case 'variable-name-generator':
      return variableNameGenerator(config)
    case 'function-name-generator':
      return functionNameGenerator(config)
    case 'api-endpoint-generator':
      return apiEndpointGenerator(config)
    case 'lorem-ipsum-generator':
      return loremIpsumGenerator(config)
    default:
...
function httpStatusLookup(text) {
  const codes = { '200': 'OK', '201': 'Created', '204': 'No Content', '400': 'Bad Request', '401': 'Unauthorized', '403': 'Forbidden', '404': 'Not Found', '500': 'Internal Server Error', '503': 'Service Unavailable' }
  const code = text.trim()
  return { code, meaning: codes[code] || 'Unknown status code' }
}
...
function imageToBase64(imageData) {
  return imageData ? { message: 'Image Base64 preview', length: imageData.length } : { error: 'No image' }
}
...
function unitConverter(text, config) {
  ...
  return isNaN(value) ? { error: 'Invalid number' } : { value, type: config.type }
}
...
function timezoneConverter(text, config) {
  return { message: 'Timezone conversion ready', from: config.fromTimezone, to: config.toTimezone }
}
...
function randomStringGenerator(config) {
  const charsets = {
    alphanumeric: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789',
    alpha: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',
    numeric: '0123456789',
    hex: '0123456789ABCDEF',
    custom: config.customCharset || 'ABC',
  }

  const charset = charsets[config.charset] || charsets.alphanumeric
  const length = parseInt(config.length) || 16
  let result = ''
...
function variableNameGenerator(config) {
  const adjectives = ['fast', 'slow', 'big', 'small', 'new', 'old', 'good', 'bad', 'hot', 'cold', 'safe', 'risky', 'smart', 'dumb']
  const nouns = ['user', 'data', 'cache', 'store', 'queue', 'handler', 'service', 'manager', 'helper', 'utils', 'config', 'state']
  ...
  const generate = (count) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      switch (config.convention) {
        case 'snake_case':
          name = base.replace(/[A-Z]/g, c => '_' + c.toLowerCase()).replace(/^_/, '').toLowerCase()
          break
        case 'PascalCase':
          name = base.charAt(0).toUpperCase() + base.slice(1)
          break
        case 'UPPER_CASE':
          name = base.replace(/[A-Z]/g, c => '_' + c).replace(/^_/, '').toUpperCase()
          break
        case 'kebab-case':
          name = base.replace(/[A-Z]/g, c => '-' + c.toLowerCase()).replace(/^-/, '').toLowerCase()
          break
        default:
          name = base
      }
  ...
  return generate(parseInt(config.count) || 5).join('\n')
}

function functionNameGenerator(config) {
  const verbs = ['get', 'set', 'fetch', 'create', 'delete', 'update', 'validate', 'process', 'handle', 'calculate', 'generate', 'parse']
  const nouns = ['user', 'data', 'request', 'response', 'error', 'token', 'session', 'config', 'state', 'event']
  ...
  const generate = (count) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const noun = nouns[Math.floor(Math.random() * nouns.length)]
      const base = verb + noun.charAt(0).toUpperCase() + noun.slice(1)

      let name = base
      switch (config.convention) {
        case 'snake_case':
          name = base.replace(/[A-Z]/g, c => '_' + c.toLowerCase()).toLowerCase()
          break
        case 'PascalCase':
          name = base.charAt(0).toUpperCase() + base.slice(1)
          break
...
function apiEndpointGenerator(config) {
  const resources = ['users', 'posts', 'comments', 'products', 'orders', 'articles', 'tags', 'categories']
  const operations = ['list', 'get', 'create', 'update', 'delete', 'search']
  ...
  const generate = (count) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (config.style === 'rest') {
        const methods = { list: 'GET', get: 'GET', create: 'POST', update: 'PUT', delete: 'DELETE', search: 'GET' }
        const paths = { list: `/${resource}`, get: `/${resource}/:id`, create: `/${resource}`, update: `/${resource}/:id`, delete: `/${resource}/:id`, search: `/${resource}/search` }
        endpoints.push(`${methods[op]} ${paths[op]}`)
      } else if (config.style === 'graphql') {
        const queryOps = { list: `${resource}s`, get: resource, search: `search${resource.charAt(0).toUpperCase()}${resource.slice(1)}` }
        endpoints.push(`query { ${queryOps[op] || resource} }`)
      } else {
        endpoints.push(`service.${resource}.${op}()`)
      }
    }
    return endpoints
  }
...
function loremIpsumGenerator(config) {
  ...
  const paragraphs = [sentences.slice(0, 2).join(' '), sentences.slice(1, 3).join(' '), sentences.slice(2, 4).join(' '), sentences.slice(3, 5).join(' ')]
  ...
  let result = generate()
  if (!config.startWithLorem && config.type !== 'words') {
