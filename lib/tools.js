...
function cleanupIDs(svg, mode = 'preserve', safetyFlags = {}) {
  ...
  if (mode === 'unused-only') {
    ...
    definedIDs.forEach((id) => {
      if (!usedIDs.has(id)) {
        // Match id attribute with any preceding whitespace/newlines and optional trailing whitespace
        cleaned = cleaned.replace(new RegExp(`\\s+id="${id.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&')}"`, 'g'), '')
        removedCount++
      }
    })
  } else if (mode === 'minify') {
...
function applyPhase2Optimization(phase0_1Result, optimizationLevel, advancedOverrides = {}, config = {}) {
  ...
  const steps = [
    ...
    { name: 'removeEmptyGroups', fn: () => ({ svg: removeEmptyGroupsFromSvg(currentSvg) }) },
