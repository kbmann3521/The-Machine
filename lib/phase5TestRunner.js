...
export async function runPhase5Tests(options = {}) {
  ...
  for (const test of testPack.tests) {
    ...
    try {
      ...
      const header = decoderResult.token?.header || {}
      const payload = decoderResult.token?.payload || {}

      result.actual.header = header
      result.actual.payload = payload
      result.actual.tokenType = decoderResult.tokenType?.type || 'Unknown'
      ...
      if (test.expectedElements?.tokenType) {
        ...
        const actualType = decoderResult.tokenType?.type || 'Unknown'
