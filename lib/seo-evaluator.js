...
function evaluateHealthChecks(post, h1Count, h2Count, html, introContent, contentLength) {
  ...
  // Internal links check - count actual links
  const internalLinkMatches = (post.content || '').match(/\[.*?\]\(\/(?:blog|tools)\/.*?\)/g) || []
  if (internalLinkMatches.length === 0) {
    checks.push({
      status: 'warning',
      label: 'No internal links detected',
      explanation:
        'Internal links help search engines discover related content and distribute authority. Consider linking to relevant tools or articles.',
    })
  } else {
    checks.push({
      status: 'success',
      label: `Internal links present (${internalLinkMatches.length})`,
      explanation: `You have ${internalLinkMatches.length} link${internalLinkMatches.length > 1 ? 's' : ''} to related content. This helps users and search engines.`,
    })
  }
