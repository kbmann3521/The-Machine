...
import { JWT_TEST_PACK } from './jwtTestPack.js'
...
export function runJWTTests() {
  ...
  JWT_TEST_PACK.tests.forEach(test => {
    ...
    // Resolve publicKey reference if it points to meta
    let publicKey = test.publicKey || ''
    if (typeof publicKey === 'string' && publicKey.startsWith('defaultPublicKeys.')) {
      const keyName = publicKey.replace('defaultPublicKeys.', '')
      publicKey = JWT_TEST_PACK.meta.defaultPublicKeys[keyName] || ''
    }
