...
const patterns = {
  ...
  unit_value: /^\d+(?:\.\d+)?\s*(?:[a-z°µ²³\/\-°\s]+)(?:\s*(?:per|\/)\s*[a-z°µ²³\/\-°\s]+)?s?$/i,
...
const validUnits = new Set([
  'kg', 'g', 'mg', 'lb', 'lbs', 'kilogram', 'kilograms', 'milligram', 'milligrams', 'gram', 'grams', 'pound', 'pounds', 'oz', 'ounce', 'ounces', 'ton', 'tonne', 'tonnes', 'stone', 'st', 't', 'carat', 'grain', 'grains',
  'm', 'meter', 'meters', 'metre', 'metres', 'km', 'kilometer', 'kilometers', 'kilometre', 'kilometres', 'centimeter', 'centimeters', 'centimetre', 'centimetres', 'millimeter', 'millimeters', 'millimetre', 'millimetres', 'micrometer', 'micrometers', 'cm', 'mm', 'µm', 'nm', 'nanometer', 'nanometers', 'ft', 'feet', 'foot', 'mi', 'mile', 'miles', 'yd', 'yard', 'yards', 'in', 'inch', 'inches', 'nmi', 'nautical',
...
function levenshteinDistanceQuick(a, b) {
  const aLen = a.length
  const bLen = b.length
...
function isValidUnit(unitString) {
  ...
  if (validUnits.has(lower) || validUnits.has(lower + 's')) {
    return true
  }

  // Fuzzy match: allow up to 2 character differences
  for (const validUnit of validUnits) {
    const distance = levenshteinDistanceQuick(lower, validUnit)
...
function analyzeMarkdownIntent(input) {
  ...
  // If markdown is VALID (well-formed, intentional), ALWAYS match with markdown_html
  // This is the key insight: valid markdown means the user INTENDED markdown, not accidental symbols
  // Validity check takes absolute priority over noise detection
  ...
  // Priority 4: INVALID/BROKEN MARKDOWN
  // If markdown is INVALID (unmatched delimiters, broken structure), match with markdown_clean
  // Only check validity failure, NOT noise (noise alone shouldn't trigger clean without features)
  if (featureCount >= 1 && !isValidMarkdown) {
    return { type: 'markdown_clean', confidence: 0.85, reason: 'Broken or incomplete markdown detected - intent: clean text' }
  }

  // Priority 5: CLEAR NOISE PATTERNS WITHOUT MARKDOWN STRUCTURE
  // Only flag as noise if there's no markdown features (prevents false positives)
  if (hasNoisePatterns && featureCount === 0) {
    return { type: 'markdown_clean', confidence: 0.80, reason: 'Detected text corruption/OCR artifacts - intent: clean text' }
  }
...
/**
 * Detect noise patterns that indicate messy/OCR text or corruption
 * This focuses on ACTUAL CORRUPTION, not just "unusual" formatting
 *
 * Examples of actual noise:
 * - OCR artifacts (spaces in middle of words)
 * - Repeated corrupted characters
 * - Excessive uncontrolled spacing
 * - Text that clearly went through OCR/corruption
 */
function detectMarkdownNoise(input) {
  // SEVERE: OCR-like spacing (spaces inside words, e.g., "Th is te xt")
  // This is a strong signal of OCR corruption
  if (/\b\w+\s{1}\w+\s{1}\w+\s{1}\w+/.test(input)) {
    return true
  }

  // SEVERE: Markdown symbols scattered inside words (not just emphasis)
  // e.g., "sen*ten*ce" or "accid*ental*" = corruption or OCR artifact
  const markdownInsideWords = (input.match(/\w\*\w/g) || []).length
  if (markdownInsideWords >= 3) { // 3+ scattered = clearly corrupt
    return true
  }

  // SEVERE: Unclosed code fences (only 1 or odd number with no matched pair)
  const codeFences = (input.match(/```/g) || []).length
  if (codeFences === 1) {
    return true // Single code fence = incomplete/broken
  }

  // SEVERE: Unclosed inline code backticks (odd number)
  const backtickCount = (input.match(/`/g) || []).length
  if (backtickCount % 2 !== 0) {
    return true // Unmatched backticks = broken inline code
  }

  // SEVERE: Misaligned table pipes (consecutive pipes like ||)
  if (/\|\|/.test(input)) {
    return true // Double pipes = malformed table
  }

  // MODERATE: Excessive line breaks (3+ in a row = formatting corruption)
  if (/\n{3,}/.test(input)) {
    return true
  }

  // MODERATE: Excessive spaces (4+ consecutive = unlikely to be intentional)
  if (/\s{4,}/.test(input)) {
    return true
  }

  // MODERATE: Repeated punctuation marks that form "noise" patterns
  // (4+ of the same character in a row, like ****, ____, ----)
  // Note: *** is valid markdown (bold+italic), so we allow up to 3
  if (/\*{4,}|_{4,}|-{4,}/.test(input)) {
    return true
  }

  // NO LONGER flagging: heading without space after # (#hello)
  // This could be valid markdown in some parsers, and we want to be lenient
  // Users can still convert it and it might work
 ...
 *
 * This checks for STRUCTURAL VALIDITY, not noise.
 * A markdown with minor noise but good structure can still be "valid" for conversion.
 */
function checkMarkdownValidity(input, lines, features) {
  // Check 1: Heading format - must have space after #
  if (features.headings) {
    const invalidHeadings = lines.filter(l => /^#+/.test(l) && !/^#{1,6}\s/.test(l))
    if (invalidHeadings.length > 0) {
      return false // Headings without space after # = malformed
    }
  }

  // Check 2: Emphasis markers must be paired and coherent
  const asteriskCount = (input.match(/\*/g) || []).length
  const underscoreCount = (input.match(/_/g) || []).length

  // Count actual valid emphasis pairs (properly matched)
  const validBoldPairs = (input.match(/\*\*[^*\n]+\*\*/g) || []).length
  const validItalicPairs = (input.match(/(?<!\*)\*[^*\n]+\*(?!\*)/g) || []).length
  const validUnderscorePairs = (input.match(/__[^_\n]+__/g) || []).length
  const totalValidEmphasisPairs = validBoldPairs + validItalicPairs + validUnderscorePairs

  // If there are asterisks but very few valid pairs, it's scattered/broken emphasis
  if (asteriskCount >= 3) {
    // With 3+ asterisks, we should have at least some valid pairs
    // If we have scattered asterisks with minimal valid structure, flag as broken
    if (totalValidEmphasisPairs === 0) {
      return false // Asterisks present but no coherent emphasis structure
    }
    // If emphasis markers >> valid pairs, it's scattered/corrupted
    // e.g., "text *** with ** and stray *" has 6 asterisks but scattered placement
    if (asteriskCount > totalValidEmphasisPairs * 2.5) {
      return false // Too many scattered asterisks relative to valid pairs
    }
  }

  // If there are asterisks and they're odd, it's unmatched
  if (asteriskCount > 0 && asteriskCount % 2 !== 0) {
    // Only allow if there's at least one valid emphasis structure
    if (totalValidEmphasisPairs === 0) {
      return false // Truly unmatched asterisks with no valid structure
    }
  }

  // Similar check for underscores
  if (underscoreCount > 2) {
    // Count valid underscore pairs - only match actual word/phrase emphasis patterns
    // __bold__ or _italic_ (where content is compact, not spanning across other underscores)
    const boldUnderscores = (input.match(/__[^_\n]{1,100}__/g) || []).length
    const italicUnderscores = (input.match(/\b_[^_\n]{1,100}_\b/g) || []).length
    const validUnderscorePairsCount = boldUnderscores + italicUnderscores

    if (underscoreCount % 2 !== 0 && validUnderscorePairsCount === 0) {
      return false // Unmatched underscores with no valid structure
    }

    // If many scattered underscores with few valid pairs, it's broken
    // E.g., "_text is _kind of _broken_" likely has scattered emphasis
    if (underscoreCount >= 3 && validUnderscorePairsCount === 0) {
      return false // Multiple underscores but no coherent emphasis pattern
    }

    // If total underscores >> valid pairs (indicating scattered markers), flag as broken
    if (underscoreCount > (validUnderscorePairsCount * 2.5)) {
      return false // Too many scattered underscores relative to valid pairs
    }
  }

  // Check 3: Backticks must be paired (for inline code)
  const backticks = (input.match(/`/g) || []).length
  if (backticks > 0 && backticks % 2 !== 0) {
    return false // Unclosed inline code = broken
  }

  // Check 4: Code fences must be paired
  const codeFences = (input.match(/```/g) || []).length
  if (codeFences > 0 && codeFences % 2 !== 0) {
    return false // Unclosed code block = broken
  }

  // Check 5: Tables must have proper structure (if detected)
  if (features.tables) {
    const tableLines = lines.filter(l => /\|.*\|/.test(l))
    // A valid table needs: at least 2 lines with pipes + at least 1 separator line
    if (tableLines.length >= 2) {
      const hasSeparator = tableLines.some(l => /\|\s*-+\s*\|/.test(l))
      if (!hasSeparator) {
        return false // Pipes detected but no table separator = incomplete table
      }
    }
  }

  // Check 6: Links must have proper format if detected
  const links = input.match(/\[[^\]]+\]\([^)]+\)/g) || []
  const brokenLinks = input.match(/\[[^\]]*\](?!\()/g) || [] // [ ] not followed by (
  if (brokenLinks.length > 0 && links.length === 0) {
    return false // Has link-like syntax but none are properly formatted
  }

  // Check 7: Check for really excessive broken patterns that indicate corruption
  // (more severe than detectMarkdownNoise)
  const hasMultipleUnmatchedSymbols = (asteriskCount % 2 !== 0) && (underscoreCount % 2 !== 0)
  if (hasMultipleUnmatchedSymbols) {
    return false // Multiple unmatched emphasis markers = likely broken
  }

  // If we pass all the validity checks, and we have actual markdown features,
  // then this is VALID markdown (regardless of minor noise/spacing issues)
  const hasAnyValidMarkdown = features.headings || features.lists || features.orderedLists ||
                               features.codeFences || (links.length > 0) || features.tables ||
                               features.emphasis || features.inlineCode || features.blockquotes || features.images

  return hasAnyValidMarkdown
}
...
export function hardDetect(input) {
  ...
  // YAML detection BEFORE markdown (since YAML arrays "- item" match markdown list pattern)
  // Traditional YAML: key: value with proper indentation
  if (patterns.yaml.test(trimmed) && /^[a-z_][a-z0-9_-]*\s*:/im.test(trimmed)) {
    // Do additional validation: check if it has YAML-specific patterns
    const lines = trimmed.split('\n')
    const yamlKeyValueLines = lines.filter(l => /^[\s]*[a-z_][a-z0-9_-]*\s*:/im.test(l.trim())).length
    const totalLines = lines.filter(l => l.trim().length > 0).length

    // If more than 50% of lines are key: value pairs, it's YAML
    if (totalLines > 0 && yamlKeyValueLines >= totalLines * 0.5) {
      return { type: 'yaml', confidence: 0.92, reason: 'YAML format detected' }
    }

    // Even if not 50%, if we see colons and indentation, it's likely YAML
    const hasProperIndentation = lines.some(l => /^  [a-z_]/.test(l))
    if (hasProperIndentation) {
      return { type: 'yaml', confidence: 0.88, reason: 'YAML format detected' }
    }
  }

  // Flattened YAML: key.path = value (e.g., server.port = 8080)
  const lines = trimmed.split('\n')
  const flattenedYamlLines = lines.filter(l => patterns.flattened_yaml.test(l.trim())).length
  const totalLines = lines.filter(l => l.trim().length > 0).length

  if (totalLines > 0 && flattenedYamlLines >= totalLines * 0.7) {
    // If 70%+ of non-empty lines match flattened YAML pattern, it's flattened YAML
    return { type: 'yaml', confidence: 0.85, reason: 'Flattened YAML format detected' }
  }

  // Markdown detection with multi-layer intent analysis (AFTER YAML to avoid confusion)
  ...
  // But exclude unit values like "50 m/s"
  if (patterns.math_expression.test(trimmed) && /[+\-*/()\^]/.test(trimmed) && !trimmed.includes('[') && !trimmed.includes('\\') && !/^\d{4}-\d{2}-\d{2}/.test(trimmed) && !/^\d+(?:\.\d+)?\s*[a-z°µ²³\/\-°\s]*$/i.test(trimmed)) {
  ...
  // Use universal fuzzy number+unit detector (catches misspellings, weird spacing, etc.)
