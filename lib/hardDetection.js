...
function checkMarkdownValidity(input, lines, features) {
  ...
  // Check 2: Emphasis markers must be paired (balanced)
  const asteriskCount = (input.match(/\*/g) || []).length
  const underscoreCount = (input.match(/_/g) || []).length

  // If there are asterisks and they're unmatched, it's broken
  if (asteriskCount > 0 && asteriskCount % 2 !== 0) {
    // Exception: single * in sentences is OK if there's valid bold/italic structure elsewhere
    const hasValidEmphasis = /\*\*[^*]+\*\*|\*[^*]+\*/.test(input)
    if (!hasValidEmphasis) {
      return false // Truly unmatched asterisks
    }
  }

  // If there are underscores and they're unmatched, might be broken
  // But allow 1 underscore (like variable_name) or 2 (like snake_case)
  if (underscoreCount > 2 && underscoreCount % 2 !== 0) {
    const hasValidUnderscore = /__[^_]+__/.test(input) || /_[^_]+_/.test(input)
    if (!hasValidUnderscore) {
      return false // Truly unmatched underscores with structure
    }
  }
