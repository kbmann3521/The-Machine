...
export function hardDetectMatrix(input) {
  ...
  // HTML tags (like <div>, <p>, etc.) indicate HTML formatting, not just entity-encoding
  // Encoded HTML tags (&lt;, &gt;) are still entities but wrapped in tag structure
  if (patterns.html_entities.test(trimmed)) {
    // Check if there are actual HTML tags (opening <tag> or closing </tag>)
    const hasHtmlTags = /<[a-z][a-z0-9]*[\s>\/>]/i.test(trimmed)

    if (!hasHtmlTags) {
      // No HTML tags found - this is pure entity-encoded text
