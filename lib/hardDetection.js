...
export function hardDetectMatrix(input) {
  ...
  // UPC list detection (for barcode-generator)
  const upcLines = trimmed.split('\n').filter(l => l.trim())
  if (upcLines.length > 0 && upcLines.every(l => /^\d{11,14}$/.test(l.trim().replace(/\D/g, '')))) {
    // If we have a list of numbers that look like UPCs
    add('upc_list', 0.95, 'List of numeric codes (possible UPC-A) detected')
  }
