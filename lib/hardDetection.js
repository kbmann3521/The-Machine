...
export const patterns = {
  ...
  js: /(^\s*(function|const|let|var|async|import|export|class)\b)|(\bconsole\.log\b)|({[\s\S]*}[\s;]*)/m,
...
function detectCaesar(str) {
  ...
  for (let shift = 1; shift < 26; shift++) {
    ...
    const matches = decoded
      .toLowerCase()
      .split(/\s+/)
      .filter(w => englishWords.has(w)).length
...
export function isUnicodeText(input) {
  ...
  // Check for emoji: Unicode characters in ranges typically associated with emojis
  // Emoji ranges: U+1F300–U+1F9FF, U+2600–U+27BF, etc.
  const emojiPattern = /[\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{27BF}]|[\u{1F600}-\u{1F64F}]/gu

  // Check for other non-ASCII characters (accented letters, symbols from other scripts)
  const nonAsciiPattern = /[^\x00-\x7F]/

  const hasEmoji = emojiPattern.test(trimmed)
  const hasNonAscii = nonAsciiPattern.test(trimmed)

  return hasEmoji || hasNonAscii
}
...
export function hardDetectMatrix(input) {
  ...
  if (patterns.csv.test(trimmed)) {
    add('csv', 0.93, 'CSV format detected')
  }
  ...
  if (patterns.sql.test(trimmed)) {
    add('sql', 0.93, 'SQL statement detected')
  }

  if (patterns.http_header.test(trimmed)) {
    add('http_header', 0.96, 'HTTP header detected')
  }
  ...
  if (detectCaesar(trimmed)) {
    add('caesar_encoded', 0.97, 'Possible Caesar cipher detected')
  }

  if (detectRot13(trimmed)) {
    add('rot13_encoded', 0.98, 'ROT13 encoded text detected')
  }
