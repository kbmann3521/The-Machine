...
function analyzeMarkdownIntent(input) {
  ...
  if (hasNoisePatterns) {
    return { type: 'markdown_clean', confidence: 0.92, reason: 'Detected noise/formatting issues - intent: clean text' }
  }
  ...
  // Check if markdown is VALID/WELL-FORMED (key insight!)
  const isValidMarkdown = checkMarkdownValidity(trimmed, lines, features)

  // ABSOLUTE STRONGEST: Tables always mean conversion (highest priority)
  if (features.tables) {
    return { type: 'markdown_html', confidence: 0.95, reason: 'Markdown table detected - intent: convert to HTML' }
  }

  // VERY STRONG: Code fences always mean conversion
  if (features.codeFences) {
    return { type: 'markdown_html', confidence: 0.94, reason: 'Fenced code block detected - intent: convert to HTML' }
  }

  // VERY STRONG: Headings + lists = clear documentation
  if (features.headings && (features.lists || features.orderedLists)) {
    return { type: 'markdown_html', confidence: 0.93, reason: 'Documentation-style markdown - intent: convert to HTML' }
  }

  // STRONG: Valid markdown with good structure = convert
  if (isValidMarkdown && featureCount >= 1 && consistencyScore > 0.5) {
    return { type: 'markdown_html', confidence: 0.88, reason: 'Valid markdown detected - intent: convert to HTML' }
  }

  // STRONG: Valid markdown with meaningful features = convert
  if (isValidMarkdown && meaningfulHTML) {
    return { type: 'markdown_html', confidence: 0.85, reason: 'Valid markdown formatting detected - intent: convert to HTML' }
  }

  // MEDIUM: Just valid markdown without complexity = still convert (user likely wants conversion)
  if (isValidMarkdown && featureCount >= 1) {
    return { type: 'markdown_html', confidence: 0.78, reason: 'Valid markdown detected - intent: convert to HTML' }
  }

  // FALLBACK: Sparse, inconsistent, or semi-broken markdown = clean/fix text
  if (featureCount >= 1) {
    return { type: 'markdown_clean', confidence: 0.76, reason: 'Markdown symbols detected but may be malformed - intent: clean text' }
  }
...
function checkMarkdownValidity(input, lines, features) {
  // If has noise patterns, it's not valid
  if (detectMarkdownNoise(input)) {
    return false
  }
  ...
  const singleAsterisks = (input.match(/\*/g) || []).length
  const singleUnderscores = (input.match(/_/g) || []).length
  if (singleAsterisks % 2 !== 0) {
    return false // Unmatched asterisks
  }
  if (singleUnderscores % 2 !== 0 && singleUnderscores > 0) {
    // Allow single underscore if it's only one (like a name_variable)
    if (singleUnderscores > 1) {
      return false
    }
  }
  ...
  return Object.values({ headings: features.headings, lists: features.lists, orderedLists: features.orderedLists, codeFences: features.codeFences, emphasis: features.emphasis, links: features.links }).some(Boolean)
}
