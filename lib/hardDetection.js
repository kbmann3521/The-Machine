...
const patterns = {
  ...
  unit_value: /^\d+(?:\.\d+)?\s*(?:kg|g|mg|lb|oz|m|cm|mm|km|mile|ft|in|°C|°F|K|Hz|kHz|MHz|GHz|V|A|W|J|L|ml|gal)$/i,
...
export function hardDetect(input) {
  ...
  // File extensions detection (comma-separated extensions like "pdf, jpg, docx, etx")
  if (/^[a-zA-Z0-9]{1,10}(?:\s*,\s*[a-zA-Z0-9]{1,10})+$/.test(trimmed)) {
    // Check if all parts look like file extensions
    const parts = trimmed.split(',').map(p => p.trim())
    const allExts = parts.every(p => /^[a-zA-Z0-9]{1,10}$/.test(p) && p.length > 1)
    if (allExts && parts.length >= 2) {
      return { type: 'file_extensions', confidence: 0.90, reason: 'File extensions format detected' }
    }
  }
  ...
  // Detect likely ROT13 encoded text (unusual letter distribution)
  if (/^[a-z\s]+$/i.test(trimmed) && trimmed.length > 8) {
    const lower = trimmed.toLowerCase()
    // Check for patterns like "yy", "yb", "yj" that appear in ROT13 text
    const hasRot13Pattern = /yy|yb|yj|qj|qb|jb/.test(lower)
    const hasCommonWords = /the|and|for|with|that|hello|world|from|have|this|are|was|been/.test(lower)
    // If no common English words and has ROT13-like patterns, might be ROT13
    if (!hasCommonWords && hasRot13Pattern) {
      return { type: 'rot13_encoded', confidence: 0.82, reason: 'Possible ROT13 encoded text' }
    }
  }

  // Detect likely Caesar cipher encoded text (shifted characters)
  if (/^[a-z\s.,;!?-]+$/i.test(trimmed) && trimmed.length > 20) {
    // Check if it looks like gibberish with unusual word patterns
    const words = trimmed.toLowerCase().split(/\s+/)
    const avgWordLength = words.reduce((a, b) => a + b.length, 0) / words.length
    // If average word length is between 3-6 and no common English words, might be Caesar
    if (avgWordLength > 3 && avgWordLength < 7 && !/the|and|for|with|that|from|have|this/.test(trimmed)) {
      return { type: 'caesar_encoded', confidence: 0.80, reason: 'Possible Caesar cipher text' }
    }
  }
