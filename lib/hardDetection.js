...
export function hardDetect(input) {
  ...
  // Detect likely ROT13 encoded text (common patterns like gibberish with vowel substitution)
  if (/^[a-z\s]+$/i.test(trimmed) && trimmed.length > 8) {
    ...
    // If the text has unusual vowel/consonant ratio, might be encoded
    if (totalLetters > 5 && (vowelCount < 2 || consonantCount < 5)) {
      return { type: 'rot13_encoded', confidence: 0.70, reason: 'Possible ROT13 encoded text' }
    }
  }
