...
export function hardDetect(input) {
  ...
  // Detect likely ROT13 encoded text (contains unusual letter patterns)
  if (/^[a-z\s]+$/i.test(trimmed) && trimmed.length > 8) {
    // Check for unusual patterns like "j", "q", "w" combined with vowels
    // or other patterns that suggest rotation cipher
    const hasUnusualPattern = /[jqw]\s|[uryyb]/.test(trimmed) && !/the|and|for|with|that/.test(trimmed.toLowerCase())
