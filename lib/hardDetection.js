...
export function hardDetect(input) {
  ...
  if (/^[a-z\s]+$/i.test(trimmed) && trimmed.length > 8) {
    ...
    // Check for "y" appearing where vowels normally would be (common in ROT13 of "hello")
    const hasYasVowel = /\by\b|\by[ybqj]/.test(lower) && /[yqj]/.test(lower)
    const hasCommonWords = /the|and|for|with|that|hello|world|from|have|this/.test(lower)
    // If no common English words and has suspicious Y patterns, might be ROT13
    if (!hasCommonWords && hasYasVowel && lower.length > 5) {
