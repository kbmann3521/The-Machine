...
function analyzeMarkdownIntent(input) {
  ...
  const meanfulHTML = features.headings || features.lists || features.blockquotes || features.links
...
function detectMarkdownNoise(input) {
  ...
  // Markdown symbols inside words (likely OCR artifact)
  if (/\w\*\w|\w_\w/.test(input)) {
    return true
  }
  ...
  const hasMultipleBrokenMarkers = (input.match(/[*_-]{2,}[^*_-\s]/g) || []).length > 3
...
export function hardDetect(input) {
  ...
  // Markdown detection with multi-layer intent analysis
