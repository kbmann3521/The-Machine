...
function checkMarkdownValidity(input, lines, features) {
  ...
  if (underscoreCount > 2) {
    // Count valid underscore pairs
    const validUnderscorePairsCount = (input.match(/__[^_\n]+__/g) || []).length +
                                      (input.match(/(?<!_)_[^_\n]+_(?!_)/g) || []).length
    ...
    // If scattered underscores (total >> valid pairs), flag as broken
    // E.g., "_text is _kind of _broken_" has 5 underscores but only 1 complete pair
    if (underscoreCount > validUnderscorePairsCount * 2) {
      return false // Too many scattered underscores relative to valid pairs
    }

    // If multiple unmatched underscores, it's broken
    if (underscoreCount > 4 && validUnderscorePairsCount === 0) {
      return false // Multiple scattered underscores = broken
    }
  }
