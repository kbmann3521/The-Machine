...
function analyzeMarkdownIntent(input) {
  ...
  // Layer 1: Count markdown features
  const features = {
  ...
  // Layer 6: Specific intent rules (highest priority)
  if (features.tables && features.tables > 0) {
    return { type: 'markdown_html', confidence: 0.95, reason: 'Markdown table detected - intent: convert to HTML' }
  }

  if (features.codeFences) {
    return { type: 'markdown_html', confidence: 0.93, reason: 'Markdown code fence detected - intent: convert to HTML' }
  }

  if (features.headings && features.lists) {
    return { type: 'markdown_html', confidence: 0.92, reason: 'Structured markdown (headings + lists) - intent: convert to HTML' }
  }

  // Layer 4: Noise detection (high priority for cleanup)
  const hasNoisePatterns = detectMarkdownNoise(trimmed)
  if (hasNoisePatterns) {
    return { type: 'markdown_clean', confidence: 0.90, reason: 'Detected noise/formatting issues - intent: clean text' }
  }

  // Layer 7: Semantic check - check if content reads like docs
  const looksLikeDocumentation = checkDocumentationPattern(trimmed, lines)

  // Layer 5: Content density ratio
  const densityScore = calculateMarkdownDensity(trimmed, featureCount)

  // Layer 2: Structure consistency
  const consistencyScore = checkMarkdownConsistency(trimmed, lines, features)

  // Layer 3: Would HTML be meaningful?
  const meaningfulHTML = features.headings || features.lists || features.blockquotes || features.links

  // Final decision logic
  if (featureCount >= 3 && consistencyScore > 0.6) {
    // Multiple features + consistent structure = intentional markdown
    return { type: 'markdown_html', confidence: 0.88, reason: 'Structured markdown detected - intent: convert to HTML' }
  }

  if (densityScore > 0.4 && meaningfulHTML) {
    // High formatting density with meaningful structure = convert
    return { type: 'markdown_html', confidence: 0.85, reason: 'Rich markdown formatting - intent: convert to HTML' }
  }

  if (looksLikeDocumentation && featureCount >= 1) {
    // Reads like documentation = convert
    return { type: 'markdown_html', confidence: 0.82, reason: 'Documentation-style markdown - intent: convert to HTML' }
  }

  if (featureCount >= 1 && consistencyScore > 0.5) {
    // At least one consistent feature = prefer convert with fallback to clean
    return { type: 'markdown_html', confidence: 0.75, reason: 'Markdown detected - likely for conversion' }
  }

  // Default to clean text if markdown is sparse or inconsistent
  if (featureCount >= 1) {
    return { type: 'markdown_clean', confidence: 0.70, reason: 'Markdown detected - intent: clean/format text' }
  }
...
function detectMarkdownNoise(input) {
  ...
  // OCR-like spacing (spaces inside words)
  if (/\b\w+\s{1,2}\w+\s{1,2}\w+\s{1,2}\w+/.test(input)) {
    return true
  }

  // Repeated characters (***broken***, ____weird____, etc.)
  if (/\*{4,}|_{4,}|-{4,}/.test(input)) {
    return true
  }

  // Broken markdown symbols (unmatched emphasis)
  ...
  // Markdown symbols inside words (likely OCR artifact) - more aggressive
  const wordsWithMarkdown = (input.match(/\w[*_]\w/g) || []).length
  if (wordsWithMarkdown > 0) {
    return true
  }
  ...
  if (backtickCount % 2 !== 0 && backtickCount > 0) {
    return true
  }
  ...
  // Random mixed markdown markers (suggests copy-paste mess)
  const hasMultipleBrokenMarkers = (input.match(/[*_]{2,}[^*_\s]/g) || []).length > 2
  if (hasMultipleBrokenMarkers) {
    return true
  }

  // Text with stray asterisks at end of lines (common in copy-paste)
  if (/[a-zA-Z]\s+\*+\s*$/m.test(input)) {
    return true
  }

  // Heading-like but no space after # (common mistake)
...
function checkMarkdownConsistency(input, lines, features) {
  ...
  if (listLines.length >= 2) {
    const allSameLevel = listLines.every(l => l.match(/^[-*]\s/))
    if (allSameLevel) consistencyScore += 0.3
  }
  ...
  if (headingLines.length >= 2) {
    ...
    if (isValidHierarchy) consistencyScore += 0.3
  }

  // Check if emphasis markers are properly paired
  const emphasis = input.match(/(\*{1,2}|_{1,2})[^*_]+?(\*{1,2}|_{1,2})/g) || []
  if (emphasis.length >= 1) {
    const properlyPaired = emphasis.every(e => {
      const parts = e.split(/(\*{1,2}|_{1,2})/)
      return parts[1] === parts[parts.length - 2]
    })
    if (properlyPaired) consistencyScore += 0.2
  }
...
function calculateMarkdownDensity(input, featureCount) {
  const markdownChars = (input.match(/[*_#`\->\[\]()!|]/g) || []).length
  const totalChars = input.replace(/\s/g, '').length

  if (totalChars === 0) return 0

  const charRatio = markdownChars / totalChars
  const featureRatio = featureCount > 0 ? Math.min(featureCount / 5, 1) : 0

  return (charRatio + featureRatio) / 2
}
