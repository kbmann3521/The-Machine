...
export function hardDetect(input) {
  ...
  if (patterns.timestamp_unix.test(trimmed) && trimmed.length >= 10) {
    ...
    // Only match as timestamp if within ±50 years from now
    ...
    // If it's a positive integer that could be an IP integer (0 to 4,294,967,295)
    if (ts >= 0 && ts <= 4294967295) {
      return { type: 'potential_ip_integer', confidence: 0.85, reason: 'Integer detected (could be IP address in integer form)' }
    }
  }
