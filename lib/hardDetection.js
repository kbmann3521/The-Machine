...
export function hardDetectMatrix(input) {
  ...
  if (patterns.csv.test(trimmed) && !patterns.sql.test(trimmed)) {
    // Additional check: CSV should have simple values, not SQL keywords
    const sqlKeywords = /(SELECT|FROM|WHERE|JOIN|GROUP|ORDER|LIMIT|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER|TRUNCATE|WITH|UNION|HAVING|ON|AND|OR|NOT|CASE|WHEN|THEN|END)/i
