...
function analyzeMarkdownIntent(input) {
  ...
  // Final decision logic - stricter rules to avoid false positives

  // STRONG SIGNAL: Multiple distinct features with high consistency
  if (featureCount >= 3 && consistencyScore > 0.65) {
    return { type: 'markdown_html', confidence: 0.92, reason: 'Structured markdown detected - intent: convert to HTML' }
  }

  // STRONG SIGNAL: Tables always mean conversion
  if (features.tables > 0) {
    return { type: 'markdown_html', confidence: 0.91, reason: 'Markdown table detected - intent: convert to HTML' }
  }

  // STRONG SIGNAL: Code fences mean conversion
  if (features.codeFences) {
    return { type: 'markdown_html', confidence: 0.90, reason: 'Code block detected - intent: convert to HTML' }
  }

  // STRONG SIGNAL: Headings + lists = documentation
  if (features.headings && (features.lists || features.orderedLists)) {
    return { type: 'markdown_html', confidence: 0.89, reason: 'Documentation-style markdown - intent: convert to HTML' }
  }

  // WEAK SIGNAL: Looks like documentation with consistent structure
  if (looksLikeDocumentation && featureCount >= 1 && consistencyScore > 0.5) {
    return { type: 'markdown_html', confidence: 0.80, reason: 'Documentation pattern detected - intent: convert to HTML' }
  }

  // WEAK SIGNAL: Some markdown but not very consistent
  if (featureCount >= 2 && consistencyScore > 0.4 && densityScore > 0.15) {
    return { type: 'markdown_html', confidence: 0.72, reason: 'Some markdown structure - likely for conversion' }
  }

  // DEFAULT: Sparse or inconsistent markdown = clean/fix text
  if (featureCount >= 1) {
    return { type: 'markdown_clean', confidence: 0.75, reason: 'Markdown symbols detected but sparse/inconsistent - intent: clean text' }
  }
