...
export const patterns = {
  ...
  uuid_like: /^[0-9a-f]{8}-[0-9a-f\w]+-[0-9a-f\w]+-[0-9a-f\w]+-[0-9a-f\w]+$/i,

  uuid_with_invalid_chars: /^[0-9a-f]{8}-[0-9a-fx\w]+-[0-9a-fx\w]+-[0-9a-fx\w]+-[0-9a-fx\w]+$/i,
...
export function hardDetectMatrix(input) {
  ...
  } else if (patterns.uuid_standard.test(trimmed)) {
    add('uuid', 0.99, 'Standard UUID format detected')
  } else if (patterns.uuid_no_hyphens.test(trimmed)) {
    add('uuid', 0.99, 'Hex UUID format (no hyphens) detected')
  } else if (patterns.uuid_like.test(trimmed)) {
    add('uuid', 0.92, 'UUID-like format detected (may be invalid)')
  }
