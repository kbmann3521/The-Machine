...
export function getToolBiasWeight(toolId, inputType, rawInput = '') {
  ...
  // Barcode generator should be prioritized for CSV data if it contains barcode-like headers
  if (toolId === 'barcode-generator' && inputType === 'csv') {
    if (/\b(upc|barcode|sku|ean)\b/i.test(rawInput)) {
      return 0.30
    }
  }
