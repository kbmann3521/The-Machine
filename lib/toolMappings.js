...
export function getToolBaseWeight(toolId, inputType) {
  ...
  // HTML Entities Converter: When html_entities is detected, prioritize the entities converter
  // and penalize HTML/Markdown formatters since the input is text with entities, not markup
  if (inputType === 'html_entities') {
    if (toolId === 'html-entities-converter') return 0.95
    if (toolId === 'markdown-html-formatter') return 0.10
    if (toolId === 'html-formatter') return 0.10
    return 0.10
  }
