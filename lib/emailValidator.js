...
function validateSingleEmail(trimmedEmail) {
  ...
  let usernameRiskLevel = 'Mild'
  if (usernameRiskScore >= 30) {
    usernamePenalty = -20
    usernameRiskLevel = 'High Risk'
  } else if (usernameRiskScore >= 20) {
    usernamePenalty = -15
    usernameRiskLevel = 'Risky'
  ...
  if (details.issues.length > 0) {
    ...
    if (usernamePenalty < 0) {
      ...
      details.penalties.push({ issue: `Username patterns (${usernameRiskLevel})`, deduction: Math.abs(usernamePenalty) })
    }
    ...
    if (domainLower === 'gmail.com') {
      ...
      details.bonuses.push({ feature: 'iCloud (trusted provider)', bonus: 4 })
    }
    ...
    // Tier 3: Older/legacy providers
      ...
      details.penalties.push({ issue: 'AOL (legacy provider)', deduction: 3 })
    }
    ...
    // Subdomain penalty: only apply to true subdomains (3+ labels before TLD)
    // Common second-level domains like .co.uk should NOT be penalized
    const domainParts = domain.split('.')
    // Check if domain has 3+ parts (e.g., mail.dept.company.com has 4 parts)
    // For true subdomains: need at least 3 labels before TLD
    // Examples: example.com (2 parts - no penalty), mail.example.com (3 parts - no penalty),
    // dept.mail.example.com (4 parts - YES penalty), example.co.uk (3 parts - no penalty because .co.uk is SLD)
    ...
    const subdominainLabelCount = isCommonSLD ? domainParts.length - 2 : domainParts.length - 1

    if (subdominainLabelCount >= 3) {
      score = Math.max(0, score - 3)
  ...
  // Map final score to meaning (updated ranges for better accuracy)
  if (score === 0) {
    details.deliverabilityMeaning = 'Guaranteed bad / invalid'
  } else if (score >= 95) {
    details.deliverabilityMeaning = 'Excellent (very high deliverability)'
  } else if (score >= 85) {
    details.deliverabilityMeaning = 'Strong'
  } else if (score >= 70) {
    details.deliverabilityMeaning = 'Likely deliverable'
  } else if (score >= 50) {
    details.deliverabilityMeaning = 'Borderline (may bounce)'
  } else if (score >= 20) {
    details.deliverabilityMeaning = 'High risk / low trust'
  } else {
  ...
  details.isInvalid = isDisposable  // Track if invalid due to disposable
