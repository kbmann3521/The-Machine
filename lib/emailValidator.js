...
const abusiveTerms = [
  ...
  'dickhead', 'dickwad', 'prick', 'cockhead', 'ballsack',
  'tool', // common insult
...
const nsfwTerms = [
  ...
  'sexy', 'hot', 'horny', 'thirsty', 'kinky', 'fetish',
  'bdsm', 'dom', 'domme', 'sub', 'submissive',

  // ==========================================================
  // BASIC SEXUAL TERMS
  // ==========================================================
  'sex', 'sexual', 'nude', 'naked', 'strip', 'stripper',
  'camgirl', 'camboy', 'webcam', 'onlyfans', 'fansly',

  // ==========================================================
  // PORN / ADULT GENRES (clean + common)
  // ==========================================================
  'porn', 'porno', 'pornhub', 'xxx', 'hentai', 'ecchi',
  'yuri', 'yaoi', 'milf', 'gilf', 'bbw', 'bj', 'amateur',
  'solo', 'fetish', 'latex', 'bondage', 'cosplay', 'roleplay',
  ...
  // ==========================================================
  'penis', 'dick', 'cock', 'shaft', 'pecker',
  'balls', 'ballbag', 'nutsack',

  'vagina', 'pussy', 'coochie', 'cooch', 'cooter',
  'clit', 'clitoris',

  'boobs', 'tits', 'titties', 'rack', 'jugs', 'breasts',

  'ass', 'butt', 'booty', 'cheeks', 'thicc', 'thick',
  ...
  'anal', 'doggystyle', 'doggy', 'cowgirl', 'reversecowgirl',
  'missionary', '69', 'sixtynine',
  'sext', 'sexting',
...
function detectContentSeverity(localPart) {
  ...
  const nsfwTermsMatched = nsfwTerms.filter(term => normalized.includes(term) || localLower.includes(term))
...
function analyzeUsernameSemantics(localPart, domain = null) {
  ...
  if (mismatchedBrands.length > 0) {
    ...
    // Note: penalty is applied in calculateIdentityScore with gates
...
// Question: "Is there a strong reason NOT to send to this address?"
// Base score starts neutral at 50, subtracts for risk signals, adds small bonuses for positive signals
function calculateIdentityScore(options) {
  const {
    localPart,
    domain,
    semantics,
  ...
  // Start perfect (100), then accrue risk (penalties)
  // Bonuses reduce the total risk
  ...
  const breakdown = [
    { label: 'Base Score', points: 100, description: 'Perfect starting point for campaign safety' }
  ]

  // Hard fail: email cannot receive mail (but disposable is now just a penalty)
  ...
  // ============ RISK SIGNALS (Accrue penalties) ============

  // 0. Disposable/temporary email provider (35 point penalty)
  if (isDisposable) {
    const penalty = 35
    totalPenalties += penalty
    signals.negative.push('disposable-email')
    breakdown.push({ label: 'Disposable/Temporary Email', points: -penalty, description: 'Uses temporary or disposable email service — high churn risk' })
  }

  // 1. Compliance/abuse monitoring roles are campaign risks (-25)
  // Only penalize roles designed to receive complaints (abuse, security, postmaster, legal, etc.)
  // Normal business roles (sales, support, info, admin, etc.) are neutral
  const localLower = localPart.toLowerCase()
  ...
  if (isPenalizedRole) {
    const penalty = 25
    totalPenalties += penalty
    signals.negative.push('compliance-risk-role')
    breakdown.push({ label: 'Compliance/System Role', points: -penalty, description: 'Abuse monitoring or automated system mailbox — high bounce/complaint risk' })
  }

  // 2. Abusive/hateful/NSFW language (25 or 15)
  // Strong predictor of complaint/abuse behavior
  if (semantics.contentSeverity.isHateful) {
    const penalty = 25
    totalPenalties += penalty
    signals.negative.push('hateful-language')
    breakdown.push({ label: 'Hateful Language', points: -penalty, description: 'Contains hate speech — high spam/complaint risk' })
    // Show each matched hateful term
    if (semantics.hatefulTermsMatched && semantics.hatefulTermsMatched.length > 0) {
      for (const term of semantics.hatefulTermsMatched) {
        breakdown.push({ label: `  • "${term}"`, points: 0, description: '' })
      }
    }
  }
  ...
  if (semantics.contentSeverity.isAbusive) {
    const penalty = 25
    totalPenalties += penalty
    signals.negative.push('abusive-language')
    breakdown.push({ label: 'Abusive Language', points: -penalty, description: 'Contains profanity/hostile terms — elevated spam/abuse risk' })
    // Show each matched abusive term
  ...
  if (semantics.contentSeverity.isNsfw) {
    const penalty = 15
    totalPenalties += penalty
    signals.negative.push('adult-content')
    breakdown.push({ label: 'Adult Content', points: -penalty, description: 'Contains explicit terms — moderate abuse risk' })
    // Show each matched NSFW term
  ...
  // 3. Gibberish patterns (individual penalties)
  // Each detected pattern adds to risk
  // All gibberish contributes to risk, not just above a threshold
  if (gibberishBreakdown && gibberishBreakdown.length > 0) {
    breakdown.push({ label: 'Gibberish Patterns', points: 0, description: '' }) // Subheader with 0 points (just a label)
    for (const item of gibberishBreakdown) {
      totalPenalties += item.points
      breakdown.push({ label: `  • ${item.pattern}`, points: -item.points, description: '' })
    }
    signals.negative.push('gibberish-detected')
  }

  // 4. Excessive length (>30 chars) (-10)
  // Often indicator of spam trap or auto-generated address
  if (localPart.length > 30) {
    const penalty = 10
    totalPenalties += penalty
    signals.negative.push('excessive-length')
    breakdown.push({ label: 'Excessive Length', points: -penalty, description: 'Local part exceeds 30 characters — spam trap or auto-generated pattern' })
  }

  // 5. Brand-domain mismatch pattern (10 to 20)
  // Apply penalty if brand detected in localPart but not in domain
  // All penalties apply together (no blocking gates)
  if (semantics.isBrandImpersonation && semantics.brandImpersonationMatched && semantics.brandImpersonationMatched.length > 0) {
    // Penalty is 10 to 20 based on number of matched brands
    // More brands = slightly higher penalty
    const penalty = semantics.brandImpersonationMatched.length > 1 ? 20 : 15
    totalPenalties += penalty
    signals.negative.push('brand-domain-mismatch')
    breakdown.push({ label: 'Brand-Domain Mismatch', points: -penalty, description: 'Brand name appears but does not match email domain — elevated impersonation risk' })

    // Show each matched brand
    for (const brand of semantics.brandImpersonationMatched) {
      breakdown.push({ label: `  • ${brand}`, points: 0, description: '' })
    }
  }

  // ============ POSITIVE SIGNALS (Reduce risk) ============

  // 1. Major provider bonus (reduces risk by 10)
  // Gmail, Outlook, Yahoo have strong infrastructure and abuse handling
  const majorProviders = ['gmail.com', 'outlook.com', 'hotmail.com', 'yahoo.com', 'aol.com', 'icloud.com']
  if (majorProviders.includes(domain.toLowerCase())) {
    const bonus = 10
    totalBonuses += bonus
    signals.positive.push('major-provider')
    breakdown.push({ label: 'Major Provider', points: bonus, description: 'Major email provider (Gmail, Outlook, Yahoo) — reduces risk' })
  }

  // 2. Plus addressing / subaddressing (reduces risk by 5)
  // Indicates deliberate address crafting, less likely to be spam/bot
  if (localPart.includes('+')) {
    const bonus = 5
    totalBonuses += bonus
    signals.positive.push('plus-addressing')
    breakdown.push({ label: 'Plus Addressing', points: bonus, description: 'Uses plus addressing (user+tag@) — reduces risk' })
  }

  // 3. Corporate/business domain (reduces risk by 5)
  // Work emails have different risk profile (lower complaint risk)
  if (isDomainCorporate) {
    const bonus = 5
    totalBonuses += bonus
    signals.positive.push('corporate-domain')
    breakdown.push({ label: 'Corporate Domain', points: bonus, description: 'Corporate/business domain — reduces risk' })
  }

  // 4. High-trust TLD (reduces risk by 5)
  // Established TLDs correlate with legitimacy
  if (tldQuality === 'high-trust') {
    const bonus = 5
    totalBonuses += bonus
    signals.positive.push('high-trust-tld')
    breakdown.push({ label: 'High-Trust TLD', points: bonus, description: 'Established TLD (.com, .org, .edu, etc.) — reduces risk' })
  }

  // 5. Clean structure bonus (reduces risk by 5)
  // Simple, readable addresses with low gibberish are less likely to be automated/bot
  if (localPart.length <= 16 && /^[a-zA-Z0-9._+-]*$/.test(localPart) && gibberishScore < 15) {
    const bonus = 5
    totalBonuses += bonus
    signals.positive.push('clean-structure')
    breakdown.push({ label: 'Clean Structure', points: bonus, description: 'Short, simple local part with low gibberish — reduces risk' })
  }

  // ============ FINALIZE SCORE ============

  // Calculate final score: 100 - (totalPenalties - totalBonuses)
  const netRisk = totalPenalties - totalBonuses
  let safetyScore = 100 - netRisk

  // Clamp to 0-100 range
  const clampedScore = Math.max(0, Math.min(100, safetyScore))

  // Map to campaign readiness label
  // With 100-based system: 100 is perfect, 0 is worst
  let label = 'Poor'
  if (clampedScore >= 85) label = 'Excellent'
  else if (clampedScore >= 70) label = 'Good'
  else if (clampedScore >= 50) label = 'Risky'
  else label = 'Poor'

  return {
    identityScore: Math.round(clampedScore),
    signals,
    label,
    breakdown
  }
...
async function validateSingleEmail(trimmedEmail) {
  ...
  // DISPOSABLE DOMAINS (using manual list only - not MailChecker which treats disposable as invalid)
  // We detect disposable separately from validity, so disposable emails can still be valid
  ...
  // Disposable emails are no longer hard-fails
  // They continue through normal validation and get a -35 penalty in campaign readiness
  if (isDisposable) {
    if (!deliverabilityWarnings.includes('Disposable/temporary email domain')) {
      deliverabilityWarnings.push('Disposable/temporary email domain')
    }
    if (!trustworthinessWarnings.includes('Disposable email domain')) {
      trustworthinessWarnings.push('Disposable email domain')
    }
  }
  ...
  if (isKeyboardWalk) {
    const keyboardWalkPoints = 15
    usernameRiskScore = Math.max(usernameRiskScore, keyboardWalkPoints)
    ...
    gibberishBreakdown.push({ pattern: 'Keyboard walk', points: keyboardWalkPoints })
  }
  ...
  if (
    ...
    if (vowelRatioLocal < 0.15) {
      ...
      gibberishBreakdown.push({ pattern: 'Low vowel ratio', points: 8 }) // 4 * 2 multiplier
    }

    if (specialCharsInLocal >= 2) {
      usernameRiskScore += 3
      usernameHeuristics.push('Multiple special characters')
      gibberishBreakdown.push({ pattern: 'Multiple special chars', points: 3 })
    }

    if (entropyIndicators >= 3) {
      entropyPoints = 5
      usernameHeuristics.push('High character entropy')
      gibberishBreakdown.push({ pattern: 'High entropy indicators', points: 5 })
    }

    if (localPart.length > 32) {
      usernameRiskScore += 5
      usernameHeuristics.push('Exceptionally long username')
      gibberishBreakdown.push({ pattern: 'Exceptionally long', points: 5 })
    ...
    if (/^\d+$/.test(localPart)) {
      ...
      gibberishBreakdown.push({ pattern: 'Numbers-only username', points: 12 })
      trustworthinessScore = Math.max(0, trustworthinessScore - 40)
    }
    ...
    if (!semantics.isRoleBasedEmail && clearBotPatterns.some(pattern => localLower.includes(pattern))) {
      ...
      gibberishBreakdown.push({ pattern: 'Bot/automated patterns', points: 6 })
    }

    if (/(.)\1{2,}/.test(localPart)) {
      repeatPoints = 3
      usernameHeuristics.push('Repeated characters detected')
      gibberishBreakdown.push({ pattern: 'Repeated characters', points: 3 })
      trustworthinessScore = Math.max(0, trustworthinessScore - 20)
    }
    ...
    if (!skipClusterCheck) {
      for (let i = 0; i <= localLower.length - 5; i++) {
        ...
        if (consonantCount >= 4) {
          clusterPoints = 6
          gibberishBreakdown.push({ pattern: 'High consonant cluster', points: 12 }) // 6 * 2 multiplier
          break
        }
      }
    }
    ...
    // Track combined high entropy + low vowels pattern
    if (entropyIndicators >= 3 && vowelRatioLocal < 0.18) {
      gibberishBreakdown.push({ pattern: 'High entropy + low vowels', points: 15 })
    }
    ...
    const estimatedGibberishScore = Math.min(100, Math.round((usernameRiskScore / 40) * 100))

    if (estimatedGibberishScore >= 50 || isBotLike) {
      usernameTrustPenalty -= 50
      trustworthinessWarnings.push('Username appears randomly generated (extreme gibberish)')
    }
    if (estimatedGibberishScore >= 40) {
      usernameTrustPenalty -= 30
      if (!trustworthinessWarnings.some(w => w.includes('randomly generated'))) {
        trustworthinessWarnings.push('Username appears randomly generated')
      }
    }
    if (estimatedGibberishScore >= 25) {
      usernameTrustPenalty -= 15
      if (!trustworthinessWarnings.some(w => w.includes('suspicious'))) {
  ...
  if (!finalValid) {
    ...
    if (phishingRisk === 'Very High') {
      combinedGrade = 'F'
    } else if (abusiveScore >= 60) {
      // Hateful content gets F grade
      combinedGrade = 'F'
    } else if (abusiveScore >= 30) {
      // Abusive content gets downgrade but not F
      combinedGrade = downgradeGrade(combinedGrade)
    } else if (abusiveScore >= 5) {
      // NSFW content gets minor downgrade
      combinedGrade = downgradeGrade(combinedGrade)
  ...
  // Calculate Identity Score for campaign readiness
  const { identityScore, signals: identitySignals, label: campaignReadiness, breakdown: identityBreakdown } = calculateIdentityScore({
    localPart,
    domain,
    semantics,
    isRoleBasedEmail,
  ...
  return {
    ...
    // Domain mail status (placeholder - will be populated by DNS check)
    domainMailStatus: getDomainMailStatus(domain),

    // Deliverability metrics
    deliverabilityScore,
    deliverabilityLabel,

    // NEW: Campaign readiness (replaces multiple overlapping scores)
    identityScore,
    campaignReadiness,
    identitySignals,
    identityBreakdown,
    ...
    warnings: [...deliverabilityWarnings, ...trustworthinessWarnings],

    // Legacy/additional metadata (kept for backwards compatibility)
    tldQuality,
    hasSyntaxError,
    businessEmail,

    // Note: Consider these deprecated - replaced by identityScore and identitySignals
...
export async function emailValidator(text) {
  ...
  if (emails.length === 0) {
    return {
      ...
      dnsCheckAvailable: dns !== undefined,
  ...
  const averageIdentityScore = avg('identityScore')

  // Map average identity score to campaign readiness label
  let averageCampaignReadiness = 'Poor'
  if (averageIdentityScore >= 85) averageCampaignReadiness = 'Excellent'
  else if (averageIdentityScore >= 70) averageCampaignReadiness = 'Good'
  else if (averageIdentityScore >= 50) averageCampaignReadiness = 'Risky'
