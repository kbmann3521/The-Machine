...
function validateSingleEmail(trimmedEmail) {
  ...
  // Domain-sensitivity scoring: amplify penalties for high-trust domains with abusive/NSFW content
  ...
  if (semantics.semanticPenalty < 0) {
    ...
    // High-trust TLDs amplify penalties for abnormal content
    if (tldQuality === 'high-trust' && semantics.contentSeverity &&
    ...
    // Corporate domains add additional amplification
    else if (isDomainCorporate && semantics.contentSeverity &&
        (semantics.contentSeverity.isAbusive || semantics.contentSeverity.isNsfw)) {
      multiplier = 1.3 // 30% stronger penalty for corporate domains
    }
