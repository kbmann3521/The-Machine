...
// FIX #F: Added missing corporate domains (Amazon, PayPal, Google, Instagram, Bank of America)
const corporateProviderDomains = new Set([
  ...
  'airbnb.com', 'netflix.com', 'dropbox.com', 'asana.com',
  'paypal.com', 'instagram.com', 'bankofamerica.com', 'bofa.com'
])
...
const brandImpersonationPatterns = [
  ...
  'notion', 'slack', 'discord', 'bank', 'bofa', 'bankofamerica'
]
...
function mapHumanLikelihoodLabel(trustScore, isInvalid = false, isCorporateEmail = false, hasAbusiveContent = false, hasStrongImpersonation = false) {
  ...
  // Abusive content = human but abusive
...
function validateSingleEmail(trimmedEmail) {
  ...
  if (!semantics.isLikelyPersonalName && !semantics.isUnicodeUsername && !semantics.isStandardSystemEmail) {
    ...
    // FIX G: Require BOTH conditions before flagging as gibberish
    const isStrongGibberish =
      (repeatedTrigrams >= 3 && vowelRatioLocal < 0.22) ||
      (entropyIndicators >= 3 && vowelRatioLocal < 0.20)
    ...
    // FIX #A: Only check consonant clusters for actual gibberish, not brand patterns or names
    if (!semantics.isRoleBasedEmail && !semantics.isBrandImpersonation && !skipClusterCheck) {
