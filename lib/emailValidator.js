...
function validateSingleEmail(trimmedEmail) {
  ...
  if (!semantics.isLikelyPersonalName && !semantics.isUnicodeUsername && !semantics.isStandardSystemEmail) {
    ...
    // FIX #A: Only check consonant clusters for actual gibberish, not brand patterns
    if (!semantics.isRoleBasedEmail && !semantics.isBrandImpersonation) {
  ...
  // FIX #4: If there are warnings or heuristics, trustworthiness cannot be perfect (100)
