...
function hasAbusiveContent(localPart) {
  ...
  return abusiveTerms.some(term => normalized.includes(term) || localLower.includes(term))
}

// Adult / negative themes
...
function analyzeUsernameSemantics(localPart, domain = null) {
  ...
  if (isBrandLike) {
    ...
    if (isStrongBrandImpersonation) {
      insights.semanticPenalty -= 75
    } else {
      insights.semanticPenalty -= 50
    }
...
function validateSingleEmail(trimmedEmail) {
  ...
  if (
    ...
    if (/^\d+$/.test(localPart)) {
      usernameRiskScore += 8
      usernameHeuristics.push('Numbers-only username')
      trustworthinessScore = Math.max(0, trustworthinessScore - 25)
    }
  ...
  // SCORES
  const abusiveScore = semantics.hasAbusiveContent ? 100 : 0
