...
const nsfwTerms = [
  ...
  // Porn categories (clean, non-graphic list of common tags)
  ...
  // NSFW slang (safe)
  'thirsttrap', 'baddie', 'insta-baddie', 'babygirl',
...
const negativeAdultTerms = [
  'nasty', 'dirty', 'kinky', 'fetish',
  'slut', 'whore', 'escort', 'xxx',
  'porn', 'explicit', 'nsfw', 'horny', 'sexy'
]
...
function analyzeUsernameSemantics(localPart, domain = null) {
  ...
  if (severity.isHateful) {
    ...
    // Abusive: 30-50 penalty, increases with concentration
    const basePenalty = 30
    const concentrationBonus = (severity.abusiveCount - 1) * 10
    insights.semanticPenalty -= Math.min(basePenalty + concentrationBonus, 50)
    insights.warnings.push('Contains abusive or hostile language')
  } else if (severity.isNsfw) {
    // NSFW: 5-20 penalty, increases with concentration
    const basePenalty = 5
    const concentrationBonus = (severity.nsfwCount - 1) * 5
    insights.semanticPenalty -= Math.min(basePenalty + concentrationBonus, 20)
...
function validateSingleEmail(trimmedEmail) {
  ...
  // Domain-sensitivity scoring: corporate domains with abnormal content get trust score caps
  const isDomainCorporate = corporateProviderDomains.has(domainLower)
  ...
  if (isDomainCorporate && semantics.contentSeverity) {
    if (semantics.contentSeverity.isHateful) {
      // Hateful slurs on corporate domain: almost impossible (cap at 15)
      trustworthinessScore = Math.min(trustworthinessScore, 15)
    } else if (semantics.contentSeverity.isAbusive) {
      // Abusive language on corporate domain: impossible (cap at 25)
      trustworthinessScore = Math.min(trustworthinessScore, 25)
    } else if (semantics.contentSeverity.isNsfw) {
      // NSFW/adult on corporate domain: extremely rare (cap at 35)
      trustworthinessScore = Math.min(trustworthinessScore, 35)
    }
  }

  // Now apply semantic penalties with multipliers for remaining high-trust domains
  ...
  if (semantics.semanticPenalty < 0) {
    ...
    if (!isDomainCorporate && tldQuality === 'high-trust' && semantics.contentSeverity &&
