...
function analyzeUsernameSemantics(localPart) {
  ...
  if (offensiveTerms.some(term => localLower.includes(term))) {
    ...
    insights.semanticPenalty -= 25
  }

  // Check for brand impersonation
  if (brandImpersonationPatterns.some(brand => localLower.includes(brand))) {
...
function mapHumanLikelihoodLabel(trustScore) {
...
function validateSingleEmail(trimmedEmail) {
  ...
  if (semantics.isLikelyPersonalName) {
    ...
    details.trustworthinessWarnings.push('Likely personal name pattern (firstname.lastname)')
  }
  if (semantics.hasOffensiveTerms) {
    ...
    details.trustworthinessWarnings.push('Contains offensive or inappropriate terms')
  }
  if (semantics.isBrandImpersonation) {
    details.trustworthinessScore = Math.max(0, details.trustworthinessScore + semantics.semanticPenalty)
    details.trustworthinessWarnings.push('Appears to impersonate known brand')
  }
  ...
  details.humanLikelihood = mapHumanLikelihoodLabel(details.trustworthinessScore)
