...
function validateSingleEmail(trimmedEmail) {
  ...
  // Domain-sensitivity scoring: amplify penalties for corporate domains with abusive/NSFW content
  let adjustedSemanticPenalty = semantics.semanticPenalty
  if (semantics.semanticPenalty < 0) {
    // If corporate domain and has content issues, apply 1.3x multiplier
    const isDomainCorporate = corporateProviderDomains.has(domainLower) ||
                               businessDomainKeywords.some(kw => domainRoot.includes(kw))

    if (isDomainCorporate && semantics.contentSeverity &&
        (semantics.contentSeverity.isAbusive || semantics.contentSeverity.isNsfw)) {
      // Corporate domains should have stricter standards - amplify penalty by 30%
      const corporateMultiplier = 1.3
      adjustedSemanticPenalty = Math.round(semantics.semanticPenalty * corporateMultiplier)
    }
