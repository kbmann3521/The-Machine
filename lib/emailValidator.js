...
// FIX #4: Keyboard walk patterns (qwerty sequences)
const keyboardWalks = ['qwerty', 'qwertyuiop', 'asdfgh', 'asdfghjkl', 'zxcvbn', 'zxcvbnm', 'poiuyt', 'lkjhgf', 'mnbvcx', 'qazwsx']

// FIX #1: Negative/adult-themed words (substring-based detection)
...
// FIX #1: Check for adult/negative-themed content (substring-based)
function hasAdultOrNegativeThemes(localPart) {
  const localLower = localPart.toLowerCase()

  // Never flag system emails
  if (standardSystemEmails.has(localLower)) return false

  // Use substring matching for negative adult terms
...
function analyzeUsernameSemantics(localPart, domain = null) {
  ...
  // FIX #1: Check for adult/negative themes (substring-based, not abusive)
  if (hasAdultOrNegativeThemes(localPart) && !insights.hasAbusiveContent) {
    insights.semanticPenalty -= 20
    insights.warnings.push('Contains adult or explicit themes')
  }
  ...
  if (hasAdultContent && !insights.hasAbusiveContent && !hasAdultOrNegativeThemes(localPart)) {
    insights.semanticPenalty -= 15
    insights.warnings.push('Contains adult-oriented content')
  }
...
function validateSingleEmail(trimmedEmail) {
  ...
  // FIX #4: Check for keyboard walk patterns EARLY (before gibberish detection)
  const isKeyboardWalk = keyboardWalks.some(seq => localLower.includes(seq))
  if (isKeyboardWalk) {
    details.usernameRiskScore = Math.max(details.usernameRiskScore || 0, 15)
    details.trustworthinessScore = Math.max(0, details.trustworthinessScore - 20)
    if (!details.usernameHeuristics.includes('Username appears to be a keyboard sequence pattern')) {
      details.usernameHeuristics.push('Username appears to be a keyboard sequence pattern')
    }
  }

  if (!semantics.isLikelyPersonalName && !semantics.isUnicodeUsername && !semantics.isStandardSystemEmail && !isMarketingName && !startsWithAcronym && !isKeyboardWalk) {
  ...
  if (!details.valid || details.isInvalid || details.hasSyntaxError) {
    ...
    // FIX #5: If impersonation + freemail + role-based = very high risk (likely phishing)
    ...
    if (semantics.isBrandImpersonation && isFreemail && isRoleBasedEmail) {
      ...
      details.trustworthinessWarnings.push('Brand impersonation on free email with role-based pattern (likely phishing)')
    }
