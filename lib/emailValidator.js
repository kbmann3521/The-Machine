...
function detectContentSeverity(localPart) {
  ...
  if (standardSystemEmails.has(localLower)) {
    return { isHateful: false, isAbusive: false, isNsfw: false, hatefulCount: 0, abusiveCount: 0, nsfwCount: 0 }
  }

  // Don't penalize slang whitelist (it just bypasses gibberish detection)
  const normalized = localLower.replace(/\d/g, '')

  // Count matches for concentration-based penalties
  const hatefulCount = hatefulTerms.filter(term => normalized.includes(term) || localLower.includes(term)).length
  const abusiveCount = abusiveTerms.filter(term => normalized.includes(term) || localLower.includes(term)).length
  const nsfwCount = nsfwTerms.filter(term => normalized.includes(term) || localLower.includes(term)).length

  return {
    isHateful: hatefulCount > 0,
    isAbusive: abusiveCount > 0,
    isNsfw: nsfwCount > 0,
    hatefulCount,
    abusiveCount,
    nsfwCount
...
function calculateIdentityScore(options) {
  ...
  const breakdown = [
    { label: 'Base Score', points: 50, description: 'Starting neutral score' }
  ]

  // If email cannot receive mail, identity score is irrelevant
  if (deliverabilityScore === 0 || isDisposable) {
    return {
      identityScore: 0,
      signals,
      label: 'Poor',
      breakdown: [{ label: 'Not Deliverable', points: -50, description: 'Email cannot receive mail' }]
    }
  }

  // POSITIVE SIGNALS

  // 1. Personal name structure (+25 max)
  if (semantics.isLikelyPersonalName) {
    identityScore += 25
    signals.positive.push('personal-name-pattern')
    breakdown.push({ label: 'Personal Name Pattern', points: 25, description: 'Name-based local part (high confidence)' })
  }

  // 2. Role-based / function mailbox (+20 max)
  if (isRoleBasedEmail) {
    identityScore += 20
    signals.positive.push('role-mailbox')
    breakdown.push({ label: 'Role-Based Email', points: 20, description: 'Functional mailbox (info, support, etc.)' })
  }

  // 3. Structure simplicity (+10 max)
  if (localPart.length <= 16 && /^[a-zA-Z0-9._-]*$/.test(localPart)) {
    const simpleScore = 10
    identityScore += simpleScore
    signals.positive.push('simple-structure')
    breakdown.push({ label: 'Simple Structure', points: 10, description: 'Clean, short local part' })
  }

  // 4. Domain credibility (+10 max)
  if (isDomainCorporate) {
    identityScore += 10
    signals.positive.push('corporate-domain')
    breakdown.push({ label: 'Corporate Domain', points: 10, description: 'Verified business domain' })
  } else if (tldQuality === 'high-trust') {
    identityScore += 6
    signals.positive.push('high-trust-tld')
    breakdown.push({ label: 'High-Trust TLD', points: 6, description: 'Established TLD (.com, .org, .edu, etc.)' })
  }

  // NEGATIVE SIGNALS

  // 1. Narrative / expressive language (-30 max)
  if (semantics.contentSeverity) {
    if (semantics.contentSeverity.isHateful) {
      identityScore -= 30
      signals.negative.push('hateful-language')
      breakdown.push({ label: 'Hateful Language', points: -30, description: 'Contains hate speech or slurs' })
    } else if (semantics.contentSeverity.isAbusive) {
      identityScore -= 20
      signals.negative.push('abusive-language')
      breakdown.push({ label: 'Abusive Language', points: -20, description: 'Contains profanity or hostile content' })
    } else if (semantics.contentSeverity.isNsfw) {
      identityScore -= 10
      signals.negative.push('adult-content')
      breakdown.push({ label: 'Adult Content', points: -10, description: 'Contains explicit or NSFW terms' })
    }
  }

  // 2. Sentence-like structure (-20)
  // Check for multiple dictionary words that form phrases
  const words = localPart.toLowerCase().split(/[._-]/).filter(w => w.length > 2)
  if (words.length >= 3) {
    identityScore -= 15
    signals.negative.push('phrase-like-structure')
    breakdown.push({ label: 'Phrase-Like Structure', points: -15, description: 'Multiple words forming a sentence' })
  }

  // 3. Excessive length (-15)
  if (localPart.length > 30) {
    identityScore -= 15
    signals.negative.push('excessive-length')
    breakdown.push({ label: 'Excessive Length', points: -15, description: 'Local part exceeds 30 characters' })
  }

  // 4. Stylization / gamer patterns (-10)
  if (/xX|Xx|_+/.test(localPart) || /\d{4,}/.test(localPart)) {
    identityScore -= 10
    signals.negative.push('stylization-pattern')
    breakdown.push({ label: 'Stylization Pattern', points: -10, description: 'Contains excessive numbers or gamer-style formatting' })
  }

  // 5. Brand impersonation (heavy penalty)
  if (semantics.isBrandImpersonation) {
    const penalty = semantics.isBrandImpersonationStrong ? -30 : -20
    identityScore += penalty
    signals.negative.push('brand-impersonation')
    breakdown.push({ label: 'Brand Impersonation', points: penalty, description: semantics.isBrandImpersonationStrong ? 'Strong brand impersonation pattern' : 'Possible brand impersonation' })
  }

  // Clamp to 0-100
  const clampedScore = Math.max(0, Math.min(100, identityScore))
  const clamped = clampedScore !== identityScore
  if (clamped) {
    breakdown.push({ label: 'Clamped to Range', points: clampedScore - identityScore, description: 'Score adjusted to valid range (0-100)' })
  }
...
async function validateSingleEmail(trimmedEmail) {
  ...
  // DISPOSABLE DOMAINS (using MailChecker's 55,000+ disposable domain database + manual fallback)
  let isDisposable = false

  // Check MailChecker's disposable domain database (much larger than manual list)
  if (typeof MailChecker !== 'undefined') {
    try {
      // MailChecker has built-in disposable domain detection
      // It maintains a comprehensive list of 55,000+ temporary/throwaway services
      isDisposable = !MailChecker.isValid(trimmedEmail, { checkDns: false })
    } catch (error) {
      // If MailChecker throws, continue to manual fallback
      isDisposable = false
    }
  }

  // Also check manual list (catches newer services not yet in MailChecker's database)
  // This ensures we catch services regardless of MailChecker's data freshness
  if (!isDisposable) {
