...
// EXPANDED ABUSIVE/OFFENSIVE WORD LIST (catches more variations)
const abusiveTerms = [
...
function calculatePhishingRisk(localPart, domain, isRoleBasedEmail, isCorporateDomain, hasAbusiveContent, hasBrandImpersonation) {
  ...
  if (!isRoleBasedEmail) return 'Low'

  // Role-based + corporate domain = very high phishing risk
  if (isCorporateDomain) return 'Very High'

  // Role-based + freemail = medium-high phishing risk
  return 'High'
}
...
function validateSingleEmail(trimmedEmail) {
  ...
  if (details.hasSyntaxError) return details
  ...
  if (details.isInvalid) return details
  ...
  // 5. ANALYZE USERNAME SEMANTICS (BEFORE role detection)
  ...
  const isCorporateDomain = isCorporateDomain(domainLower)
  if (businessProvider) {
  ...
  // 8. GIBBERISH DETECTION (SKIP for personal names)
