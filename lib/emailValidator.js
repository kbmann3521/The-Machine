...
function validateSingleEmail(trimmedEmail) {
  ...
  // DISPOSABLE DOMAINS (using MailChecker's 55,000+ disposable domain database)
  ...
  if (typeof MailChecker !== 'undefined') {
    try {
      ...
      // Fallback to manual list if MailChecker fails
      isDisposable = knownDisposableDomains.some(disposable => domainLower.includes(disposable))
    }
  } else {
    // Fallback to manual list if MailChecker not available
    isDisposable = knownDisposableDomains.some(disposable => domainLower.includes(disposable))
  }
