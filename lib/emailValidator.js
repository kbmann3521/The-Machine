...
function calculateIdentityScore(options) {
  ...
  // NOTE: Major provider detection now happens via MX record analysis in the component
  // (calculateDnsRecordPenalties → getMxProvider) so we don't double-count here
...
async function validateSingleEmail(trimmedEmail) {
  const baseInvalid = (issues, opts = {}) => ({
    ...
    isDisposable: false,
    identityScore: 0,
    identitySignals: { positive: [], negative: [] },
    identityBreakdown: [{ label: 'Not Deliverable', points: -100, description: 'Email is unreachable' }],
  ...
  return {
    ...
    usernameHeuristics,
    domainHeuristics: [...(semantics.domainHeuristics || [])],
