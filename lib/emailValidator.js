...
function validateSingleEmail(trimmedEmail) {
  ...
  // HIGH-RISK PENALTIES (direct deliverability failure - drop to 0)
  if (details.issues.length > 0) {
    ...
    // Tier 1: Premium providers (+5, +3, +3)
    if (domainLower === 'gmail.com') {
      ...
      details.bonuses.push({ feature: 'Outlook/Hotmail (premium provider)', bonus: 3 })
    } else if (domainLower === 'icloud.com') {
      score = Math.min(100, score + 3)
      details.bonuses.push({ feature: 'iCloud (premium provider)', bonus: 3 })
    }
    // Tier 2: Established providers (+2)
      ...
      // Zoho is legitimate, no penalty or bonus
    }
    // Tier 3: Legacy/Lower-trust providers
    else if (domainLower === 'aol.com') {
      score = Math.max(0, score - 3)
      details.penalties.push({ issue: 'AOL (legacy provider, higher bounce risk)', deduction: 3 })
    } else if (domainLower === 'protonmail.com' || domainLower === 'mail.protonmail.com') {
      score = Math.max(0, score - 5)
      details.penalties.push({ issue: 'ProtonMail (privacy-focused)', deduction: 5 })
    } else if (domainLower === 'gmx.com' || domainLower === 'mail.com') {
      score = Math.max(0, score - 10)
      details.penalties.push({ issue: 'GMX/Mail.com (lower reputation)', deduction: 10 })
  ...
  details.isInvalid = isDisposable
