...
function calculateIdentityScore(options) {
  ...
  // ============ STEP 1: COMPUTE DHS (Domain Health Score) ============
  const dhsResult = calculateDomainHealthScore({
    tldQuality,
    isDomainCorporate
  })
  const dhsScore = dhsResult.score
  const dhsBreakdown = dhsResult.breakdown

  // ============ STEP 2: COMPUTE LCS (Local-Part Credibility Score) ============
  ...
  // Final score = min(Domain Health, Local-Part Credibility)
  // This ensures bad identity cannot be rescued by good domain
  const finalScore = Math.min(dhsScore, lcsScore)
  ...
  // Show DHS components, LCS components, and cap applied
  const breakdown = [
    { label: 'Domain Health Score', points: dhsScore, description: 'Infrastructure & domain maturity (base: 77)', isSectionHeader: true }
  ]

  // Add DHS breakdown items
  for (const item of dhsBreakdown) {
    breakdown.push(item)
  }

  breakdown.push({ label: '', points: 0, description: '', isSeparator: true })
  breakdown.push({ label: 'Local-Part Credibility Score', points: lcsScore, description: 'Username quality & engagement likelihood (base: 100)', isSectionHeader: true })
  ...
  breakdown.push({ label: '', points: 0, description: '', isSeparator: true })
  breakdown.push({ label: 'Final Campaign Readiness Score', points: Math.round(clampedScore), description: 'min(Domain Health Score, Local-Part Credibility Score)' })

  // Determine signals from LCS (credibility) score
  const signals = {
    positive: [],
    negative: []
  }

  // Positive signals: no major LCS penalties
  if (lcsScore >= 85) {
    signals.positive.push('clean-identity')
  }
  if (!semantics.contentSeverity.isHateful && !semantics.contentSeverity.isAbusive) {
    signals.positive.push('no-hostile-language')
  }

  // Negative signals: from LCS breakdowns
  if (isDisposable) signals.negative.push('disposable-email')
  if (isPenalizedRole(localPart)) signals.negative.push('compliance-risk-role')
  if (semantics.contentSeverity.isHateful) signals.negative.push('hateful-language')
  if (semantics.contentSeverity.isAbusive) signals.negative.push('abusive-language')
  if (semantics.contentSeverity.isNsfw) signals.negative.push('adult-content')
  if (gibberishBreakdown && gibberishBreakdown.length > 0) signals.negative.push('gibberish-detected')
  if (localPart.length > 30) signals.negative.push('excessive-length')
  if (semantics.isBrandImpersonation) signals.negative.push('brand-domain-mismatch')
  ...
  return {
    ...
    label,
    breakdown,
    // AUTHORITATIVE SCORES - Frontend uses these directly without modification
    dhsScore: Math.round(dhsScore),      // Domain Health Score (infrastructure, base 75)
    lcsScore: Math.round(lcsScore),      // Local-Part Credibility Score (identity, base 100) - SINGLE SOURCE OF TRUTH
    lcsCap: Math.round(lcsScore)         // For compatibility: lcsCap = lcsScore (no bucketing)
  }
}
...
// Max: 100 (77 + 5 TLD + 5 corporate + 3 MX provider + 2 redundancy + 5 DMARC reject + 3 monitoring)
function calculateDomainHealthScore(options) {
  const { tldQuality, isDomainCorporate } = options
  ...
  if (tldQuality === 'high-trust') {
    dhs += 5
    breakdown.push({ label: 'High-Trust TLD', points: 5, description: 'Established TLD (.com, .org, .edu, etc.)' })
  } else if (tldQuality === 'low-trust') {
    dhs -= 10
    breakdown.push({ label: 'Low-Trust TLD', points: -10, description: 'Suspicious or uncommon TLD' })
  }

  // Corporate Domain
  if (isDomainCorporate) {
    dhs += 5
    breakdown.push({ label: 'Corporate Domain', points: 5, description: 'Verified corporate/business domain' })
  }

  return {
    score: Math.max(0, Math.min(100, dhs)),
    breakdown
  }
}
...
function calculateLocalPartCredibility(options) {
  ...
  if (localPart.length > 30) {
    lcs -= 10
    breakdown.push({ label: 'Excessive Length', points: -10, description: 'Local part exceeds 30 characters' })
  }

  // Gibberish patterns → -3 to -15 each
...
async function validateSingleEmail(trimmedEmail) {
  ...
  const {
    ...
    dhsScore,        // Domain Health Score
    lcsScore,        // Local-Part Credibility Score
    lcsCap           // Credibility cap (derived from LCS)
  ...
  return {
    ...
    valid: finalValid && !isDisposable,
    isInvalid: !finalValid || isDisposable,
    isDisposable,
    ...
    identitySignals,
    identityBreakdown,
    dhsScore,        // REQUIRED: Domain Health Score for component calculation
    lcsScore,        // REQUIRED: Local-Part Credibility Score (single source of truth)
    lcsCap,          // REQUIRED: Credibility cap for final score calculation
