...
// Throwaway/Test Email Detection
// These ALWAYS match if found anywhere (substring matching)
const throwawayWordsAlways = [
  'notreal',
  'fakeemail',
  'dummyemail',
  'invalidemail',
...
// These require word boundaries (like tit, dom, ass in NSFW detection)
// Boundary = start/end of string OR any non-letter character
const throwawayWordsBounded = [
  'fake',
  'test',
  'testing',
  'example',
...
// DHS/Gibberish Architecture: Calculate Campaign Readiness Score
// Step 1: Compute DHS (domain health) independently
// Step 2: Compute Gibberish Score (local-part gibberish detection) independently
// Step 3: Apply Gibberish Score → score cap
...
function calculateCampaignScore(options) {
  ...
  // ============ STEP 1: COMPUTE Gibberish Score (Local-Part Gibberish Score) ============
  // SINGLE SOURCE OF TRUTH: gibberishScore is computed here with ALL penalties applied
  // gibberishBreakdown must contain all penalties that should reduce Gibberish Score
  const gibberishResult = calculateLocalPartGibberishScore({
    localPart,
    semantics,
    isRoleBasedEmail,
    isDisposable,
  ...
  const localPartGibberishScore = gibberishResult.score  // <-- AUTHORITATIVE Gibberish Score VALUE
  const gibberishBreakdownItems = gibberishResult.breakdown

  // ============ STEP 2: COMPUTE CAMPAIGN SCORE ============
  // Frontend will calculate DHS and determine final score
  // For now, backend returns Gibberish Score as the campaign readiness base
  const campaignScore = localPartGibberishScore
  let label = 'Very Poor'
  ...
  // ============ BUILD BREAKDOWN ============
  // Frontend will add DHS section before Gibberish Score; backend only provides Gibberish Score breakdown
  const breakdown = [
    { label: 'Local-Part Gibberish Score', points: localPartGibberishScore, description: 'Username gibberish detection & engagement likelihood (base: 100)', isSectionHeader: true }
  ]

  // Add Gibberish breakdown items
  for (const item of gibberishBreakdownItems) {
    breakdown.push(item)
  }

  breakdown.push({ label: '', points: 0, description: '', isSeparator: true })
  breakdown.push({ label: 'Final Campaign Readiness Score', points: Math.round(campaignScore), description: 'Frontend will calculate: min(Domain Health Score, Local-Part Gibberish Score)' })
  ...
  return {
    ...
    lcsScore: Math.round(localPartGibberishScore)       // Local-Part Gibberish Score (gibberish detection, base 100) - SINGLE SOURCE OF TRUTH
  }
}
...
// Calculate Local-Part Gibberish Score
// Purpose: Username gibberish detection & engagement likelihood
// Base: 100 (assume credibility until patterns indicate otherwise)
// Gibberish Score only goes down, never up (except reaching max)
function calculateLocalPartGibberishScore(options) {
  const {
    localPart,
    semantics,
    isRoleBasedEmail,
  ...
  // NOTE: Hateful, Abusive, and NSFW content are NO LONGER penalized in LCS
  // These are flagged as warnings but do not affect the score
  ...
  // NOTE: Brand-domain mismatch is NO LONGER penalized in LCS
  // It is flagged as a warning but does not affect the score

  // ===== MEDIUM RISKS =====

  // NOTE: Adult/NSFW content is NO LONGER penalized in LCS
  // It is flagged as a warning but does not affect the score
...
async function validateSingleEmail(trimmedEmail, config = {}) {
  ...
  for (const label of domainLabels) {
    ...
    // Check for invalid characters in domain labels
    // Domain labels can only contain letters, digits, and hyphens
    if (!/^[a-zA-Z0-9-]*$/.test(label)) {
      tldIssues.push(`Domain label "${label}" contains invalid characters (only letters, digits, and hyphens allowed)`)
      isInvalid = true
    }
  }
  ...
  // BRAND-DOMAIN MISMATCH (moved from LCS to warnings)
  if (semantics.isBrandImpersonation && semantics.brandImpersonationMatched && semantics.brandImpersonationMatched.length > 0) {
    const brandsStr = semantics.brandImpersonationMatched.join(', ')
    trustworthinessWarnings.push(`Brand-domain mismatch (${brandsStr})`)
  }

  // THROWAWAY/TEST EMAIL DETECTION
  let isThrowaway = false
  let throwawayWordMatched = null

  // Check ALWAYS words (substring matching - no boundaries needed)
  for (const word of throwawayWordsAlways) {
    if (localLower.includes(word)) {
      isThrowaway = true
      throwawayWordMatched = word
      break
    }
  }

  // Check BOUNDED words (require word boundaries like tit, dom, ass)
  if (!isThrowaway) {
    for (const word of throwawayWordsBounded) {
      const boundaryRegex = new RegExp(`(^|[^a-z])${word}([^a-z]|$)`)
      if (boundaryRegex.test(localLower)) {
        isThrowaway = true
        throwawayWordMatched = word
        break
  ...
  const {
    ...
    lcsScore         // Local-Part Gibberish Score
  ...
  return {
    ...
    campaignReadiness,
    campaignBreakdown,
    lcsScore,        // Local-Part Gibberish Score (single source of truth)
    ...
    isThrowaway,
    throwawayWordMatched,

    // Issues and warnings
    issues: [...issues, ...(isThrowaway ? [`Throwaway/test email pattern detected ("${throwawayWordMatched}") — unsuitable for campaigns`] : []), ...tldIssues],
    usernameHeuristics,
    ...
    hatefulTermsMatched: semantics.hatefulTermsMatched || [],
    abusiveTermsMatched: semantics.abusiveTermsMatched || [],
    nsfwTermsMatched: semantics.nsfwTermsMatched || [],
    phishingRisk,
...
// MAIN EXPORT
export async function emailValidator(text, config = {}) {
  const emails = text
    .split(/[,\n]+/)
    .map(e => e.trim())
    .filter(e => e)
  ...
  // Enforce email count limit to prevent abuse and performance issues
  // Limit is 5000 emails per request to prevent:
  // - DNS flooding (5000 emails * 7 DNS lookups = 35k queries)
  // - Browser memory/rendering issues
  // - Network timeout on massive responses
  ...
  if (emails.length > MAX_EMAILS) {
    return {
      ...
      averageCampaignReadiness: 'Poor',
      results: [],
      error: `Too many emails. Maximum ${MAX_EMAILS} emails per request. You provided ${emails.length} emails. Please split your list into smaller batches.`,
      limitExceeded: true
    }
  }

  const results = await Promise.all(emails.map(email => validateSingleEmail(email, config)))
  ...
  return {
    ...
    results,
    config: {
      ignoreLcsIfLower: config.ignoreLcsIfLower || false
    }
  }
}
