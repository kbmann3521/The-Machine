...
function validateSingleEmail(trimmedEmail) {
  ...
  if (reservedDomains.has(domainLower)) {
    ...
    // Skip username heuristics for reserved domains
  ...
  if (semantics.hasOffensiveTerms) {
    ...
    details.trustworthinessWarnings.push('Offensive/abusive content - likely bot or malicious')
  }
  ...
  if (isInvalidAddress) {
    ...
    details.humanLikelihood = 'Invalid (no human likelihood)'
  } else {
    details.combinedGrade = calculateCombinedGrade(details.deliverabilityScore, details.trustworthinessScore)
    details.humanLikelihood = mapHumanLikelihoodLabel(details.trustworthinessScore, false)
  }
  ...
  // Track invalid status for UI
  details.isInvalid = isInvalidAddress
