...
export function classifyIPv4(ip) {
  ...
  // 0.0.0.0 - Unspecified address (RFC 1122, RFC 791)
  if (ip === '0.0.0.0') {
    return {
      type: 'Current Network',
      subtype: 'Unspecified/This Host',
      rfc: 'RFC 1122 / RFC 791',
      scope: 'This Network',
      isSpecialUse: true,
      isPrivate: false,
    }
  ...
  // 255.255.255.255 - Limited Broadcast (RFC 919, RFC 922, RFC 1112)
  if (ip === '255.255.255.255') {
    return {
      type: 'Broadcast',
      subtype: 'Limited Broadcast',
      rfc: 'RFC 919 / RFC 922 / RFC 1112',
      scope: 'Local Broadcast Domain',
      isBroadcast: true,
    }
  }

  // 255.255.255.254 - Reserved (RFC 1112)
  if (ip === '255.255.255.254') {
    return {
      type: 'Reserved',
      subtype: 'Reserved Address',
      rfc: 'RFC 1112',
      isReserved: true,
    }
  }
  ...
  if (int >= 4026531840 && int <= 4294967294) {
    return {
      type: 'Reserved',
      subtype: 'Future Use',
      rfc: 'RFC 1112',
...
export function classifyIPv6(ip) {
  ...
  if (expanded.startsWith('0000:0000:0000:0000:0000:ffff:')) {
    return {
      ...
      subtype: 'Transition / Compatibility',
      rfc: 'RFC 4291',
