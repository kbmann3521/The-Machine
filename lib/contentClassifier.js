...
function stripStyleTags(value) {
  // Remove <style> tags and their content before checking for Markdown patterns
  // This prevents CSS syntax like * or () from being misidentified as Markdown
...
function detectHtml(text) {
  ...
  // Strip style tags to prevent CSS patterns from interfering with HTML detection
  const sanitized = stripInlineCode(stripCodeBlocks(stripStyleTags(text)))
...
function detectMarkdown(text) {
  if (!text) return false
  // Strip style tags first to prevent CSS syntax from matching Markdown patterns
  const stripped = stripStyleTags(text)
  return MARKDOWN_SIGNATURES.some(pattern => pattern.test(stripped))
}
