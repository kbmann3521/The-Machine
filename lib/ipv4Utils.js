...
export function findCoveringSubnet(startIP, endIP) {
  ...
  // Find the position of the highest (leftmost) bit set in XOR
  // This tells us the most significant bit position where start and end differ
  // Math.clz32 counts leading zeros, so 31 - clz32(value) gives us the highest bit position
  const highestBitPos = 31 - Math.clz32(xorValue)

  // The prefix length is: 32 - (highest_bit_position + 1)
  // Example: if highest bit is at position 3, we need 4 bits for host part
  // So prefix = 32 - 4 = 28
  const prefix = 32 - (highestBitPos + 1)

  // Create mask: all bits from position 31 down to (highestBitPos + 1) are 1
  const mask = prefix === 0 ? 0 : (0xFFFFFFFF << (32 - prefix)) >>> 0

  // Extract network address by ANDing with mask
  const networkInt = minInt & mask

  // Calculate broadcast address by ORing with inverted mask
  const broadcastInt = networkInt | (~mask >>> 0)

  // Verify entire range fits within calculated subnet
  ...
  // Should rarely if ever reach here with correct algorithm
  // But return /0 as absolute fallback
  return {
    subnet: '0.0.0.0/0',
    prefix: 0,
    networkAddr: '0.0.0.0',
