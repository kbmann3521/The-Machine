...
export function findCoveringSubnet(startIP, endIP) {
  ...
  let highestDifferingBit = 31
  for (let i = 31; i >= 0; i--) {
    // Safe bit extraction: convert to string, work with binary representation
  ...
  // Prefix length: all bits above the highest differing bit are network bits
  // If highest differing bit is at position i, prefix = i
  // (because we need bits 31 through i+1 as network, which is i+1 bits total... no wait)
  // If bit at position i differs, we need all bits 31 down to i+1 to be network
  // That's 31 - (i+1) + 1 = 31 - i bits? No...
  // Actually: prefix = i (bits 31 to i are network, but we need to count from 31 backwards)
  // If highest differing bit is at position i, then prefix = 31 - i
  const prefix = highestDifferingBit
