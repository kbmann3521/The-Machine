...
export function findCoveringSubnet(startIP, endIP) {
  ...
  // For /28: we want 28 ones, 4 zeros
  // Bit positions 31 down to 4 should be 1, positions 3 down to 0 should be 0
  let mask
  if (prefix === 0) {
    mask = 0
  } else if (prefix === 32) {
    ...
    // Shift 0xFFFFFFFF left by (32 - prefix) positions to get the right mask
    // For /28: shift left by 4 positions
    // 0xFFFFFFFF << 4 with unsigned right shift by 0 gives us the correct mask
