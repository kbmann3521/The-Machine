...
/**
 * Detects if a range of IPs fits within a specific CIDR subnet
 * Returns the covering subnet if found, or null
 */
export function findCoveringSubnet(startIP, endIP) {
  ...
  // Try each possible prefix length from /24 down to /0
  for (let prefix = 24; prefix >= 0; prefix--) {
    const networkAddr = getNetworkAddress(startIP, prefix)
    const broadcastAddr = getBroadcastAddress(startIP, prefix)

    if (!networkAddr || !broadcastAddr) continue

    const networkInt = ipv4ToInteger(networkAddr)
    const broadcastInt = ipv4ToInteger(broadcastAddr)

    // Check if entire range fits within this subnet
    if (startInt >= networkInt && endInt <= broadcastInt) {
      return { subnet: `${networkAddr}/${prefix}`, prefix, networkAddr, broadcastAddr }
    }
  }
