...
export function findCoveringSubnet(startIP, endIP) {
  ...
  // Calculate prefix: if highest differing bit is at position i,
  // we need bits 31 down to (i+1) as network bits
  // Example: if highest differ bit is at position 3, we have bits 31-4 as network = 28 bits = /28
  const prefix = 32 - (highestDifferingBit + 1)

  // Create mask: first 'prefix' bits are 1, remaining bits are 0
  // For /28: we want the last 4 bits as 0, rest as 1
  let mask
  if (prefix === 0) {
    mask = 0
  } else if (prefix === 32) {
    ...
    // Build mask safely: create a number with 'prefix' ones
    // For /28: we want to zero out the last 4 bits
    // This is equivalent to (Math.pow(2, prefix) - 1) << (32 - prefix)
    // But safer using Math operations
    const hostBits = 32 - prefix
    // Create a mask with hostBits of zeros at the right
    // Math.pow(2, 32) = 4294967296, subtract Math.pow(2, hostBits)
    mask = (Math.pow(2, 32) - Math.pow(2, hostBits)) >>> 0
  }

  // Extract network address by ANDing with mask
  const networkInt = (minInt & mask) >>> 0
  ...
  if (networkInt <= minInt && maxInt <= broadcastInt) {
    const networkAddr = integerToIPv4(networkInt >>> 0)
    const broadcastAddr = integerToIPv4(broadcastInt >>> 0)
