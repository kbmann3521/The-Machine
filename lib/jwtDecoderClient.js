...
// Helper function to parse JWE (JSON Web Encryption)
function parseJWE(parts, options = {}) {
  // JWE Format: header.encryptedKey.iv.ciphertext.tag (5 parts)

  // Validate all 5 parts are present (note: encryptedKey can be empty for alg: "dir")
  if (!parts[0] || !parts[2] || !parts[3] || !parts[4] || parts.length !== 5) {
    const missingParts = []
    if (!parts[0]) missingParts.push('protectedHeader')
    if (!parts[2]) missingParts.push('iv')
    if (!parts[3]) missingParts.push('ciphertext')
    if (!parts[4]) missingParts.push('tag')
    if (parts.length !== 5) missingParts.push('wrong number of segments')
    return {
      error: `Invalid JWE: missing ${missingParts.join(', ')} part${missingParts.length > 1 ? 's' : ''}. JWE must have exactly 5 parts separated by dots (encryptedKey can be empty for alg: "dir").`,
      decoded: false,
      kind: 'JWE',
    }
  }

  // Validate base64url characters (skip validation for empty encryptedKey, which is valid for dir algorithm)
  for (let i = 0; i < parts.length; i++) {
    const partNames = ['protectedHeader', 'encryptedKey', 'iv', 'ciphertext', 'tag']
    // Skip base64url validation for empty encryptedKey (allowed for alg: "dir")
    if (i === 1 && parts[i] === '') {
      continue
    }
    if (!/^[A-Za-z0-9_-]*$/.test(parts[i])) {
      ...
      const uniqueChars = [...new Set(invalidChars)].join(', ')
      return {
        error: `Invalid JWE: ${partNames[i]} contains non-base64url characters (${uniqueChars}). Base64url allows: a-z, A-Z, 0-9, hyphen (-), underscore (_)`,
        decoded: false,
        kind: 'JWE',
      }
    }
  ...
  const protectedHeaderDecoded = decodeBase64Url(parts[0])
  if (!protectedHeaderDecoded.success) {
    return {
      error: `Failed to decode JWE protected header: ${protectedHeaderDecoded.error}`,
      decoded: false,
      kind: 'JWE',
      jwe: {
        rawSegments: {
        ...
        protectedHeaderError: protectedHeaderDecoded.error,
      },
    }
  }

  // Parse protected header JSON
  const protectedHeaderParsed = parseJSON(protectedHeaderDecoded.value)
  if (!protectedHeaderParsed.success) {
    return {
      error: `Invalid JSON in JWE protected header: ${protectedHeaderParsed.error}`,
      decoded: false,
      kind: 'JWE',
      jwe: {
        rawSegments: {
        ...
        protectedHeader: null,
        protectedHeaderError: protectedHeaderParsed.error,
      },
    }
  }

  const header = protectedHeaderParsed.value

  // Extract key JWE header fields
  const alg = header.alg || null
  const enc = header.enc || null
  const kid = header.kid || null
  const zip = header.zip || null
  const typ = header.typ || null

  // Build encryption diagnostics
  const encryptionDiagnostics = []

  // Check for missing alg
  if (!alg) {
    encryptionDiagnostics.push({
      level: 'error',
      message: 'Missing "alg" (key encryption algorithm) in JWE protected header',
    })
  } else {
    const supportedKeyAlgs = ['RSA1_5', 'RSA-OAEP', 'A128KW', 'A192KW', 'A256KW', 'dir', 'ECDH-ES+HKDF-256', 'ECDH-ES+HKDF-512', 'A128GCMKW', 'A192GCMKW', 'A256GCMKW', 'PBES2-HS256+A128KW', 'PBES2-HS384+A192KW', 'PBES2-HS512+A256KW']
    if (!supportedKeyAlgs.includes(alg)) {
      encryptionDiagnostics.push({
        level: 'warning',
        message: `Unknown key encryption algorithm: "${alg}". Standard algorithms: RSA1_5, RSA-OAEP, A128KW, A192KW, A256KW, dir, ECDH-ES, etc.`,
      })
    }
  }

  // Check for missing enc
  if (!enc) {
    encryptionDiagnostics.push({
      level: 'error',
      message: 'Missing "enc" (content encryption algorithm) in JWE protected header',
    })
  } else {
    const supportedContentAlgs = ['A128CBC-HS256', 'A192CBC-HS384', 'A256CBC-HS512', 'A128GCM', 'A192GCM', 'A256GCM']
    if (!supportedContentAlgs.includes(enc)) {
      encryptionDiagnostics.push({
        level: 'warning',
        message: `Unknown content encryption algorithm: "${enc}". Standard algorithms: A128GCM, A192GCM, A256GCM, A128CBC-HS256, etc.`,
      })
    }
  }

  // Check for missing kid (warning, not error)
  if (!kid && alg && !alg.startsWith('dir')) {
    encryptionDiagnostics.push({
      level: 'info',
      message: 'Missing "kid" (key ID) in JWE protected header. Consider adding kid for key identification.',
    })
  }
  ...
  if (zip) {
    if (zip === 'DEF') {
      encryptionDiagnostics.push({
        level: 'info',
        message: `Payload is compressed with ${zip} (deflate algorithm)`,
      })
    } else {
      encryptionDiagnostics.push({
        level: 'warning',
        message: `Unknown compression algorithm: "${zip}". Standard is DEF (deflate).`,
      })
    }
  }
  ...
  return {
    ...
    jwe: {
      ...
      decryption: {
        ...
        attempted: false,
        success: false,
        error: 'Decryption not implemented yet. Phase 7B will support AES-GCM, RSA-OAEP, and other key/content algorithms.',
      },
    },
    summary: {
      valid: encryptionDiagnostics.filter(d => d.level === 'error').length === 0,
      errorCount: encryptionDiagnostics.filter(d => d.level === 'error').length,
      warningCount: encryptionDiagnostics.filter(d => d.level === 'warning').length,
      infoCount: encryptionDiagnostics.filter(d => d.level === 'info').length,
    },
  }
}

// Base64url to Uint8Array converter for JWE decryption
export function base64urlToUint8Array(base64url) {
  // Add padding if needed
  let padded = base64url
  const remainder = base64url.length % 4
  if (remainder) {
    padded = base64url + '='.repeat(4 - remainder)
  }
...
// Decrypt JWE with dir + A256GCM using Web Crypto API
// Returns { plaintext, payload } on success
// Throws on error
export async function decryptJWE_Dir_A256GCM(jwe, keyBase64Url) {
  // Extract base64url-encoded segments from rawSegments
  const { protectedHeader: protectedHeaderB64Url, iv: ivB64Url, ciphertext: ciphertextB64Url, tag: tagB64Url } = jwe.rawSegments

  // 1) Validate and convert key
  const keyBytes = base64urlToUint8Array(keyBase64Url)
  if (keyBytes.byteLength !== 32) {
    throw new Error(`Key must be 32 bytes (256-bit). Got ${keyBytes.byteLength} bytes.`)
  }

  // 2) Convert JWE segments to bytes
  const iv = base64urlToUint8Array(ivB64Url)
  const ciphertext = base64urlToUint8Array(ciphertextB64Url)
  const tag = base64urlToUint8Array(tagB64Url)

  // 3) Concatenate ciphertext and tag (Web Crypto expects combined ciphertext+tag)
  const combined = new Uint8Array(ciphertext.length + tag.length)
  combined.set(ciphertext, 0)
  combined.set(tag, ciphertext.length)

  // 4) Additional Authenticated Data (AAD) is the ASCII bytes of the protected header segment
  const enc = new TextEncoder()
  const additionalData = enc.encode(protectedHeaderB64Url)

  // 5) Import the symmetric key
  let cryptoKey
  try {
  ...
  try {
    ...
    throw new Error(`Decryption failed: ${err.message}. This may indicate a wrong key or corrupted token.`)
  }
