...
export function lineColToOffset(state, line, column) {
  try {
    ...
    return Math.min(offset, state.doc.length)
  } catch (error) {
    console.warn('Error converting line/column to offset:', { line, column, error })
    return 0
  }
}
...
export function createFormatterLinter(diagnosticsArray, options = {}) {
  ...
  return linter((view) => {
    ...
    for (const d of diagnosticsArray) {
      ...
      // Must have line and column to convert
      if (!d.line || !d.column) {
        continue
      }
      ...
      try {
        ...
        console.warn('Failed to process diagnostic:', d, error)
      }
    }
