...
/**
 * Detects common IPv6 input errors
 */
export function detectIPv6Errors(input) {
  ...
  if (doubleColonCount > 1) {
    errors.push('Multiple :: (double colon) found. Only one :: is allowed per address.')
  }

  // Check for too many groups
  const groups = trimmed.split(':').filter(g => g !== '')
  if (groups.length > 8) {
    errors.push(`Too many groups: ${groups.length} found, maximum 8 allowed`)
  }

  // Check for invalid characters in groups
  groups.forEach((group, idx) => {
    if (!/^[0-9a-fA-F]*$/.test(group)) {
      errors.push(`Group ${idx + 1}: Contains invalid characters (must be hex)`)
    }
    if (group.length > 4) {
      errors.push(`Group ${idx + 1}: Too many characters (max 4 hex digits)`)
    }
  })

  // Check for invalid compression
  if (trimmed.includes(':::')) {
    errors.push('Invalid compression: ::: found (should be ::)')
  }

  if (trimmed === '::') {
    warnings.push('Unspecified address (::) - only valid in specific contexts')
  }

  if (trimmed === '::1') {
    tips.push('✓ This is the loopback address')
  }

  if (trimmed.startsWith('fe80:')) {
    tips.push('✓ This is a link-local address')
  }

  if (trimmed.startsWith('fc') || trimmed.startsWith('fd')) {
    tips.push('✓ This is a unique local address (ULA)')
  }
  ...
  if (!errors.length && isValidIPv6(trimmed)) {
    tips.push('✓ Valid IPv6 address format')
  }
