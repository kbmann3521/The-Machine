...
export function expandIPv6(ip) {
  ...
  // Pad each group to 4 characters, and convert IPv4-mapped notation to hex
  const hextets = expanded.split(':')

  // ISSUE 2 FIX: Detect and convert IPv4 notation in last hextet (e.g., 192.168.1.1 -> c0a8:0101)
  if (hextets.length === 8) {
    const lastHextet = hextets[7]
    if (lastHextet.includes('.')) {
      const ipv4Match = lastHextet.match(/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/)
      if (ipv4Match) {
        const [, a, b, c, d] = ipv4Match.map(x => parseInt(x, 10))
        // Convert to two hex hextets: high = (a<<8)|b, low = (c<<8)|d
        hextets[6] = ((a << 8) | b).toString(16).padStart(4, '0')
        hextets[7] = ((c << 8) | d).toString(16).padStart(4, '0')
      }
    }
  }
