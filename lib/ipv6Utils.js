 ...
 * Handles IPv4-mapped notation: "::ffff:192.168.1.1" -> "0000:0000:0000:0000:0000:ffff:c0a8:0101"
 */
export function expandIPv6(ip) {
  ...
  // STEP 1: Detect and convert IPv4 notation BEFORE :: expansion
  // This handles cases like ::ffff:192.168.1.1 or ::192.168.1.1
  const colonLastIdx = expanded.lastIndexOf(':')
  if (colonLastIdx !== -1) {
    const lastPart = expanded.substring(colonLastIdx + 1)
    const ipv4Match = lastPart.match(/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/)
    if (ipv4Match) {
      const [, a, b, c, d] = ipv4Match.map(x => parseInt(x, 10))
      ipv4High = ((a << 8) | b).toString(16).padStart(4, '0')
      ipv4Low = ((c << 8) | d).toString(16).padStart(4, '0')
      // Remove the IPv4 part from expanded for :: processing
      expanded = expanded.substring(0, colonLastIdx + 1)
    }
  }

  // STEP 2: Handle :: expansion on IPv6 part only
  if (expanded.includes('::')) {
    ...
    if (emptyIndex === 0 || emptyIndex === parts.length - 2) {
      ...
      // Account for IPv4 part if present (takes 2 hextets)
      const hextetsNeeded = ipv4High && ipv4Low ? 2 : 0
      const missingGroups = 8 - nonEmptyGroups.length - hextetsNeeded
      ...
      const missingGroups = 8 - totalParts - hextetsNeeded
      const zeros = Array(Math.max(0, missingGroups)).fill('0000')
  ...
  // STEP 3: Split into hextets and pad each to 4 characters, normalize to lowercase
  let hextets = expanded.split(':').map(group => group.padStart(4, '0').toLowerCase())

  // STEP 4: Append IPv4-converted hextets if present
  if (ipv4High && ipv4Low) {
    hextets.push(ipv4High.toLowerCase(), ipv4Low.toLowerCase())
  }
...
export function detectIPv6Errors(input) {
  ...
  // ISSUE 1 FIX: Strip zone ID before any IPv6 validation
  const zoneIndex = trimmed.includes('%') ? trimmed.split('%')[1] : null
  const ipv6Only = trimmed.split('%')[0]

  // Check for multiple ::
  const doubleColonCount = (ipv6Only.match(/::/g) || []).length
  ...
  // STEP 1: Detect and convert IPv4-mapped notation BEFORE hex validation
  // Check if the last group contains IPv4 notation (dots)
  let hasIPv4Mapped = false
  let groupsToValidate = ipv6Only.split(':').filter(g => g !== '')

  if (groupsToValidate.length > 0) {
    const lastGroup = groupsToValidate[groupsToValidate.length - 1]
    const ipv4Match = lastGroup.match(/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/)

    if (ipv4Match) {
      // IPv4 notation found - convert to hex before validation
      hasIPv4Mapped = true
      const [, a, b, c, d] = ipv4Match.map(x => parseInt(x, 10))

      // Validate IPv4 octets
      let ipv4Valid = true
      if (parseInt(a, 10) > 255 || parseInt(b, 10) > 255 ||
          parseInt(c, 10) > 255 || parseInt(d, 10) > 255) {
        ipv4Valid = false
        errors.push({
          code: 'INVALID_IPV4_OCTET',
          message: 'IPv4 portion contains invalid octet values (must be 0-255)',
          severity: 'error',
        })
      }

      if (ipv4Valid) {
        // Convert IPv4 to hex hextets
        const hex1 = ((a << 8) | b).toString(16).padStart(4, '0')
        const hex2 = ((c << 8) | d).toString(16).padStart(4, '0')

        // Replace the last group with the two hex hextets
        groupsToValidate.pop()
        groupsToValidate.push(hex1, hex2)
      }
    }
  }

  // Check for too many groups
  if (groupsToValidate.length > 8) {
    errors.push({
      code: 'TOO_MANY_GROUPS',
      message: `Too many groups: ${groupsToValidate.length} found, maximum 8 allowed`,
      severity: 'error',
    })
  }

  // STEP 2: Now validate hex characters (after IPv4 conversion)
  ...
  if (ipv6Only.startsWith('fc') || ipv6Only.startsWith('fd')) {
    tips.push({
  ...
  if (!errors.length && isValidIPv6(ipv6Only)) {
    if (hasIPv4Mapped) {
      tips.push({
        code: 'VALID_IPV6_MAPPED',
        message: '✓ Valid IPv4-mapped IPv6 address',
        severity: 'info',
      })
    } else {
      tips.push({
...
/**
 * Normalizes IPv6 with IPv4 notation (e.g., ::ffff:192.168.1.1) to full hex
 * Converts the IPv4 part to hex hextets
 */
export function normalizeIPv6WithIPv4Notation(expanded) {
  if (!expanded || !expanded.includes('.')) return expanded

  // Find the last colon and the IPv4 part after it
  const lastColonIndex = expanded.lastIndexOf(':')
  if (lastColonIndex === -1) return expanded

  const beforeIPv4 = expanded.substring(0, lastColonIndex + 1)
  const ipv4Part = expanded.substring(lastColonIndex + 1)

  // Check if the part after last colon is IPv4 notation (a.b.c.d)
  const ipv4Match = ipv4Part.match(/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/)
  if (!ipv4Match) return expanded

  const [, a, b, c, d] = ipv4Match.map(x => parseInt(x, 10))

  // Convert IPv4 to two hex hextets
  const hex1 = ((a << 8) | b).toString(16).padStart(4, '0')
  const hex2 = ((c << 8) | d).toString(16).padStart(4, '0')

  // Replace the last two hextets (remove the IPv4 part, add hex)
  const parts = beforeIPv4.split(':')
  parts.pop() // Remove empty string from trailing colon
  parts.push(hex1, hex2)
...
export function classifyIPv6Special(ip) {
  ...
  if (upper.startsWith('0000:0000:0000:0000:0000:FFFF:')) {
    return {
