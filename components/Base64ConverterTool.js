...
export default function Base64ConverterTool() {
  ...
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool encodes text to Base64 and decodes Base64 strings back to readable text using strict, rule-based logic. It provides auto-detection, multiple encoding variants (standard, URL-safe, MIME), character encoding options, and detailed metadata analysis. It does not attempt to infer meaning, repair fundamentally invalid data silently, or apply creative interpretation. Every transformation follows explicit rules designed to preserve correctness.\n\nThe purpose of this tool is to save time, reduce human error, and make Base64 handling easier while remaining fully transparent about how the result was produced.',
    whyExists: 'Many Base64 tools are limited or unreliable. This tool exists to provide a focused, comprehensive, deterministic alternative for developers who need more control.\n\nIts goal is to give you confidence that encodings are accurate, repeatable, and suitable for real-world use—whether that means encoding credentials, preparing data for transmission, debugging encoding issues, or converting between Base64 variants.',
    commonUseCases: 'This tool is commonly used when working with Base64-encoded data that needs to be encoded, decoded, or analyzed. It is especially useful in API development, data transmission, authentication, and debugging workflows where correctness matters.\n\nUsers often rely on this tool to encode credentials, decode API responses, prepare images for embedding, fix encoding issues, convert between variants, and verify Base64 data.',
    ...
    faq: [
      ...
      { question: 'Will this tool modify my Base64?', answer: 'Only if you explicitly request transformation. The tool provides options for standard, URL-safe, and MIME variants.' },
      { question: 'Can I rely on the encoding results?', answer: 'Yes. The encoding is deterministic and follows Base64 specification, making it suitable for real-world use.' },
    ],
  ...
  // Execute the base64-converter tool
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.leftPanel} ${isPreviewFullscreen ? styles.hidden : ''}`}>
        <InputTabs
          selectedTool={base64ConverterTool}
          inputTabLabel="Text"
          onActiveTabChange={() => {}}
          infoContent={null}
          tabActions={null}
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={base64ConverterTool?.outputType}
          loading={loading}
          error={error}
          toolId="base64-converter"
