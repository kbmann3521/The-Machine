...
export default function ToolOutputPanel({ result, outputType, loading, error, toolId, activeToolkitSection, configOptions, onConfigChange }) {
  ...
  // Special handling for ASCII/Unicode Converter - display summary and conversions
  if (toolId === 'ascii-unicode-converter' && displayResult && typeof displayResult === 'object' && !displayResult.error) {
    const tabs = []

    // Output tab with summary and table
    if (displayResult.summary || (displayResult.conversions && displayResult.conversions.length > 0)) {
      const outputContent = (
        <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {displayResult.conversions && displayResult.conversions.length > 0 && (
            <table style={{
              width: '100%',
