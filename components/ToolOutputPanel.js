...
export default function ToolOutputPanel({ result, outputType, loading, error, toolId, activeToolkitSection, configOptions, onConfigChange, inputText, imagePreview, warnings = [], onInputUpdate, showAnalysisTab, onShowAnalysisTabChange, showRulesTab, onShowRulesTabChange, isPreviewFullscreen, onTogglePreviewFullscreen, renderCssTabOnly = false, activeMarkdownInputTab = 'input', markdownInputMode = 'input', markdownCustomCss: externalMarkdownCss = null, onMarkdownCustomCssChange = null, onCssFormattedOutput = null, markdownCustomJs: externalMarkdownJs = null, onMarkdownCustomJsChange = null, onJsFormattedOutput = null, cssConfigOptions = {}, onCssConfigChange = null, jsConfigOptions = {}, onJsConfigChange = null, onJsFormatterDiagnosticsChange = null }) {
  ...
  const renderHexToTextConverterOutput = () => {
    if (!displayResult || typeof displayResult !== 'object') {
      return (
        <OutputTabs
          key={toolId}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {metadata['Input Format'] && (
          <div style={{
            ...
            Detected input as {metadata['Input Format']}
          </div>
        )}
        ...
        {/* All Conversion Formats (for text→hex) */}
        {Object.keys(formats).length > 0 && (
          <div>
            <div style={{
              fontSize: '12px',
                ...
                <div key={formatName} className={styles.copyCard}>
                  <div className={styles.copyCardHeader}>
                    ...
                    <button
                      type="button"
                      className="copy-action"
                      onClick={() => handleCopyField(formatValue, `hex-to-text-format-${formatName}`)}
                      title="Copy to clipboard"
                      style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                    ...
                    >
                      {copiedField === `hex-to-text-format-${formatName}` ? '✓' : <FaCopy />}
                    </button>
                  </div>
                  <div className={styles.copyCardValue} style={{ wordBreak: 'break-all', maxHeight: '150px', overflowY: 'auto' }}>
                    {formatValue}
  ...
  const renderBinaryConverterOutput = () => {
    if (!displayResult || typeof displayResult !== 'object') {
      return (
        <OutputTabs
          key={toolId}
                ...
                <div key={formatName} className={styles.copyCard}>
                  <div className={styles.copyCardHeader}>
                    ...
                    <button
                      type="button"
                      className="copy-action"
                      onClick={() => handleCopyField(formatValue, `binary-to-text-format-${formatName}`)}
                      title="Copy to clipboard"
                      style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                    ...
                    >
                      {copiedField === `binary-to-text-format-${formatName}` ? '✓' : <FaCopy />}
                    </button>
                  </div>
                  <div className={styles.copyCardValue} style={{ wordBreak: 'break-all', maxHeight: '150px', overflowY: 'auto' }}>
                    {formatValue}
  ...
  const renderOutput = () => {
    switch (toolId) {
      ...
      case 'hexadecimal-converter':
        return renderHexToTextConverterOutput()
      case 'binary-converter':
        return renderBinaryConverterOutput()
      case 'caesar-cipher':
