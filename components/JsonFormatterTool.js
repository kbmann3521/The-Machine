...
export default function JsonFormatterTool() {
  ...
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool formats, validates, and transforms JSON using strict, rule-based logic. It performs beautification, minification, validation, key sorting, flattening, extraction using JSONPath, and conversion to YAML, CSV, and XML formats. It does not attempt to infer meaning, repair fundamentally invalid data, or apply creative interpretation. Every transformation follows explicit rules designed to preserve structure and correctness.\n\nThe purpose of this tool is to save time, reduce human error, and make JSON handling easier while remaining fully transparent about how the result was produced.',
    whyExists: 'Many JSON tools are limited to beautification or basic validation. This tool exists to provide a focused, comprehensive, deterministic alternative for developers who need more control.\n\nIts goal is to give you confidence that transformations are accurate, repeatable, and suitable for real-world use—whether that means debugging API responses, validating data, reformatting for readability, minifying for production, or converting between formats.',
    commonUseCases: 'This tool is commonly used when working with JSON data that needs to be analyzed, formatted, validated, or transformed. It is especially useful in API development, data processing, debugging, and deployment workflows where correctness matters.\n\nUsers often rely on this tool to fix formatting, validate API responses, sort keys, flatten nested structures, extract specific data, convert between formats, minify for production, and clean up malformed JSON.',
    deterministic: 'This tool is fully deterministic. The same input will always produce the same output. There is no randomness, learning, or adaptive behavior involved at any stage of processing.\n\nNo AI models are used to infer, interpret, or modify your data. All results are generated using explicit, predefined logic so you always know what to expect and can rely on the output in automated or repeatable workflows.',
    ...
    faq: [
      ...
      { question: 'Is my JSON sent to a server?', answer: 'No. Your input is not stored, logged, or transmitted. Processing happens entirely in your browser.' },
      { question: 'Will this tool modify my JSON?', answer: 'Only if you explicitly request it. Validation and beautification preserve structure. Transformations only happen in chosen modes.' },
      { question: 'Can I rely on the validation results?', answer: 'Yes. The validation is deterministic and follows JSON specification, making it suitable for real-world use.' },
    ],
  ...
  // Execute the json-formatter tool
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.leftPanel} ${isPreviewFullscreen ? styles.hidden : ''}`}>
        <InputTabs
          selectedTool={jsonFormatterTool}
          inputTabLabel="JSON"
          onActiveTabChange={() => {}}
          infoContent={null}
          tabActions={null}
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={jsonFormatterTool?.outputType}
          loading={loading}
          error={error}
          toolId="json-formatter"
