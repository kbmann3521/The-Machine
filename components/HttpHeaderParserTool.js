...
export default function HttpHeaderParserTool() {
  ...
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool parses, validates, and analyzes HTTP headers using strict, rule-based logic to produce clean, predictable output. It validates headers against RFC 7230, 7231, and 9110 standards, performs security analysis, simulates cache behavior, detects conflicts, and exports in multiple formats. It does not attempt to infer meaning, repair fundamentally invalid data, or apply creative interpretation. Every analysis follows explicit rules designed to preserve accuracy and correctness.\n\nThe purpose of this tool is to save time, reduce human error, and make HTTP debugging easier while remaining fully transparent about how the analysis was produced.',
    whyExists: 'Many HTTP inspection tools are part of large platforms like Postman or Cloudflare, making them difficult to use quickly or integrate into workflows. This tool exists to provide a focused, deterministic alternative.\n\nIts goal is to give you confidence that the analysis is accurate, repeatable, and suitable for real-world use—whether that means debugging before deployment, validating security headers, analyzing cache behavior, identifying conflicts, or ensuring RFC compliance.',
    commonUseCases: 'This tool is commonly used when working with HTTP request and response headers that need to be validated, analyzed, or debugged. It is especially useful in API development, security auditing, performance optimization, and deployment workflows where correctness matters.\n\nUsers often rely on this tool to validate headers before deployment, detect security vulnerabilities, analyze cache behavior, check HTTP/2 and HTTP/3 compatibility, verify CORS configuration, understand header interactions, and ensure RFC compliance.',
    ...
    faq: [
      ...
      { question: 'Will this tool change my headers?', answer: 'No. The tool analyzes and validates without modifying your input headers.' },
      { question: 'Can I rely on the analysis for production?', answer: 'Yes. The analysis is consistent and based on RFC standards, making it suitable for real-world use.' },
    ],
  ...
  const httpHeaderParserTool = {
    ...TOOLS['http-header-parser'],
  ...
  // Configuration for http-header-parser
  ...
  // Execute the http-header-parser tool
  ...
  const handleImageChange = () => {
    // Not used in http-header-parser
  }
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.leftPanel} ${isPreviewFullscreen ? styles.hidden : ''}`}>
        <InputTabs
          selectedTool={httpHeaderParserTool}
          inputTabLabel="Headers"
          onActiveTabChange={() => {}}
          infoContent={null}
          tabActions={null}
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={httpHeaderParserTool?.outputType}
          loading={loading}
          error={error}
          toolId="http-header-parser"
