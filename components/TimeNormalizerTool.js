...
export default function TimeNormalizerTool() {
  // Standalone page uses different detailed description format
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool parses, normalizes, and converts dates between timezones using strict, rule-based logic. It detects date formats, normalizes to standard formats, converts between 30+ timezones, handles daylight saving time, and calculates UTC offsets. It does not attempt to infer meaning, repair fundamentally invalid dates, or apply creative interpretation. Every analysis follows explicit rules designed to preserve temporal accuracy.\n\nThe purpose of this tool is to save time, reduce human error, and make date handling easier while remaining fully transparent about how the result was produced.',
    whyExists: 'Many date tools are unreliable or don\'t handle timezones properly. This tool exists to provide a focused, comprehensive, deterministic alternative for developers.\n\nIts goal is to give you confidence that date conversions are accurate, repeatable, and suitable for real-world use—whether that means debugging date issues, converting between timezones, parsing timestamps, normalizing formats, or handling daylight saving time.',
    commonUseCases: 'This tool is commonly used when working with dates and times that need to be parsed, normalized, or converted. It is especially useful in API development, scheduling, logging, and international applications where timezone handling matters.\n\nUsers often rely on this tool to convert between timezones, parse timestamps, normalize date formats, debug date issues, handle DST transitions, and verify date/time consistency.',
    deterministic: 'This tool is fully deterministic. The same input will always produce the same output. There is no randomness, learning, or adaptive behavior involved at any stage of processing.\n\nNo AI models are used to interpret, infer, or modify your dates. All results are generated using explicit, predefined logic so you always know what to expect and can rely on the output in automated or repeatable workflows.',
    ...
    faq: [
      ...
      { question: 'Is my date sent to a server?', answer: 'No. Your input is not stored, logged, or transmitted. Processing happens entirely in your browser.' },
      { question: 'Will this tool modify my date?', answer: 'The tool normalizes and converts without changing the actual moment in time represented.' },
      { question: 'Can I rely on the conversion results?', answer: 'Yes. The conversions are deterministic and handle DST properly, suitable for real-world use.' },
    ],
  ...
  const timeNormalizerTool = {
    ...TOOLS['time-normalizer'],
    toolId: 'time-normalizer',
    id: 'time-normalizer',
  ...
  // Configuration for time-normalizer
  ...
  // Execute the time-normalizer tool
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.leftPanel} ${isPreviewFullscreen ? styles.hidden : ''}`}>
        <InputTabs
          selectedTool={timeNormalizerTool}
          inputTabLabel="Date/Time"
          onActiveTabChange={() => {}}
          infoContent={null}
          tabActions={null}
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={timeNormalizerTool?.outputType}
          loading={loading}
          error={error}
          toolId="time-normalizer"
