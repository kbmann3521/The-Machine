...
export default function CssFormatterTool() {
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool formats, validates, and transforms CSS using strict, rule-based logic. It beautifies CSS with proper indentation, minifies for production, validates syntax, detects linting issues, handles vendor prefixing, and provides sorting options. It does not attempt to infer meaning, repair fundamentally invalid CSS silently, or apply creative interpretation. Every transformation follows explicit rules designed to preserve CSS correctness.\n\nThe purpose of this tool is to save time, reduce human error, and make CSS handling easier while remaining fully transparent about how the result was produced.',
    whyExists: 'Many CSS formatters are limited, unreliable, or don\'t provide comprehensive validation. This tool exists to provide a focused, comprehensive, deterministic alternative for developers.\n\nIts goal is to give you confidence that formatting is accurate, repeatable, and suitable for real-world use—whether that means beautifying for readability, minifying for production, validating syntax, or ensuring consistency.',
    commonUseCases: 'This tool is commonly used when working with CSS that needs to be formatted, validated, or standardized. It is especially useful in development, code review, performance optimization, and deployment workflows where CSS readability and correctness matter.\n\nUsers often rely on this tool to beautify messy CSS, minify for production, validate syntax before deployment, detect linting issues, add vendor prefixes, standardize formatting, and ensure consistency across stylesheets.',
    deterministic: 'This tool is fully deterministic. The same input will always produce the same output. There is no randomness, learning, or adaptive behavior involved at any stage of processing.\n\nNo AI models are used to interpret, infer, or modify your CSS. All results are generated using explicit, predefined logic so you always know what to expect and can rely on the output in automated or repeatable workflows.',
    ...
    faq: [
      ...
      { question: 'Is my CSS sent to a server?', answer: 'No. Your input is not stored, logged, or transmitted. Processing happens entirely in your browser.' },
      { question: 'Will this tool change my CSS logic?', answer: 'No. The tool formats and beautifies without changing the actual CSS behavior.' },
      { question: 'Can I rely on the formatting results?', answer: 'Yes. The formatting is deterministic and preserves CSS semantics, suitable for real-world use.' },
    ],
  ...
  return (
    ...
    >
      <div className={`${styles.leftPanel} ${isPreviewFullscreen ? styles.hidden : ''}`}>
        <InputTabs
          selectedTool={cssFormatterTool}
          inputTabLabel="CSS"
          onActiveTabChange={() => {}}
          infoContent={null}
          tabActions={null}
          optionsContent={<ToolDescriptionContent tool={cssFormatterTool} isStandaloneMode={true} />}
          globalOptionsContent={
            <div style={{ padding: '16px' }}>
              <ToolConfigPanel
                tool={cssFormatterTool}
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={cssFormatterTool?.outputType}
          loading={loading}
          error={error}
          toolId="css-formatter"
