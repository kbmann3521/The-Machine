...
export default function XmlFormatterTool() {
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool formats, validates, and transforms XML using strict, rule-based logic. It beautifies XML with proper indentation, minifies for production, validates syntax, detects structural errors, supports XPath queries, and provides conversion to JSON and YAML. It does not attempt to infer meaning, repair fundamentally invalid XML silently, or apply creative interpretation. Every transformation follows explicit rules designed to preserve XML correctness.\n\nThe purpose of this tool is to save time, reduce human error, and make XML handling easier while remaining fully transparent about how the result was produced.',
    whyExists: 'Many XML formatters are limited, unreliable, or don\'t provide comprehensive validation. This tool exists to provide a focused, comprehensive, deterministic alternative for developers.\n\nIts goal is to give you confidence that formatting is accurate, repeatable, and suitable for real-world use—whether that means beautifying for readability, minifying for production, validating syntax, or converting to other formats.',
    commonUseCases: 'This tool is commonly used when working with XML documents that need to be formatted, validated, or transformed. It is especially useful in API development, data integration, configuration management, and deployment workflows where XML correctness matters.\n\nUsers often rely on this tool to beautify messy XML, validate before parsing, minify for transmission, debug XML structures, execute XPath queries, convert to JSON/YAML, and ensure consistency across documents.',
    deterministic: 'This tool is fully deterministic. The same input will always produce the same output. There is no randomness, learning, or adaptive behavior involved at any stage of processing.\n\nNo AI models are used to interpret, infer, or modify your XML. All results are generated using explicit, predefined logic so you always know what to expect and can rely on the output in automated or repeatable workflows.',
    ...
    faq: [
      ...
      { question: 'Is my XML sent to a server?', answer: 'No. Your input is not stored, logged, or transmitted. Processing happens entirely in your browser.' },
      { question: 'Can I query XML with XPath?', answer: 'Yes. This tool supports XPath queries to extract and manipulate XML data.' },
      { question: 'Can I convert XML to other formats?', answer: 'Yes. This tool can convert XML to JSON and YAML formats.' },
    ],
  ...
  return (
    ...
    >
      <div className={`${styles.leftPanel} ${isPreviewFullscreen ? styles.hidden : ''}`}>
        <InputTabs
          selectedTool={xmlFormatterTool}
          inputTabLabel="XML"
          onActiveTabChange={() => {}}
          infoContent={null}
          tabActions={null}
          optionsContent={<ToolDescriptionContent tool={xmlFormatterTool} isStandaloneMode={true} />}
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={xmlFormatterTool?.outputType}
          loading={loading}
          error={error}
          toolId="xml-formatter"
