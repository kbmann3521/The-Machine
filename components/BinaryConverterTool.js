...
export default function BinaryConverterTool() {
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool converts between binary strings (0s and 1s) and readable text with support for multiple character encodings (UTF-8, ASCII, UTF-16) and input formats. It auto-detects the conversion direction, validates binary input, and handles various binary formatting styles (space-separated, compact, 0b prefix). It does not attempt to infer meaning or apply creative interpretation—every conversion follows explicit rules designed to preserve data integrity.',
    whyExists: 'Manual binary-to-text conversion is error-prone and time-consuming. This tool exists to provide a focused, comprehensive, deterministic alternative for developers, students, and engineers.\n\nIts goal is to give you confidence that conversions are accurate, repeatable, and suitable for real-world use—whether that means understanding binary data, debugging low-level code, learning about encoding, or working with binary protocols.',
    commonUseCases: 'This tool is commonly used when working with binary data. It is especially useful in computer science education, protocol debugging, embedded systems, data analysis, and low-level programming.\n\nUsers often rely on this tool to decode binary strings from specifications, understand binary representation of characters, debug binary protocols, convert between formats, and verify data transformations.',
    ...
    faq: [
      ...
      { question: 'What binary input formats are supported?', answer: 'Supports space-separated (01001000 01100101), compact (0100100001100101), and 0b prefix (0b01001000 0b01100101).' },
  ...
  const binaryConverterTool = {
    ...TOOLS['binary-converter'],
    toolId: 'binary-converter',
    id: 'binary-converter',
  ...
  // Configuration for binary-converter
  ...
  // Execute the binary-converter tool
  ...
  // Build inputTabResults from output formats
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.leftPanel} ${isPreviewFullscreen ? styles.hidden : ''}`}>
        <InputTabs
          selectedTool={binaryConverterTool}
          inputTabLabel="Text"
          onActiveTabChange={() => {}}
          infoContent={null}
          tabActions={null}
          optionsContent={<ToolDescriptionContent tool={binaryConverterTool} isStandaloneMode={true} />}
          globalOptionsContent={
            <div style={{ padding: '16px' }}>
              <ToolConfigPanel
                tool={binaryConverterTool}
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={binaryConverterTool?.outputType}
          loading={loading}
          error={error}
          toolId="binary-converter"
