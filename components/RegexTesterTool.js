...
export default function RegexTesterTool() {
  ...
  const standaloneDetailedDescription = {
    whatToolDoes: 'This tool tests, debugs, and explains regular expressions using strict, rule-based logic. It performs real-time pattern matching, provides human-readable pattern explanations, detects common regex mistakes, extracts capture groups, shows match positions, and previews find-and-replace operations. It does not attempt to infer meaning, repair fundamentally invalid patterns silently, or apply creative interpretation. Every analysis follows explicit rules designed to preserve accuracy.\n\nThe purpose of this tool is to save time, reduce human error, and make regex development easier while remaining fully transparent about how the analysis was produced.',
    whyExists: 'Many regex tools are limited or unreliable. This tool exists to provide a focused, comprehensive, deterministic alternative for developers.\n\nIts goal is to give you confidence that patterns work correctly, repeatable, and suitable for real-world use—whether that means validating data, extracting information, testing replacements, debugging patterns, or learning regex syntax.',
    commonUseCases: 'This tool is commonly used when developing, testing, or debugging regular expressions. It is especially useful in form validation, data extraction, text processing, and development workflows where pattern correctness matters.\n\nUsers often rely on this tool to validate email and URL patterns, extract data from logs, test replacements before implementation, learn regex syntax, debug complex patterns, and create form validation rules.',
    deterministic: 'This tool is fully deterministic. The same input will always produce the same output. There is no randomness, learning, or adaptive behavior involved at any stage of processing.\n\nNo AI models are used to interpret, infer, or modify your patterns. All results are generated using explicit, predefined logic so you always know what to expect and can rely on the output in automated or repeatable workflows.',
    ...
    inputOutput: 'This tool expects valid or invalid regex patterns. It does not attempt to guess intent or silently correct fundamentally broken patterns.\n\nThe output is structured, consistent, and designed to be immediately usable. Any limitations or constraints are intentional and exist to preserve correctness rather than introduce uncertainty.',
    whoFor: 'This tool is intended for users who value accuracy, transparency, and control. It is ideal for developers, system administrators, security professionals, and anyone working with regular expressions.\n\nIt is not intended for creative rewriting, content generation, or automated decision-making. If you need interpretive or generative behavior, this tool is intentionally not designed for that purpose.',
    reliability: 'Because this tool is deterministic and rule-based, it can be safely used in professional and production-adjacent environments. You always know what patterns match and why.\n\nThis makes it suitable for validation, debugging, testing, and quality assurance tasks where unpredictable behavior would be unacceptable.',
    faq: [
      ...
      { question: 'Will this tool modify my pattern?', answer: 'No. The tool analyzes and tests without modifying your pattern. You control all options.' },
      { question: 'Can I rely on the matching results?', answer: 'Yes. The matching uses standard JavaScript regex engine, making it suitable for real-world use.' },
    ],
  ...
  // Configuration for regex-tester
  ...
  return (
    ...
    >
      ...
      <div className={styles.rightPanel}>
        <ToolOutputPanel
          result={outputResult}
          outputType={regexTesterTool?.outputType}
          loading={loading}
          error={error}
          toolId="regex-tester"
